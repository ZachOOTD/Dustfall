...
**ALL FOUR user directions + "wider/weirder ships" now shipped** (non-ship industrial = tank_cluster; satellites; debris; hollow husks; weirder ships = derelict). **Deferred (honest, backlog §G):** RETIRING the legacy linear ship in favour of the socket derelict (optional — the derelict already delivers the weird-ship value without the regression risk of migrating the heavily-refined ACAZ ship; then the composite share → 1.0); a formal COLLIDER-AUDIT harness; yard cluster-merge (cut the ~3193 ground-view); sev-2/3 polish (tank terracotta→steel gradient, fleet palette cohesion, pipe-strut wiring, sand-drift banking, husk exterior detail, derelict wide-body refinement). **OWED:** the attended in-world walk-test (felt collision + variety + seating under sky/fog — verified headless only).
