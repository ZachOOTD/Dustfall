...
- **Sliced terrain-tile builds** — `buildTile` refactored into staged fns (`fillRows` → `assembleGeometry` → `finalizeTile`) sharing ONE code path: the boot ring + the anchor tile still build synchronously (fall-through protection, byte-identical), while ring tiles now build as a single in-flight background job advancing ONE stage-step per frame (`TERRAIN_SLICE_ROWS` 12 rows ≈ 8-10ms; geometry+normals one ~17ms frame; mesh+collider finalize ATOMIC — no partial tile ever in the scene, rule 9). The fill merges the old heights+colors passes into one row sweep (pure per-vertex fns → byte-identical arrays; the shared index buffer is built once). MEASURED before/after: the old synchronous bake was ~90-200ms × 3 tiles per crossing; the worst sliced step is ~10-25ms headless-swiftshader (much less in real play).
- **Bounded chunk loads** — `CHUNK_LOADS_PER_FRAME` 2→1 (a POI chunk load measured 20-57ms — one per frame max). **wreck_knot landmarks render DEFERRED**: the 5 heavy pieces (3 wreck assemblies + 2 ribcages) queue as thunks executed ONE per frame — all rng draws happen at load time in the original order, each piece gets its own derived seed, so pieces are deterministic regardless of execution frame (measured ~7-8ms/piece staged vs a potential ~150ms single-frame knot).
- **NEW permanent gate: `chunk-perf`** (3rd leg of `verify:chunks`) — a multi-km teleport walk routed through a real wreck_knot (wide descriptor scan — an unexercised deferred path is the cycle-3 lesson) asserting: slicing is the NORM (≥100 slice steps) with sync anchor-safety bakes RARE (≤4 at teleport pace — the probe moves ~100× play speed, so occasional anchor catches are the safety net working; a real regression does 30+) + bounded (≤250ms); slice steps ≤60ms; chunk loads ≤120ms; landmark pieces ≤120ms; draw calls + bodies ceilinged and baseline-exact at home. Tripwire thresholds per D291 (gross-regression detectors, machine-load flake-resistant).
- **Perf instrumentation (permanent)** — `terrain.perfStats()`/`resetPerf` + chunk load maxima in `chunks.stats().perf`, exposed via `__game.chunkPerf()`/`resetChunkPerf`.

## Campaign "Infinite Sands" cycle 4 — 2026-07-11 — S4 distributed landmarks + per-region biomes: the far field has DESTINATIONS ✓ all gates

`verified` — `verify:all` PASS first-pass (tsc + placement 5-seed 0-fails + colliders 55 + chunks: determinism ×2 + cross-seed, streaming bodies 332→332 EXACT incl. a landmark-site walk leg); all 5 smokes PASS; VISUAL: the colossal-ribcage landmark is a genuine hero read (a 30m titan skeleton arcing the dune crest, player-eye); regional wreck-yard ground-truth PROVEN by vertex-color diag (wy=1, biome=wreck_yard, ashen mottle baked at 8m). One cycle commit. D295. **Mid-cycle interruption**: the machine-slowdown investigation (leaked headless browsers — the reap-orphans fix, now framework-wide) paused this cycle before its final suite; resumed + closed after the fix landed.

- **Distributed hero landmarks (a)** — a REGION grid (16 chunks = 1792m) rolls at most one landmark per region (`CHUNK_LANDMARK_CHANCE` 0.3 → a destination within ~1.5-2km of anywhere) from a pure per-region hash; the hosting CHUNK renders it on the normal lifecycle. Two kinds, both reusing existing art: **`colossal_ribcage`** (placeRibcage at 5-8× scale — collider box scales WITH the mesh, rule 9; a titan skeleton breaching the dunes) and **`wreck_knot`** (a tight 3-wreck salvage triangle + 2 carcasses — a REWARD destination; salvage transient per D292). `placeRibcage` gained `scale` + returns `{group, collider}` (boot callers byte-identical). This is Skyfall's future plug-in slot.
- **Per-region wreck-yard biomes (b)** — `biomes.ts` rolls rare regional wreck-yard anchors (`WRECK_YARD_REGION_CHANCE` 0.08/region) from ONE APPENDED rand draw (all prior anchor draws byte-stable) + a pure region hash, memoized; `wreckYardAt` = max(origin falloff, 3×3 regional neighbors) → far graveyards get the ashen mottled ground, terrain flatten, `wreck_yard` biome id → the graveyard POI-mix weights AND a 6× POI density (a yard should feel crowded) — all for free through existing paths. Anchors keep ≥2200m from the origin (the ±1200m boot ring corner is 1697m) so the released world bakes byte-identically — placement gate 0-fails ×5 seeds is the tripwire, green.
- **Gates extended** — landmark descriptor↔render equality over the active ring; a landmark-site walk leg; a ±15km pure-biome scan asserting regional yards exist; vista landmark + regional-yard shots (+ a vertex-color yard diagnostic).
- **Known-deferred** — regional yards lack the origin yard's dense boot-style CLUSTER (backlog `[polish]`, knob: the ×6); streamed-landmark horizon silhouettes still skipped (S2 note stands).
...
- **Streamed POI wrecks (`chunkManager.ts` + the D290 descriptor model)** — `ChunkDesc` gains a fixed-shape `poi` roll from a DEDICATED per-chunk rng (presence `CHUNK_POI_CHANCE` 0.07 ≈ the origin field's density; position inside a 25m edge margin so seam neighbors can't kiss; biome-weighted archetype via the real `pickArchetype`; a fresh `renderSeed`). Chunks inside `CHUNK_POI_ORIGIN_EXCLUSION_M` (1250m) roll EMPTY — boot placement owns the origin field untouched. `loadChunk` renders the descriptor via the REAL `placeProcgenPOI` (forced archetype, biome, `parent: group`, fresh rng) — panels mount, salvage registers, per-POI static merge, declared colliders; `unloadChunk` removes the POI body, splices this chunk's salvage entries from the registry, disposes merge-output geometry (panel/component geometry + ALL wreck materials are shared — never disposed).
- **Save safety (the S2 danger zone)** — streamed wrecks are `transient`: excluded from save serialization (their registry ids are load-order-dependent; a persisted id could silently patch the WRONG wreck after reload — the meteorCrash precedent). v1 semantics = regenerate pristine (per-chunk diffs are S5's rung). The streaming probe now SAVES at the far point with streamed salvage live and asserts no transient id reaches `save.salvageables` + the saved count equals the boot count. Save schema v16 untouched.
- **Deliberately skipped vs the boot path** — `addHorizonSilhouette` (module-global, no removal path — S4's landmark concern) and the scrap-debris ring (pickup ids are save-coupled; S5).
- **`placeProcgenComposite` now stashes `userData.poiBody`** (the 'ship' archetype delegate never captured its compound-collider body — streamed ships would have leaked it on unload); loadChunk normalizes the archetype stamp so stats/probes count every streamed POI.
- **Gate hardening** — `verify-chunks.mjs`: the streaming child gets a 900s spawnSync timeout (420s KILLED it mid-probe under machine load, which read as a boot failure AND leaked the child's dev server — the taskkill teardown never ran); probe additions: descriptor↔render POI count check, salvage-registry leak assert, world-space content snapshots (POI placement identity across unload/reload), POI-aware castDown clear point; `chunk-vista` gains the streamed-POI player-eye shot.

## Campaign "Infinite Sands" cycle 1 — 2026-07-11 — S1 ChunkManager spike: the world STREAMS ✓ all gates + 2 NEW permanent probes

`verified` — `verify:all` PASS (tsc + placement 5-seed 0-fails + colliders 55 + NEW `verify:chunks`: chunk-determinism 2 seeds + cross-seed digest distinctness, chunk-streaming/leak walk); all 5 smoke gates PASS (smoke-intro `{ok,beats:12}`, smoke-pod-tutorial, pickup-take-sweep, survival-probe, diurnal-probe); VISUAL sanity `chunk-vista` ×4 player-eye shots at/past +1500m (continuous ground, grounded markers, no seams). One cycle commit. D288–D291.

- **The ground is INFINITE (terrain streams)** — `terrain.ts` refactored from a fixed 3×3×800m boot grid to an ANCHOR-MARGIN tile ring: tiles keyed by integer coords in an unbounded grid, a 3×3 ring follows the player, tiles outside dispose fully (geometry + Rapier heightfield body). The anchor re-centers only when the player moves `TERRAIN_ANCHOR_MARGIN_M` (24m) past the anchor tile's edge — boundary straddling can never thrash a 37k-vert rebuild. The initial ring is byte-identical to the old fixed grid (placement gate stayed 0-fails on all 5 seeds — the released intro region is untouched). ONE shared terrain material across tiles (the per-mesh material registered a never-released shader ref in `_shaderRefs` — a real leak under streaming). `heightAt` = baked bilinear inside loaded tiles (collider-exact), closed-form `computeHeightAt` fallback anywhere else — height is now queryable at ANY coordinate.
- **ChunkManager (NEW `src/world/chunkManager.ts`)** — the content-streaming grid: 112m chunks, load radius 3 (Chebyshev), same anchor-margin model (8m). Full teardown on unload (meshes disposed, every Rapier body removed — rule 9). `chunkSeed(worldSeed,cx,cz)` murmur-style avalanche; `describeChunk` derives a chunk's full content descriptor PURELY from that seed (loading only renders it) — the determinism law extended to the infinite grid. S1 spike content = marker posts (seed-tinted, satellite-varied, real colliders), OFF by default (`localStorage['dustfall.chunkMarkers']` / `__game.setChunkMarkers`); S2 hangs real POIs on this lifecycle. Ticked via `updateChunks` after `updatePlayer`; inert during the intro (`ctx.intro?.active`).
- **2 NEW permanent gates (in `verify:all` via `npm run verify:chunks`)** — `chunk-determinism` (descriptors byte-identical across derivations at 8 coord spreads incl. ±90071; adjacent-chunk distinctness; cross-seed digest distinctness in the wrapper) + `chunk-streaming` (walks +1500m past the old ±1200m world edge and back TWICE: terrain follows + collider agrees with heightAt at distance, active set bounded, no seam duplicates, unload→reload regenerates byte-identical marker positions, global Rapier body count returns to baseline ±3).
- **Debug hooks** — `__game.chunkDescribe/chunkStats/setChunkMarkers` + `resetWormCrossing` (kills an in-flight dorsal-ridge crossing — its ~480m humps photobombed the first vista shot).
- **Probe/tooling hardening** — rig-shot `startDev` timeout 30s→120s (a cold Vite boot exceeds 30s when the machine is loaded — every probe flaked); walk-length numeric probes shrink the render target to 320×240 (swiftshader fill made the first streaming run take ~18 min); `chunk-vista` scenario added (crest-framed player-eye shots + a fwd-ray identify diagnostic + height-profile print).
- **Known-deferred (by ladder design)** — the ~100-200ms terrain-tile bake blocks its frame (3 tiles per 800m crossing, budgeted 1/frame) → S6 frame-budgeted generation; wordless scenes/rocks/POIs/creatures still origin-bound → S2/S3/S4.
...
- **Pickup instancing (D281)** — seed-spawned branch + scrap (367 individual Meshes ≈ 43% of probe draw calls) now render via shared `InstancedMesh` pools (1 scrap pool + 5 branch-variant pools; capacity 384/96·5, overflow falls back to legacy meshes). The interaction raycast resolves instanced hits via `intersection.instanceId → imesh.userData.instanceToPickupId` (interaction.ts) instead of the per-mesh `pickupId` parent-walk; `despawnPickup` frees the slot (swap-with-last, dense `[0,count)`, moved pickup's index fixed up). Probe: drawCalls 852→**817** at the probe camera (the modest delta is per-mesh frustum culling already hiding ~330 off-screen pickups; the real win is worst-case flattening — a dense field view no longer costs 1 draw/pickup); tris +70K (`frustumCulled=false`, acceptable — draw calls were the bottleneck); rand-draw order preserved (seeded worldgen byte-identical). NEW permanent gate `pickup-take-sweep` (rig-shot): teleport → aim → real KeyE per pool, swap-fixup + id↔slot bijection invariants. Sweep-harness diagnostics that mattered: stale `ctx.player.body` after `enterGame` rebuild; enterLive defaults 3P (spring-arm cam); the body keeps settling post-teleport so aim must re-pin per frame; a gated `__interactionDebug` tap now exposes the game ray's raw hits.
- **Housekeeping** — decisions.md archival (D221–D235 → archive, 280=280 conserved, both headers updated); panel dead-code sweep (mostly already shipped in the 2026-06-18 campaign — the backlog entry was stale; deleted the one truly-dead `clearPanelDebris`, kept the live `SALVAGE_PANEL_INTERIOR_V2` fallbacks); `survivalProbe` crash-heat determinism guard (throws if live crash sites would perturb `crashHeatAt`); scrubbed the forbidden "endgame/Long-Storm finale" candidate from CLAUDE.md/roadmap/next-session-prompt (2026-06-18 no-finale directive; ledger mentions removed too).
- **M2 discovery (feature-audit win)** — the campaign ladder's M2 "survival curve" was **already built + probe-pinned in C38/D246** (and M3's sun-shade half in C31); the vision-gap analysis that motivated it read stale state. M2 re-scoped to verify-only (probe re-run green this cycle); the survival-curve FEEL walk-test remains a human item at the campaign end review. M3 re-scoped to: the C31 occluder-threshold decouple + water-scarcity/exposure.
...
- **Crafting — PICKUP-GATED discovery (D277; replaces the Session TT combine-to-discover input slots).** Discovery is now coupled to GATHERING, not guessing a combo: a recipe unlocks once the player has ever collected all of its ingredient TYPES (counts gate craftability, not the unlock). NEW `InventoryState.collectedItemTypes: Set<ItemId>` — SAVE_VERSION 15→**16** (additive; pre-v16 saves seed the set from their discovered recipes' ingredients so nothing re-locks). `recipeCardState` + `unlockNewlyEligible` in `recipeDiscovery.ts`, hooked into `inventory.addItem` (every pickup/craft/loot → record type + unlock + a batched discovery chime/toast). "Collected = obtained by ANY means" so crafting a rope unlocks the rope-dependent recipes (verified end-to-end).
- **Crafting UI — card grid.** `craftingMenu.ts` rewritten from 4 input slots → a category-grouped CARD GRID (cold `?` → warm tease "scrap + ?" → unlocked) + a detail footer (ingredients→output, have/need, item description, CRAFT); dev free-craft preserved; themed scrollbar/states (screenshot-iterated). DELETED `recipeBookPanel.ts` (folded into the grid; TAB now opens the grid too); dissolved the multi-match chooser + partial-match hints (fire_kit/signal_flare now just two independent cards). Retired the dead `matchRecipes`/`partialMatchRecipes`/`canonicalInputKey`/`__registerTestRecipe` helpers + the input-slot CSS. Intro machete cue reworded (gather→craft). **Net −723 LOC.** New dev hooks: `giveItem`/`craftCardState`/`openCrafting`/`loadGame`.
- **Desktop packaging — Tauri v2 (D278).** The game is now an installable desktop app. NEW `src-tauri/` (`identifier io.github.zachootd.dustfall`, 1280×800 window, `csp:null` so Rapier WASM / blob workers / Web Audio aren't blocked, `mainBinaryName Dustfall`, NSIS installer, on-brand sun-over-dunes icon). `build:desktop` = `cross-env VITE_BASE=/ vite build` (the GH-Pages `/Dustfall/` build stays byte-identical); `tauri:dev`/`tauri:build` scripts. Toolchain installed on this machine (Rust `stable-msvc` + VS 2022 Build Tools). Artifacts: **Dustfall.exe 9.3 MB** + **Dustfall_0.1.0_x64-setup.exe 2.9 MB**; launch-tested (13 WebView2 procs, per-app localStorage save dir). Follow-ups (backlog): code signing, file-based saves, CSP tighten, WebView2 re-profile.

## Escape-pod intro — 2026-07-07 (attended, live-feedback round ABQ) — cockpit console + pod interior + audio + airlock fix batch ✓ tsc clean

`verified` — `npm run verify` (tsc --noEmit) clean. A long attended live-feedback session across the ship cockpit + pod interior; each fix probe-/render-verified via the ship-shot / pod harnesses (D272-D276). Two commits: the hero **cockpit console rework** (`f17fce6`) then this batch.

- **Cockpit console (hero):** rebuilt as 3 straight ANGULAR panels chorded off the dome sill (front + two sides parallel to their glass), clean mitred deck + folded fascia corners (no thick posts), fascia canted back, instruments mounted proud (green MFD + amber readouts + throttle + LED annunciator grids on the front, rocker-breaker banks + amber screens + blue nav units on the sides). Moved **closer to the glass** (`INSET 0.42→0.24`, verified exterior + graze: no pane poke-through). Personal clutter (photo/mug) re-anchored to `_consoleShelfL/R` so it tracks INSET instead of floating (D272).
- **Pod brown floor — a stray model, not lighting (D273):** a raycast probe found the charred heat-shield `baseCap` (`_podScorchMat`) sat at `y=0.11`, poking ~18cm ABOVE the interior deck in the board + landed (full-pod) builds; the descent uses an interior-only cabin so only it read grey. Dropped both base caps to `y=−0.15` (below the deck). Reverted the earlier lighting-based misdiagnosis (deck-fill/wake tweaks).
- **Pod white splotches:** the shader "bare-metal scuff flecks" (`fleckStrength`) read as white speckle on an INTERIOR; zeroed flecks on all 7 interior mats + the 6 exterior hull mats (the doorway splotches). Deck given a cool `rustHex` so it no longer reads rusty under warm light.
- **Pod floor z-fight flicker (D274):** the grey deck + dark sub-floor were coplanar (both tops at `y=0.05`) → radial-streak flicker on camera move; dropped the sub-floor top 1cm. Also stripped the deck streak/seam-rust layers ("streaks on the floor") and removed the **footwell** disc (the "circular thing on the floor").
- **Eject lever redesigned:** the guarded housing + cage + hazard stripes + clevis + T-handle → a simple pull-down handle (backplate + pivot + arm + red grip); animation contract unchanged.
- **Rounded airlock → plain hallway:** removed the gappy bellows sleeve + the exterior mating shroud (skirt rings/seal/clamp band/arms) — these also crossed the pod-door **porthole view before eject** (the docking bands the user saw). The boxy collar housing + the pod hull frame the door. Door confirmed ONE `buildUnifiedDoorSlab` across board/descent/landed (the divergent blown-door is debug-only).
- **Audio:** desert wind removed (`WIND_BODY_MASTER`/`WIND_WHISTLE_MASTER → 0` + `startDesertWind` call commented) + ship hum removed (`startCockpitHum` call commented) — both reversibly.
- **Cockpit glass:** closed a gap-into-space under the sill (the kick skirt ran at the outboard footprint but the floor is inset 0.06 → a slit; now slopes inboard to the floor edge + below); added a slight haze (`opacity 0.05→0.09`, glaze `0.62→1.05`).
- **Airlock corner z-fight (D275):** the `--probe` (no-merge) pinned it — the collar SIDE WALL (`escapePodBay`) coplanar with the ship's −X corridor wall (`#686e73`); trimmed the 5cm overhang + polygonOffset. (Cost 3 wrong guesses first → memory `ship-zfight-probe-first`; D276.)
- **Tooling:** `scripts/floor-probe.mjs` (raycast floor-mesh check — cracked the brown-floor + z-fight diagnoses), `scripts/pod-floor-shot.mjs` (floor-down + eject-wall capture), ship-shot `--airlock=open` + an all-ray-hits diag + `bay-appr`/`bay-collar`/`con-sillfloor`/`airlock-leaf`/`collar-rwall`/`collar-lwall` views.

## Escape-pod intro — 2026-07-06 (overnight, autonomous) — the Z-queue: 10-unit playtest-fix round on the released intro ✓ full gate suite + build:intro

`verified` — the full intro gate suite PASS on the merged tree: smoke-intro `{ok:true,beats:12}`, door-flush-audit (9 states), pod-seal-sweep (0 cracks all yaws), pod-rotation-clearance (0 violations across the 0→π/2 sweep), airlock-motion, quarters-walk, zfight engine-glass (hardPct 0), cockpit-glass-cells (every cell 0.0% dev), pod-walkin, pod-walkout (foreignGlobals=[] — no world-light bleed), doorway-torture (6/6 after a harness fix), verify:colliders, and `build:intro`. The two hero visuals each cleared an adversarial gate (5 lens critics + refuters) + a fix pass. From the user's round-3 playtest (5 screenshots + verbal notes), executed fully autonomously overnight across parallel podScene/shipScene modeler agents (per-unit commits):
- **Z1 `9d594fa`** — cockpit glass haze PARITY per-cell (root cause: winding/DoubleSide → all 12 cells 0.0% dev) + the crown keel re-centred.
- **Z5a+Z7 `7b1ec69`** — the eject rotation 180°→90° (frames the planet + ship flank through the porthole) + the wake door-kick is now PLAYER-GATED (the auto-timer removed, per "nothing on a timer").
- **Z9+Z10 `67fd140`** — the far-space approach time-warped 0.55× (was too fast/unreal) + the blackouts trimmed (phase 2.3→1.6s, wake 2.5/1.2→1.8/0.8).
- **Z6 `8ff11a8`** — pod ONE-model parity: the explosion blast-flood → a window flare, plasma/shimmer ~40% slower (the descent "fast light textures" glitch), a local deck-fill PointLight.
- **Z5b `cb99682`** — the sealed-door edge gaps (4th report) ended STRUCTURALLY: the shell is cut ~6cm wider than the flat slab each side; `_addDoorStopLip`→`_addDoorSeal` box-section jamb returns on all 4 sides, both doors. New permanent pod-seal-sweep gate (0 cracks; catches 156/yaw without the seal).
- **Z4 `49a20fc`** — "ship models in the pod": the umbilical plate + hoses were authored 0.5–0.6m INSIDE the hull (masked at yaw 0, swept visible on rotation) → relocated to the collar flank. New pod-rotation-clearance gate (0 intrusions across the sweep). Airlock control-panel readout was mounted backwards-in-the-wall → flipped proud.
- **Z2 `ee46082` (+tooling `04e8941`)** — crew-quarters full sci-fi overhaul (hero): built-in berth, locker bank, fold-down desk, base cabinet, panels/conduit/vent; the "back-wall overlap" killed; the scene-global HemisphereLight (desert-bleed class) removed; 4 rule-9 furniture colliders. Adversarial gate 0 SEV1; 3 SEV2s (a returning header-cap seam, a cube pillow, a dead wall) then fixed.
- **Z8 `04e8941`** — parachute round 2: lines EMBED into the runtime brim (root: build-vs-runtime squash + grp/dome parent split), flutter 10s→1.6s (pop→settled 5.65s), a per-vertex gravity drape wrapping the hull to a ground pool.
- **Z3 `9d18da4`** — the engine bay's "blocky cylinders" → a real reactor hall (hero): containment column + emissive core channel, coils, coolant towers, manifold, control station, hazard rail; CALM=cool cyan / CRITICAL=hot-orange breach via setEngineFire. Adversarial gate found + fixed 2 SEV1 (a dead-grey calm core → emissive cyan + local light; the glass mullion bisecting the hero core → seamless pane) + a collar overhang.
- **doorway-torture harness `af5c51e`** — a TEST regression (not a game bug): Z7's movement-gated wake advance broke the stale `scratch.t=999` time-forcing → false "doorway blocked" FAIL. Bisected to the pre-queue baseline (passed there); pod-walkin/out prove the landed pod is walk-in-able. Fixed the harness to reach step-out by walking (mirroring pod-walkout); the 6× assertion unchanged and now genuinely passes.

## Escape-pod intro — 2026-07-01 (overnight) — hardening: full-intro coherence + audio wiring ✓ verify:all + smoke

`verified` — verify:all + smoke `{ok:true,beats:12}`. Two hardening passes on the feature-complete intro. **Coherence** (`768890f`): re-shot every beat with the now-faithful rig, caught the biggest cross-beat defect — the ride-down/wake POD CABIN read as pale near-white plastic (a tonal break from the worn dark cockpit/corridor/bay). Two root causes fixed: the desert noon sun+ambient followed the pod up to orbit and flooded the cabin (now dimmed in vacuum via `applySpaceMode` — physically correct), and the shell base color was too light (`0xa3a8ac`→`0x71767a` worn aluminium). Now the whole intro shares one worn-industrial tone; the descent/explosion vistas are framed by a cabin that belongs to the ship. Flagged for walk-test: the wake-interior lighting ramp on the direct-jump path, the explosion husk porthole background. **Audio** (`5d8bc4c`): audited every beat + filled the silent gaps — 6 new procedural synths (engine-fire loop, explosive-bolt shear, hatch pressure-seal, a sustained ship-death roar+tail, a near-subliminal desert wind, an awe-swell for the reveal), wired at the right beats, leak-safe. Every beat now scored; the mix balance = the user's LISTEN pass.

## Escape-pod intro — 2026-07-01 (overnight) — polish batch + Phase-3 → **INTRO FEATURE-COMPLETE** ✓ verify:all + smoke

`verified` — verify:all + smoke `{ok:true,beats:12}` each step. After REBUILD v2, a polish + last-beat batch that makes **every beat of the vision built**: **pod-bay** greybox-plus → corridor bar (`3fb8daa`); the **Beached Leviathan** horizon-hook (`90b8749`) — a colossal wrecked ship on the dawn horizon making the step-out reveal land ("go there"), gated behind `FEATURES.escapePodIntro` (doesn't touch the live master world); the **chute-pop** bigger/comic billow (`78359ba`); a **bug-hunt** (`bf07847`) → the intro code essentially clean; and **Phase-3 — the SHIP EXPLOSION** (`44c7354`): the vision's climactic beat (was a flash) — the player watches their hauler die through the porthole (a churning fireball + 38-chunk debris field + shockwave + cabin flash, pod held level, no tumble). The hauler mesh already existed (T3.1, gated) but was never wired in — this wired it + built the explosion FX. **The whole intro now flows: orbit → disaster → eject → watch-the-ship-die → the beautiful descent → the failed parachute → crash → wake (same pod) → step out → the horizon reveal → craft → salvage → chute-pop.** Remaining = FEEL walk-tests + minor art-direction polish (the user's domain).

## Escape-pod REBUILD v2 — 2026-07-01 (overnight) — R5 FINISH: material + corridor + pod-bay → **REBUILD v2 COMPLETE** ✓ verify:all + smoke

`verified` — verify:all PASS + smoke `{ok:true,beats:12}` at each step. **The whole REBUILD v2 real-world physical intro (R1–R5) is now built end-to-end.** This overnight (user: "work through the night, lots of progress"): **R5a-finish** (`65af5eb`) — the user's rugged/matte hull steering (roughness↑ metalness↓ envMap↓ + grime), and the 4-round seat "tan wedge" root-caused (it was the harness straps, not the bolsters → dark oxblood + slimmed). **R5b** (`49b4e08`) — the greybox corridor → a fully-modelled freighter passage (rib frames, deck plating, conduit/cable, hatches, grab-rails, real can-lights, cockpit materials reused) with real fire + red-alert disaster lighting. **R5c** (`fb7cb6f`) — the pod docked in a hazard-framed airlock bay; `tickEnterPod` is now a physical walk-up → continuous scripted climb-in → hatch-seal (no teleport, R3a cabin swapped under the dim); `tickShipExplode` fires explosive-bolt release before the blast → the R1b descent. Cost-scoped the R5 gates (own-eyes + verify:all for walked-through/flow pieces; reserved the full 5-critic gate for the hero opening frame). **Residuals for the user's review:** planet framing (being done next), the pod-bay + cockpit look polish, and all the motion/feel walk-tests (climb-in, eject, corridor strobe, the beautiful descent).

## Escape-pod REBUILD v2 — 2026-06-30 (attended) — R5a: hero cockpit redesign (box → worn fuselage, 4-gate loop) — SHIPPED as strong default → ⏸ USER-PAUSED

`verified` — tsc + verify:all + smoke `{ok:true,beats:12}`. **R5a redesigned the greybox-box cockpit into a real worn industrial-hauler spaceship cockpit** (commit `07fa124`), driven through a **4-round adversarial visual-gate loop** (beauty 4 → 5.2 → 6.5 → 6.75). Built: a lofted vaulted ribbed **D-section fuselage** tapering to the nose, a raked mullioned **windscreen** (glazing specular streaks), a multi-tier **asymmetric avionics console** (killed a grinning-face pareidolia), a real **crash-seat** (bucket + bolsters + headrest + 5-point harness + central buckle + slim converging straps — root-caused the "leaning over a console" read: the straps were too wide/close → tan wedges), a deep recessed bulkhead **doorway**, and cool **neutral metal** (killed a "wine-barrel" wood read). The adversarial gate (5 critics + a code-auditor, run 3×) repeatedly caught real defects under confident builder self-grades (a 5.2 under a claimed 8). Contract byte-identical. **⏸ USER-PAUSED** ("pause where we are, pick it up later") mid a rugged/matte material pass (user steering: the metal reads "too pristine/shiny"). **Residuals → the user's art-direction:** finish the matte-material pass · the FP strapped-in seat read (plateaued) · the planet frames the windscreen too large/flat at the cockpit beat (R2). **Unbuilt:** R5b corridor, R5c pod-bay + enter/eject. Resume via `/campaign-approve` or steer.
...
**T3.3 — the COCKPIT INTERIOR** (the game's opening shot — Beat 0, seated in the worn single-pilot bridge). The procedural-modeler reworked the greybox cockpit in `src/world/escapePodIntro/shipScene.ts` into a hero single-pilot bridge: a worn riveted-aluminium shell, a big FORWARD WINDOW framing a self-contained orbit view (curved desert planet + terminator + atmosphere limb + stars), a pilot SEAT + side-stick + armrest foreground, a CONSOLE bank (green "ORBIT ACHIEVED" CRT + gauges + telltales), the PERSONAL TOUCH (photo/mug/token), a bulkhead DOOR, warm self-contained lighting. New `setCockpitAlert(0|1|2)` + `cockpitAlertLevel()` hooks (the E2 escalation, wired in T3.4). Contracts preserved (`getShipSpawn` moved forward to the dash; the Z thresholds + walkable colliders + dispose intact); `__game.setCockpitAlert` + `--scenario=cockpit` rig.
- **DRAMATICALLY improved over greybox** (the opening shot now reads as "a lone pilot seated at the worn controls, looking out at the planet below" — the planet dominates the window, the dash foreground reads). The C20 redesign found the real root causes the first pass missed: the spawn was 2.75m back from the dash (bare floor) + the rig wasn't killing world weather (the "dirty-lens" smears were rain/dust). Both fixed + a surface/lighting/prop pass.
- **⚠ BELOW THE HERO BAR — DEFERRED to user art-direction (like the hauler).** The adversarial 4-critic gate FAILED it twice (beauty 4→5): the CODE auditor passed (beauty 8 — geometry/lighting/hooks sound) but the visual critics flag persistent **surface fidelity** (flat untextured walls, no rivets-as-geometry, blank CRT text, candy-button gauges, blank photo, flat-ish lighting) + the planet cropped too big. The autonomous modeler plateaus on hero surface-quality.

### ⚠ STRATEGIC PIVOT (C20) — overnight builds the PLAYABLE INTRO; hero-VISUALS → the user's morning
Two consecutive Phase-3 hero VISUAL assets (C19 hauler exterior, C20 cockpit interior) plateaued below the released-game bar across multiple modeler rounds + adversarial gates — and BOTH are assets the user will art-direct regardless (the hero ship + the opening shot; precedent: the descent took 5 user feedback rounds). So the overnight loop **pivots from "grind the modeler on each hero visual" to "build the full playable intro (logic/staging/audio)"** — the work the main loop does well + that does NOT need the user's eye. **Deferred to the user's morning art-direction:** the hauler exterior (C19), the cockpit surface-fidelity (C20), the hero corridor geometry, the explosion FX polish — each with the gate's specific issue list. **Overnight builds:** T3.4 disaster STAGING (over the greybox corridor) · Phase 4 (wake-in-pod + blow-door, desert reveal, craft+salvage tutorial, chute-pop) · Phase 5 (audio). **Goal:** the user wakes to a fully-playable intro end-to-end + a clear hero-visual art-direction list. (Recorded in roadmap + next-session-prompt + campaign-state.)
...
**more-wreck-types-new-pois COMPLETE — the WELL + the DEBRIS-TRAIL.** Finishes ⑥ (the watchtower shipped C42); 3 new non-ship POIs now vary the scatter beyond ships.
- **WELL** (a scavenger water cache) — NEW `wellHead()` component (`world/poiComponents.ts`): a metal CURB ring (taller, irregular salvaged rim-plates, a chunky cap band) around a **recessed dark shaft** (an inner wall dropping into shadow + a near-black void floor → reads as a hole, not a lid), with a 2-post WINCH over it (a thick windlass DRUM + an offset L-crank + grip + a rope + a banded hanging bucket). Curb + the 2 posts declare colliders; the rest is `isWreckDecoration`. NEW `assembleWell` archetype (`bucket:'dark', burySink:false, list:0.03, seatSink:0.10, salvageKind:'cargo_container'`).
- **DEBRIS-TRAIL** (a directional crash-ejecta streak, vs `debris_field`'s scattered disc) — NEW `assembleDebrisTrail`: a heavy lootable IMPACT chunk (1.7× scale, tilted, bedded deep) at the deep end with a dark skid streak + crater, and debris trailing back getting SMALLER (a strict monotonic gradient on a tight centreline). Reuses `debrisPiece`, now extended with a **`scale` param** (multiplies geo + collider + bbox together; default 1 → `assembleDebris` byte-identical). NEW dark `_skidMat` (the muted `_scorchMat` was invisible at grazing angle).
- **Weights/audit:** `well` (~0.04/biome, rarest in the wreck_yard) + `debris_trail` (~0.06, peaks in the wreck_yard) added to `ARCH_WEIGHTS` + the `collider-audit` list. **D251.**
- **Owed/backlog:** sev-3 nits (well: s7 crank ambiguity · thin rope · subtle rim-plates at distance; trail: s7 impact-chunk dominance + faint crater · s42 compressed gradient). The walkable-ramp + well/trail FEEL → walk-test. **⑦ walkable-wreck-interiors is the last M7 unit.**

## Campaign C42 — 2026-06-20 — M7: **more-wreck-types-new-pois `[partial]`** — a NEW WATCHTOWER landmark (a walkable ramp-vantage) joins the scatter ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders **0/30** — the watchtower was added to the collider-audit list, its legs/deck/ramp colliders cover all structural meshes; phash-deterministic, one `seedOf` draw). **Visual gate PASS in 2 ROUNDS** (2 critics): r1 FAIL/split (too squat → a raised hide · a tiny floating-parasol roof · a narrow ambiguous ramp) → r2 (TALLER tower [~1.6×] + diagonal X-bracing + thicker legs · a WIDE roof overhanging the deck on visible posts · a wider ramp overlapping the deck + a mid-support strut) → PASS, reads as an intentional scavenged lookout. **No save bump** (additive archetype, D227). Cycle 42/75. **M7 unit ⑥ `[partial]` — watchtower done; the well + debris-trail continue next cycle.**

**more-wreck-types-new-pois `[partial]` — the WATCHTOWER.** Adds NON-ship variety to the procedural scatter via the socket grammar: a scavenged-metal desert LOOKOUT — 4 braced corner legs (cross-ties + diagonal braces) carrying a railed deck under a wide hipped sun-shade roof, with a straight **walkable RAMP** up to the deck (a "ramp-vantage", NOT a ladder movement-mode — D250).
- **NEW component** (`world/poiComponents.ts`): `watchtower(seed)` — a single unitary structure (legs/deck/ramp declare colliders; braces/railing/roof/posts/side-rails/mid-strut are `isWreckDecoration` → collider-audit-exempt). The RAMP is a real inclined box collider at **~37° slope** (under the KCC ~50° climb limit) so the player can walk up it (feel → walk-test).
- **NEW archetype** (`world/poiArchetypes.ts`): `assembleWatchtower` (single-component, stands ~level — `burySink:false`, a tiny `list:0.04` so the ramp stays walkable, a `seatSink` + a windward drift) + registered in `ARCHETYPES` + `ARCH_WEIGHTS` (~0.07 per biome, rarest in the wreck_yard — a graveyard, not a settlement).
- **Audit:** added `watchtower` to the `collider-audit` default list (`scripts/rig-shot.mjs`) so it's gate-verified (25→30 audits).
- **Owed/backlog:** the rest of ⑥ — a **well/cistern** + a **debris-trail** (next cycle). Sev-3 watchtower nits (roof fascia depth · ramp curbs · a foot-scuff drift · rail height). The walkable-ramp FEEL → walk-test.

## Campaign C41 — 2026-06-20 — M7: **procedural-wreck-overhaul** — net-new structure axes (splayed engines, sensor masts, asymmetry) make wrecks read wider/weirder, not tubes → **M7 BEGINS** ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — new components declare colliders + are phash-deterministic; the salvage-panel stream stays stable). **Visual gate PASS in 3 ROUNDS** (a recon agent + 5 adversarial critics): r1 FAIL (thin pin-mast · a bare-tube form · an asymmetric "blob+stick" · a "loose-barrel" trimaran · the splay under-shown) → r2 (thicker housing'd mast · the bare-tube form replaced with a layered tower+mast · pod NOSE cones · bigger asymmetric outrigger · splay rate ↑) → r3 (elongated hull proportions + a **length-frame rig pin** — the "blob" was an END-ON camera artifact, not a bad asset) → both critics PASS, 4 distinct silhouettes, no broken geometry. **No save bump** (procgen POI placement is archetype-agnostic + additive, D227). Cycle 41/75. **M7 unit 5/7 — first M7 unit.**

**procedural-wreck-overhaul.** Long-standing complaint: "every wreck is a long tube." The socket-grammar `derelict` archetype (ACBA, builds non-linear hulls) existed but was under-varied (3 forms) + under-weighted vs the legacy linear `ship`. Overhauled additively:
- **2 NEW structure-axis components** (`world/poiComponents.ts`): `splayedEngineCluster` (a hub + 3-4 bells FANNED around the axis — a wider/weirder stern than the single nozzle) + `dorsalMast` (a tall sensor spike with a base housing + cross-arms + a dish — a distinctive VERTICAL silhouette). Both phash-deterministic, with declared colliders (decoration detail exempt via `isWreckDecoration`).
- **`assembleDerelict` expanded 3 forms → 5** (`world/poiArchetypes.ts`): wide-body trimaran (now with pod nose cones so they read as hull sections, not barrels) · stacked tower · NEW mast · NEW asymmetric single-outrigger+mast · NEW layered tower+mast. EVERY form now carries a secondary feature — the bare-linear branch is GONE (no tube regression). The stern is a single bell or the splayed cluster (≈60%).
- **`hullBarrel` elongated** (len 2.4-4.4→3.2-5.0, r 0.9-1.4→0.82-1.18) so a short-fat roll can't read as a blob; `list` 0.20→0.26 + `bury` 0.55→0.45 (more hull shows, clearer crash).
- **Re-weighted `ARCH_WEIGHTS`** (~0.08 from the legacy linear `ship` → `derelict` in every biome) so the wider/weirder hulls appear roughly as often as the tubes.
- **Rig harness:** a NEW `--pinyaw` length-frame option (`spawnProcgenWreckRig`) — the socket-ship spine is +Z-long but the framer assumes X-long, so a random world-yaw caught some derelicts END-ON (read as a blob, graded the camera not the asset — the harness footgun). The pin frames the spine broadside for a fair gate. D249.
- **Owed/backlog:** sev-3 polish nits (mast-tip housing · the splayed cluster reads slightly drum-like · deepen the trimaran seam · nose-tip daylight · the half-buried debris spheres read a touch loose). The in-world FEEL (walking among the new hulls) → walk-test.

## Campaign C40 — 2026-06-20 — M6: **remove-hud-stat-bars** — diegetic survival (per-stat vignettes + heartbeat/growl audio) replaces the HUD bars → **M6 COMPLETE** ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — UI/screen-FX/audio, no scatter/geometry/collider). **diegetic-probe gate PASS** (NEW: forces each stat dangerous → asserts its vignette lights while the others stay dark + the bars hide/show on toggle). **Visual gate PASS in 3 rounds** (2 critics; r1 too faint → r2 boosted + bolder gradients [+ a render-harness transition-disable fix] → r3 separated the heat/health hues → PASS, all four tells distinct + readable over the bright desert). **No save bump** (a localStorage setting, not the save schema). `FEATURES.diegeticSurvival` **flipped ON** (bars hidden by default; reversible; user vetoes FEEL at review). Cycle 40/75. **M6 unit 4/4 → M6 COMPLETE.**

**remove-hud-stat-bars (the M6 finale).** The always-on HUD stat bars are replaced by DIEGETIC tells so the player FEELS their survival state. Behind `FEATURES.diegeticSurvival` (master flag) + `settings.diegeticSurvival` (a pause-menu opt-in, default-ON; the bars stay the always-available floor). Effective `diegeticActive = flag && opt-in`.
- **Infrastructure:** NEW `FEATURES.diegeticSurvival` (`config/features.ts`) + `settings.diegeticSurvival` (`core/settings.ts`, persisted/loaded) + a tiny shared `ui/diegeticMode.ts` (`isDiegeticActive`/`setDiegeticActive`, set by `menus.applySettings`, read by hud + statVignette — avoids a circular menus import). `hud.setStatsBarsVisible()` hides the `#stats` column; a pause-menu toggle row (gated on the flag) flips + persists the opt-in.
- **Diegetic visual tells (`ui/statVignette.ts` extended):** the existing cold (blue) + thirst (sepia) warnings now BOOST `×DIEGETIC_VIGNETTE_BOOST` (1.7) in diegetic mode so they read as the primary readout; NEW **heat** (amber-orange sun-glare), **hunger** (a dark sickly-green starvation pall), and **low-health** (a deep blood-crimson that PULSES at `HEALTH_PULSE_HZ`). Every lethal stat now has a screen-edge tell. Each ramps with severity; CSS radial-gradient overlays (no new shader programs). The heat/health hues were deliberately separated (orange vs crimson) so the two danger states never read the same.
- **Diegetic audio (`audio/audio.ts`):** NEW `playHeartbeat(intensity)` (a low "lub-dub" that quickens as health drops) + `playStomachGrowl()` (a low gurgle, periodic while starving), driven on a cadence from `statVignette`.
- **Verification:** NEW `__game.diegeticProbe()` (per-stat vignette + bar-toggle assertion) + `__game.showDiegeticVignette(stat)` + a `diegetic-probe` (numeric) + `diegetic-vignette --stat=` (render) rig scenarios. Footgun fixed: the CSS opacity transition lagged a single-frame render → the render hook disables the transition so the screenshot shows the true opacity.
- **Owed/backlog:** the FELT read — "can I actually survive without bars?" — is walk-test-only (the user vetoes at the Phase-B review). Sev-3 nits: hunger's green is sky-band-only; heat saturates the whole plate; cold is faintest over the bright horizon. Cold-shiver / hot-pant audio cues were scoped out (vignettes cover those) → backlog.
...
**survival-rebalance-newgame.** Survival was never real — the game ran with `GOD_MODE: true` (the player never died). Flipped it OFF for the real new-game + tuned a **forgiving Long-Dark** curve.
- **Flags (tuning.ts):** `GOD_MODE` true→**false**, `DEBUG_UNLIMITED_STAMINA` true→**false** (sprint now drains stamina in real play). Both reversible; the FLIP-AUTHORITY charter authorizes the loop to flip testing flags once gates pass.
- **Dev-mode safety (survival.ts/D246):** the godmode floor in `die()` is now gated on `Tuning.GOD_MODE || ctx.flags.devMode` — a REAL new-game (devMode off) is lethal, but DEV-mode boots (rig-shots, feature walk-tests) still floor the stats so a long testing session isn't interrupted by starvation.
- **The forgiving curve (per real-second drains, computed targets):** a PREPARED player (drinks/eats/shelters) survives INDEFINITELY; a NEGLECTED player dies in ~8–10 min on whichever urgent need they ignore (heat/cold/thirst), ~15 min on hunger (the slow background pressure). Slowed the urgent paths a lot: `THIRST_DRAIN_PER_SEC` 1/300→1/480, `HEAT_GAIN_PER_SEC` 1/90→1/420, `COLD_NIGHT_DRAIN` 1/120→1/420, and softened every damage rate (`DEHYDRATION` 1/30→1/120, `HEATSTROKE` 1/25→1/150, `COLD_DAMAGE` 1/35→1/150, `STARVATION` 1/40→1/180) so a depleted stat is a recoverable spiral, not a cliff. Shade/shelter cool/warm an order of magnitude faster than the drains, so retreating to cover is always a way out.
- **Health REGEN (the forgiving keystone, survival.ts):** when thirst+hunger are comfortably up AND temperature is near neutral, health slowly regenerates (`HEALTH_REGEN_PER_SEC` 1/150 → ~2.5 min to full), so a bad patch heals off once you re-secure water/food/shelter (Long Dark "condition recovers when needs are met"). The thresholds (`*_THIRST_MIN`/`*_HUNGER_MIN` 0.4, `*_TEMP_MAX` 0.5) sit strictly inside the no-damage zone, so regen + damage never overlap.
- **Verification (debug hooks + scenario):** NEW `__game.survivalProbe(env)` (deterministic time-to-death under heat/cold/thirst/hunger/prepared, bypassing the dev floor) + `__game.triggerDeath()` (forces the real death path + reports the overlay un-hid) + a NEW `survival-probe` rig scenario asserting the time-to-death bands + prepared-indefinite + the death-overlay wiring. Confirms the death→Continue path still works now that GOD_MODE is off.
- **Owed/walk-test:** the FELT survival pacing (does ~8-12 min feel right? is the spiral tense-but-fair?) + the re-enabled stamina cadence (6s sprint / 9s recover — never play-tested) → Phase-B walk-test (the user vetoes FEEL at the review). The stamina values are unchanged (just enabled) → tunable.

## Campaign C37 — 2026-06-20 — M6: **crafting-chooser-colliding-recipe** — a new `signal_kit` flare lights up the dormant multi-match chooser → **PHASE B BEGINS** ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — a new inventory item + recipe + a transient FX module; no scatter-rand/geometry/collider). Visual: **adversarial gate PASS in 2 rounds** (2 critics; r1 FAIL on a beaded/straight/no-gradient trail → r2 fixed → both PASS, no sev≥2). Chooser fires for real: `craft-chooser` scenario `real=true` (`branch×3+scrap×1 → ["fire kit","signal flare"]`, CRAFT gated). **No save bump** (additive item + additive recipe id, per the C29 spyglass precedent). Cycle 37/75. **M6 unit 1/4 — first Phase-B unit.**

**crafting-chooser-colliding-recipe.** The multi-match crafting **chooser** UI (built ACAS B3, dormant — the live recipe set had no input-multiset collisions) now fires in real play via ONE data-level recipe collision: a NEW **`signal_kit` (SIGNAL FLARE)** shares `fire_kit`'s exact recipe (`branch×3 + scrap×1`), so combining those materials surfaces a "fire kit OR signal flare" choice — "warm yourself **OR** call out." Design deviation (D245): the Phase-B proposal suggested `scrap×2+branch×1`, but that key is already owned by `scrap_bar` (id 15) and `fire_kit` is actually `branch×3+scrap×1` — reusing fire_kit's existing recipe yields the intended clean 2-way chooser with zero churn (the literal spec would have made a 3-way pileup + changed fire_kit's long-standing recipe).
- **signal_kit = a one-shot signal flare** (NOT a fire — a genuine either/or so neither choice dominates). LMB fires a transient bright flare that arcs skyward, peaks, and burns out trailing cooling embers; consumed; **fully transient** (`world/signalFlare.ts` — never touches `ctx.fires` or the save schema). Self-luminous **additive sprites only**, NO dynamic PointLight (would change the scene light count → shader-recompile hitch, per the fire.ts AAY-fix).
- **Files:** `inventory/types.ts` (ItemId `signal_kit`), `inventory/items.ts` (def + flare-tube viewmodel + burst icon + `ALL_ITEM_IDS`), `inventory/recipeDiscovery.ts` (recipe id 18 — the collision), `world/signalFlare.ts` (NEW transient arc module), `audio/audio.ts` (`playSignalFlare` — launch hiss + rising whistle), `config/tuning.ts` (`SIGNAL_FLARE_*`), `player/ghostPreview.ts` (exempt the flare from the ground footprint ring — it's fired, not placed), `main.ts` (tick + dev loadout), `debug/debugPanel.ts` (`__game.fireSignalFlare`), `scripts/rig-shot.mjs` (NEW `signal-flare` scenario + extended `craft-chooser` to assert the REAL collision).
- **Owed/backlog:** flare gameplay consequence ("signal attracts a raider/rescuer") + flare illumination + the sev-3 critique nits (head core size, tail ash-fade, day-arc framing) → backlog §A. **Flare in-motion FEEL** (launch cadence, arc) → Phase-B walk-test.
...
**wind-mood-soundscape.** **ASSESS-FIRST found the real gap:** the Session-X soundscape (`soundscape.ts`) loads wind/ambient/music from `/public/audio/`, which is **EMPTY** — the CC0 sample pack never landed, so the whole ambient bed has been **SILENT** (only the procedural SFX in `audio.ts` played). So per Dustfall's no-sample-files ethos, I **synthesized the wind procedurally**: a looping pink-ish noise buffer (`makeWindNoiseBuffer`) fanned into a low-pass **BODY** (the wind's tone) + a band-pass **WHISTLE** (a lonely moan) → the ambient bus (so the existing `setStormMuffle` master low-pass still applies). It **carries MOOD** — the body cutoff shifts with sun height (`WIND_CUTOFF_DAY 2600` bright airy hiss at noon → `WIND_CUTOFF_NIGHT 620` dull lonely moan at deep night, `×0.5+0.5` ramp), a storm opens it toward `WIND_CUTOFF_STORM 5200` (full roar), the level tracks the existing `windLvl` (storm/drift) with a gust shimmer, and the whistle is subtler by day / fuller at night. `setTargetAtTime` per frame = zipper-free. The game finally **breathes** (was silence between SFX). NEW `__game.audioState().pwind` debug fields. Determinism N/A (audio) + transient (no save). **FEEL** (levels, cutoffs, the whistle character) → the attended walk-test (audio can't be heard headlessly). **Follow-up (backlog):** the day/night LIFE beds + music are still silent (no samples) — procedural versions are a future unit. Next M5b: rare-sky-phenomena.

## Campaign C32 — 2026-06-19 — M5b START: **wordless-prop-scenes** — death tableaux that tell a story with no text ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — scenes placed from a DEDICATED seeded RNG → the scatter stream is untouched; decoration-only, no colliders). Visual: adversarial gate (2 critics) — **iterated 3 gate-driven rounds**; concept + somber tone gate-confirmed both rounds, top blockers (skull, pairing, cairn) fixed in r3. **No save bump.** Cycle 32/50. **M5b unit 1/5.**

**wordless-prop-scenes.** Environmental storytelling with NO text: `world/wordlessScenes.ts` scatters 4 hand-composed prop tableaux across the mid-field that imply a story you read by looking — caps the M5a arc (crest → glass a wreck → reach it → and find who died here). Composed from the existing slumped-seated `makeSkeleton` + simple set-dressing. **3 archetypes** (cycled for variety): **"the last fire"** (a skeleton slumped by a cold stone fire-pit + a dropped canteen), **"the watcher"** (slumped against a graduated stone cairn, facing the horizon), **"the pair"** (two skeletons leaning toward each other across a dead fire — sheltered together to the end). Determinism-safe (a dedicated `makeRng(worldSeed^k)` stream, no scatter-rand draw) + decoration-only (no colliders); **clears scatter rocks within `WORDLESS_SCENE_CLEAR_M` of each tableau** so it gets a clean stage. **Iteration (Rule 8, 3 rounds):** r1 = blobby (oversized rubble-fire, beach-ball/floating skull, dark-merged masses, tiny canteen); r2 = shrank/lit the props, fixed the floating skull (lowered + ellipsoidal in the shared `makeSkeleton`), bigger canteen, archetype variety, rock-clear; r3 = **skull now reads** (recessed eye-socket discs + brow + nasal void — the load-bearing fix both gate rounds flagged), the **pair reads** (widened to ~1.7m + inward lean + central fire), a **deliberate graduated cairn**. Reads as deliberate human tragedies in the encounter (up-close) view. **Residual polish** (backlog): prop legibility at far distance, a backlit skull reading dark at oblique 3q, occasional ambient rocks at the stage edge. NEW `--scenario=wordless [--idx=] [--angle=]` rig-shot. Next M5b: wind-mood-soundscape.

## Campaign C31 — 2026-06-19 — M5a: **sun-shade-exposure** — wreck shadows + dune shade relieve heat → **M5a COMPLETE** ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — a per-frame heat-modulation reading existing geometry; no scatter-rand, no new collider). Logic/survival unit → **code-auditor review: SHIP CLEAN** (slab math, raymarch, survival integration, determinism, null-safety, smoothing, perf all sound). A NEW `--scenario=sun-probe` functional probe confirmed the occlusion (a point on a 20m wreck's shadow side reads shaded; the sunny side clear). **No save bump** (transient `sunExposure01`). Cycle 31/50. **M5a unit 4/4 → M5a COMPLETE.**

**sun-shade-exposure.** Direct sun vs SHADE now matters for survival. The heat model already heated by `sunHeight` + cooled `inShelter`, but standing in real SHADE did nothing. NEW `world/sunExposure.ts` (ticked after `updatePlayer`, before `updateStats`, in the pause gate) computes `ctx.player.sunExposure01` (1 = full sun, 0 = shaded) two ways: **(1)** a throttled heightfield **raymarch toward the sun** — a dune crest above the ray = dune shade (long shadows at low sun); **(2)** a **ray-vs-AABB slab test** against the big WRECK bounding boxes (reusing the C28 horizon-silhouette boxes — a tall wreck casts a real ground shadow even at midday) + the deployable shelter zones. `survival.ts` then scales heat GAIN by `sunExposure01` (floored `SHADE_HEAT_FLOOR 0.35`) + gently cools a hot player in deep shade — so ducking into a wreck's shadow to cool off is now a real tactic (pairs with the spyglass/silhouette exploration: spot a wreck → reach it → shelter in its shade). Smoothed (no temperature pop stepping in/out). **Assessment-first** (the brief flagged it): the heat/shelter path already existed; this EXTENDS it (the shade detection + the heat hook) rather than rebuilding. Determinism-safe (heightAt + sunDir + fixed AABBs, no rand) + transient. NEW `__game.sunInfo()` debug hook (exposure + occluder boxes) for the walk-test. **FEEL** (the heat floor, cool rate, raymarch thresholds) + the **coverage gap** (only wrecks ≥8m cast shade — short wrecks don't; backlog) → walk-test/follow-up. **→ M5b is the last tier before the Phase-A milestone PAUSE.**

## Campaign C30 — 2026-06-19 — M5a: **vista-crest-reveal** — cresting a ridge lifts the fog + swells (net-new atmosphere) ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — a transient per-frame fog modulation + audio; no scatter-rand, no geometry/collider). Logic/audio/atmosphere unit → **code-auditor review: CLEAN** (no sev1/sev2 across 7 checks: fog non-compounding, envelope terminates, deterministic, null-safe, audio leak-free, pause-gated). **No save bump** (transient runtime state). Cycle 30/50. **M5a unit 3/4.**

**vista-crest-reveal.** The pull of exploration: **cresting a ridge is now rewarded.** `world/vistaReveal.ts` (ticked after `updatePlayer` + `updateWeather`, inside the pause gate): throttled terrain sampling computes the player's **prominence** (height minus an 8-dir × 2-radius ring average); on the **rising edge** of prominence crossing `VISTA_PROMINENCE_M` (7m), off a `VISTA_COOLDOWN_S` (50s) cooldown, and not in a storm → **fire a reveal**: the **fog briefly LIFTS** (`fog.density ×0.5` at peak, eased attack→sustain→release over ~7.9s — re-multiplied each frame AFTER `updateWeather` sets the base, so it never compounds) so the vista opens up + the distant C28 landmark silhouettes sharpen, under a soft warm **audio swell** (`playVistaReveal` — an open-fifth/octave pad, slow attack + long release, low-passed, on the ambient bus). This closes the **M5a exploration loop**: crest a ridge → the air clears + a landmark surfaces on the skyline → glass it (C29 spyglass) → steer toward it. Determinism-safe (terrain.heightAt + fixed ring offsets, no rand) + transient (no save). A `--scenario=vista --fogmult=` render confirmed the lift reads as a gentle clearing (the megaShip + horizon crisp up at ×0.5). **FEEL** (the prominence threshold, cooldown, envelope timing, fog amount, swell character) → the attended walk-test. Next M5a: sun-shade-exposure (the last M5a unit, then M5b → the Phase-A milestone pause).

## Campaign C29 — 2026-06-19 — M5a: **salvaged-spyglass** — hold-RMB horizon zoom + a scope vignette (net-new item) ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — the spyglass is an inventory item + a render-time FOV mechanic + a recipe; no new scatter-rand draw, no world collider → determinism/colliders untouched). Visual: adversarial gate — **SCOPE-VIEW PASS** (1st pass); **MODEL** iterated **3 rounds** (gate-driven). **No save bump** (additive item + a `discoveredRecipes` id — both additive). Cycle 29/50. **M5a unit 2/4.**

**salvaged-spyglass.** A net-new `spyglass` item: **hold RMB to zoom + scan the horizon** (read the C28 distant-landmark silhouettes, spot wrecks/water to navigate toward — the two M5a units pair: the silhouette says "something's there," the spyglass says "it's a wreck"). **The zoom** (`player/spyglass.ts`, ticked after the camera is final): hold-RMB while the spyglass is equipped eases the camera FOV `78°→24°` (`SPYGLASS_FOV`, ~3.25×) and fades in a **scope vignette** (a dark telescope tunnel with a clear magnified centre); restores to the player's settings FOV on release/unequip. Driven off `mouseHeld(2)` (not `wieldLmb`, so LMB stays inert). **The scope-view PASSED the gate 1st time** — "reads unmistakably as looking THROUGH a spyglass; large obvious magnification; the wreck goes from a tiny horizon speck to a readable multi-towered structure." A NEW `--scenario=spyglass-view [--raw]` rig-shot renders the zoom/vignette + the un-zoomed compare. **The held-item MODEL** (a brass draw-telescope) took **3 gate-driven rounds**: r1 read as a "stubby thermos" + flat-cap lens → r2 lengthened + structured → r3 flipped to the classic spyglass taper (narrow eyepiece widening to a flared objective bell, ~6.5:1), visible nested step-downs, bold steel collars, a leather grip, and **solid dark recessed lenses behind brass bezels** (r1/r2's transparent glass showed brass through → read as a painted cap; also fixed a recess-direction bug where the objective lens poked OUT). It now reads clearly as a brass spyglass in **profile/3q** — the in-play views. Residual gate nitpicks (a perfectly-end-on objective funnel + matte brass under the FLAT studio light — the latter excused as a lighting artifact, since the FP viewmodel is sun-lit in play) are rare-view/lighting, assessed low-impact. **Obtainable:** a craft recipe (`scrap×3 + cloth×1`, discovered by experimentation) + a dev-start for the walk-test. **Feel** (zoom speed/curve, vignette timing, the missing aim-down-sight pose) → walk-test. Next M5a: vista-crest-reveal.

## Campaign C28 — 2026-06-19 — M5a START: **horizon-landmark-silhouettes** `[partial]` — the system + concept ship; the look needs the impostor follow-up ✓ verify pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — silhouette billboards are decorations at landmark positions with NO new rand draw → determinism-safe + not a placement/collider POI). Visual: adversarial gate (2 critics) — **FAIL → iterated 2 rounds**, shipping `[partial]`. **No save bump.** Cycle 28/50. **M5a unit 1/4 `[partial]`.**

**horizon-landmark-silhouettes `[partial]`.** **The gap (confirmed by a NEW `vista` rig-shot scenario):** the world's FogExp2 fades distant geometry toward the SKY color, so a hero landmark at ~600m is **invisible** — it blends into the sky + can't be a navigation cue. **Built the system:** `horizonSilhouettes.ts` — a fog-RESISTANT (`fog:false`) dark silhouette billboard per major landmark (the hand-modeled flagships via `addHorizonSilhouettesByName` + the taller hero-landmark wrecks inline), **distance-gated** so it fades IN (`HORIZON_SILHOUETTE_FADE_START/FULL`) exactly as the real model fogs OUT, Y-billboarded to face the camera, ticked in `main.ts`. Determinism-safe (no rand) + decoration-only (no collider). The vista render confirms a dark landmark mass now stands on the skyline at 600m (was invisible). **The LOOK is a `[partial]` first pass:** the adversarial gate (pixel-level) FAILed it — the billboard reads as a soft blob/rectangle (no structure), is faintest at FAR range (the r1 texture faded out at the top — the part against the sky), and floats above the dune. **Iterated 2 rounds:** r2 swapped the soft quad for a STRUCTURED hand-drawn wreck silhouette (hull + tower + mast) + darker/bolder (`0x262320` @ 0.85) → now reads as a recognizable dark structure on the horizon. **Still queued (backlog §A):** a per-model IMPOSTOR (the EXACT landmark outline via render-to-texture, not a generic profile), a cleaner ground-tuck (extend-below + depthTest occlude), + boldness tuning at far range. Shipping `[partial]` — the hard infrastructure (the fog-resistant distance-gated system + the vista gate scenario) + the functional nav cue (landmarks now visible from afar) are in; the visual polish is a focused follow-up. Next M5a: salvaged-spyglass (the horizon-silhouette impostor polish is a backlog item).
...
**3p-camera-and-render-polish — the 3P render/camera system is THOROUGHLY built** (backlog §A's "3P pass" grab-bag). Assessed every named sub-item; **all already built:** (1) **cameraSnapNextFrame** — the flag + the chase-cam lerp exist (`controller.ts:511`), and it's SET on save-load (`save.ts:687`) + speeder mount + dismount (`speeder.ts:991/1013`) — the camera doesn't lerp across teleports. (2) **held-items-in-3P** — a 2nd `makeViewModel` instance (`tpMesh`) is attached to the rig's `rightHandAttach`, visible in 3P, with per-item `handAttachTransform` (placement) + `thirdPersonScale` (small-item visibility boost, ABY P3); `rig3p` confirms the machete renders in the hand. (3) **footstep ↔ gait sync** — already FIXED (ACE Tier 4A): the footstep cadence is phase-locked to `rig.stepCount` so audio no longer fires mid-air. **The speeder ride-pose foot-lift (queued C23) is a STRUCTURAL limit** — the seated-pose code's own comment: "the 3-DOF leg can't hit the pads exactly" (the sweep already optimized it; the feet read tucked/astride), NOT a tuning gap. **Remaining (modest, deferred):** 3P camera-ORBIT-around-player mode (a design/feel control-scheme feature — mouse-look turns the player today; an orbit MODE is a design call → walk-test), a speculative `thirdPersonColorBoost` for dark items (the SCALE side is done; color is a "?"), the foot-IK idle→walk snap (cosmetic, no clean fix — continuous per-frame IK). **No blind build** into design/feel/structural items (the C20/C22/C23 discipline). **Meta-note for the milestone review:** M5's polish units keep landing on heavily-built mature systems (riding + 3P) — the remaining work batches well into the Phase-A walk-test (handling/camera/tie feel) + the user may want to re-prioritize toward Phase B. Next M5: lie-down-to-sleep.

## Campaign C24 — 2026-06-19 — M5: **rope-attach-speeder-rear-bar** — explicit tie/untie a sled to the tow-bar ✓ gates pass (ultracode, code-auditor)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — an interaction + a userData tag, not a placement/collider POI or a procgen draw). **Logic feature → code-auditor gate** (PASS, 0 sev≤2). **No save bump** (reuses the existing `'speeder'` tether kind). Cycle 24/50. **M5 unit 2/5.**

**rope-attach-speeder-rear-bar.** The rope→speeder TOWING was already built (the `'speeder'` `RopeEndpoint` kind + the `speederTowBar` mesh + a visible catmull-rom rope tube + the mount AUTO-TRANSFER `transferTetherOnMount/Dismount`). **The genuine gap (the roadmap unit + backlog §B):** you could only attach by MOUNTING (the implicit auto-transfer) — there was no EXPLICIT attach to the rear bar. **Built:** the tow-bar is now a rope-attach interactable — tag the `speederTowBar` `interactType='attach_rope'`/`interactRegistry='speeder'`, push it to the raycast targets (parked or mounted), and sub-dispatch in `interaction.ts` `case 'speeder'` (tow-bar → tie/untie; seat → the existing `mount` prompt). LMB ties a player-tethered sled to the speeder, or unties a speeder-tethered one back into hand — **works while PARKED (no mounting needed)**, the explicit alternative the unit asked for. **Mirrors the `stakes`/`companion` rope-attach exactly** (ACU #50: the rope deploys while tied → no wielded-rope check) — so **zero RMB conflict** (the backlog's design-gate was on the *RMB-on-rope-raycast-pick* UX, a SEPARATE item; this uses the established LMB-on-target metaphor). ADDITIVE — the mount auto-transfer still works. **Gate (code-auditor, ultracode):** PASS — "correct, consistent with the stake pattern, sub-dispatch sound, coexistence with the auto-transfer non-breaking." Lone nit: LMB on the tow-bar also fires a wielded-weapon swing — **pre-existing, shared by ALL `attach_rope` paths** (stake/companion too), not new here → backlog §A. **FEEL** (does tying to the bar read well in play) → feel-pending walk-test. Next M5: 3p-camera-and-render-polish.
...
**smoke-signal-plume — NET-NEW** (the title pyre had smoke + `particleTrail.ts` anticipated "future smoke", but the in-game fire had none). **Every ALIVE fire now emits a tall rising smoke column**, so a lit fire reads as a SIGNAL/landmark visible across the dunes (find your way back to camp; spot a camp from afar) — minimal coherent design: reuses `fire.ts` (no new item), billboarded soft grey puffs pooled + parented to the fire group, spawned at the flame top, rising + billowing (growing) + fading + drifting with the prevailing dune wind, **torn flat in a storm**. Transient → **no save bump**. New `Tuning.FIRE_SMOKE_*` (Rule 2), `__game.spawnFire` / `__game.warmSmoke` debug hooks, + a `smoke-plume` rig-shot scenario (`--storm` variant). **Footguns fixed:** the decorative sprites were hit by the interaction raycaster (`Raycaster.camera` warning + a null-matrixWorld crash) → `sprite.raycast = () => {}`; headless rAF throttling starved the real-time column → the deterministic `warmFireSmoke` fast-forward; a backlit shot (camera at the sun) → noon + a side-spawn for a clean frame. **Gate arc (6 rounds — net-new visual, Rule 8):** the plume READS as a signal every round; the iteration was the storm wind-response + boldness: faint→bold (opacity/density), then storm cohesion (R2 big-puffs blobbed → R3 smaller/denser/cooler-grey + `fog:false` + f² root-anchor → too vertical → R4-5 stronger shear + flattened vertical reach so it tears toward horizontal) → **R6 both critics PASS** (`reads_as_smoke_signal=true,true`; "clear bold near-vertical beacon; storm tears hard downwind, cool grey separates from both the warm sky + brown murk, no sprite edges"). **MOTION feel** (rise cadence/billow in play) → feel-pending Phase-A walk-test. Next M4 (last): amban-rifle-balance.

## Campaign C20 — 2026-06-19 — M4: **atmosphere-feeltunes** — root-caused the storm-dust "snow" read + a NEW storm-render enabler ✓ gates pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — atmosphere particles + a new rig-shot scenario, not a placement/collider POI or a procgen draw). Visual: adversarial gate on the peak-storm LOOK, **3 dust rounds**. **No save bump.** Cycle 20/50. **M4 unit 2/4.**

**atmosphere-feeltunes — the weather system was ALREADY exhaustively built + tuned** (a Dune-style sweeping storm WALL with distance-derived intensity, 3 volumetric dust layers with staggered ramps, FogExp2 curve, cloud coupling, sun/ambient dimming, warm vignette, anisotropic wind, shelter dampening `perceivedIntensity`, wind-push, **in-storm camera sway** `applyStormCameraSway` — all Rule-2 tunable). **Verified, not rebuilt** (the recurring pattern). The in-storm SWAY feel is multi-session-tuned → **not blind-tuned** (walk-test territory). **The headless-verifiable gap: the peak-storm LOOK had no render scenario + the dust read as SNOW.** Shipped: **(1) a NEW `storm` rig-shot scenario** (forces a peak storm centered on the player + frames the dust-storm — the reusable atmosphere-iteration enabler, C12 pattern). **(2) Root-caused + fixed the dust "snow-flurry" read:** the storm dust is unlit + `toneMapped:false`, so it ignored the storm's sun/ambient DIMMING and rendered at full raw brightness → bright high-contrast white specks popped against the darkened sky (snow, not sand). Fix: **couple the dust albedo to the scene-dim** — `mat.color = baseColor · (1 − STORM_SUN_DIM·intensity)` so the dust dims in lockstep with the storm (sits in the murk at peak, stays bright in a light storm); + darkened/warmed the 3 layer base colors, softened the sprite (feathered, lower peak alpha), thinned the far layer (0.40→0.30) so it dissolves into the fog. **Gate arc (3 rounds):** R1 1 critic flagged the white-speck pop (sev2) → R2 darker colors (helped, still popped) → R3 the intensity-dim + softer sprite → the holistic TONE critic now PASSES (`dust_reads_as_sand=true`, "dim warm motes, blowing sand not snow"); `reads_as_sandstorm=true` **unanimous every round** (the storm TONE — dark warm murk + fog + dimming + vignette — was always excellent). Residual: a few specks over the very darkest sky band (frozen-frame; the dust streaks at 6-7 m/s in play → motion-masked) → walk-test. **MOTION feel** (sway/wind/dust-in-motion) → feel-pending Phase-A walk-test. Next M4: smoke-signal-plume.

## Campaign C19 — 2026-06-19 — M4 START: **vulture-motion-feel** — fixed the dangling-leg soar (infra pre-existing) ✓ gates pass (ultracode)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — the vulture is a creature + the change is a render-time POSE, not a placement/collider POI or a procgen draw). Visual: lean adversarial gate (2 critics) on the soaring silhouette. **No save bump.** Cycle 19/50. **M4 unit 1/4.**

**vulture-motion-feel — the motion system was ALREADY deeply built** (ACAH model + ACAI flight): a full FSM (perched/flying/landing/dead + the carcass ecology circling/swooping/carrying/feeding/returning), per-state rig animation, **banking into turns** (`applyFlightOrientation` smooths a heading-delta into roll, `VULTURE_BANK_ANGLE`), **flap↔glide envelopes** (`VULTURE_FLAP_HZ`/`VULTURE_GLIDE_CYCLE_HZ` — flap bursts gated by a glide cycle), dihedral, landing flare, terrain clearance, gaggle separation. Like multi-worm/chroma/dish — **verified, not rebuilt.** **The genuine gap (found by rendering the flight poses, which had only ever been gated PERCHED):** in the soaring states the **legs DANGLED straight down + splayed** — the leg-tuck used MIRRORED signs (`legL=-LEG_TUCK, legR=+LEG_TUCK`) so the hips swung the feet in *opposite* front-back directions (a splay, not a tuck) and the 1.2-rad magnitude only reached horizontal. A soaring raptor draws both legs back+up under the tail. **Fixed:** both hips now rot.z `-VULTURE_LEG_TUCK` (same sign → tuck together) + bumped 1.2→1.85 (back **and** up); `carrying` legs-down (clutching prey) + `landing` legs-reaching untouched. **Gate:** 2 critics both verdict-PASS, `legs_tucked=true,true` + `reads_as_soaring_bird=true,true`, no geometry defect introduced (frozen-flap-phase explicitly disregarded). Residual sev1 (the tucked-leg mass is subtle at close-up side-profile — irrelevant at in-game distance where the bird is a distant dark silhouette) → backlog/nit. **MOTION feel** (the actual animation over time — flap cadence, banking in turns, the wheel over a carcass) → feel-pending Phase-A walk-test (a frozen frame can't judge it). Next M4: atmosphere-feeltunes.

## Campaign C18 — 2026-06-19 — M3: **sarlacc-lure-ambush** (the user-requested worm LURE) → **M3 COMPLETE** ✓ gates pass (ultracode, 4 model rounds)

`verified` — `npm run verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25 — the lure is a held item + a pickup, not a placement/collider POI). Visual: adversarial Workflow gate on the lure model, **4 rounds**. **No save bump** (reuses `ctx.pickups.list` + the worm feeding loop — D81 additive, the placed lure persists as a normal pickup). Cycle 18/50. **LAST M3 unit → M3 COMPLETE.**

**sarlacc-lure-ambush — the deliberate worm LURE (user add).** A NEW craftable consumable `worm_lure` the player **plants ~4m ahead** (`onUse` → `spawnDroppedPickup` at the camera-forward XZ; static, recoverable until consumed). The worm homes on it via the EXISTING `findNearbyMeat` (the lure joins the `meatItems` set) but at a **much longer scent range** — new `Tuning.SANDWORM_LURE_DETECT_RADIUS_M = 70` vs meat's 30 — so it's a real summoning bait, not incidental food; the worm surfaces to feed (the 2×-damage strike window) and consumes it. **Design (3-agent recon):** chose the minimal coherent design — a consumable reusing pickups + feeding + the pickup save — over a ~625-LOC deployable device; **no new save schema.** Hooks: `types.ts` ItemId, `tuning.ts` radius, `sandWorm.ts findNearbyMeat` (per-item detect radius), `items.ts` def + a circular-import for `spawnDroppedPickup` (runtime-only → init-safe, confirmed by a clean perf-probe boot), `main.ts` start-inventory ×2 (testability; a craft recipe is a noted follow-up). **Gate arc (4 rounds, held item):** R1 FAIL (bright cherry-red gland reads cartoonish vs the muted palette) → R2 desaturated oxblood + lumpy/clotted + bigger bone crown (1/2) → R3 rounded gland (was a faceted peak/roof) + clean up-and-out THICK barb ring (rule 7) + collar (1/2) → R4 **removed the emissive** (the glow was the "hot orange-red" the palette critic kept flagging) → a muted dried-blood gland nestled in the pale bone crown. `reads_as_lure=true` was **unanimous every round** (geometry sound, no defects); 4 rounds is the held-item cap (Rule 8). **FEEL** (does the summon-and-ambush loop read in play; the in-hand model edge-on) → feel-pending Phase-A walk-test. **M3 COMPLETE** (C12-18: model · tail-buried · charge-dive · audio-rumble · multi-worm · lure). Next tier: **M4 — critters + atmosphere.**
...
**Tier 1 — weathering cohesion (the §F per-bucket-rebalance debt).** The 3 hull buckets were collapsing to one warm-brown (cool→rust, dark→pale-ceramic) under the shared `HULL_WEATHERING_ACAY`, and the satellite solar wings rendered near-BLACK (the lone value-outlier). Fixed at the `getBucketMats` lever (D234): NEW per-bucket `BUCKET_WEATHERING` strength overrides — cool pulls the warm-rust channels DOWN (stays cool-steel), dark pulls the PALE channels down + a cooler-steel base `0x3a3d43` (a genuine DARK-steel anchor: tank + husk were terracotta-pale), warm unchanged. Solar wings `0x44525c`→`0x8d8a84` (in-family grey, +LED bezel); `wearAmplitude` break-up so smooth low-poly flanks read less flat. Material-only → ZERO new shader programs (shared `onBeforeCompile`).

**Tier 2 — sand integration ("swallowed by a living dune").** Root-caused the "clean seam, no banking" the critique flagged: `makeSandMound` sank the drift apex ~0.094·size BELOW terrain. Added a backward-compatible `proud` param (ship/megaWreck callers byte-identical); `poiAssembler` now sizes the drift to span the footprint+1.8m and requests a PROUD crest → drifts visibly BANK against the base. **Satellite** reads crash-landed (was floating/showroom — the critique's #1 issue): list 0.15→0.30 + seatSink 0.12→0.38. **Derelict** bedded in (bury 0.25→0.55). **Debris** gets an impact scorch-DISC + deeper-seated pieces. **Tank** gets a component-baked sand-tongue through its torn mouth.

**Tier 3 — the COLLIDER-AUDIT gate (the §G headline) + §E dish.** NEW `npm run verify:colliders` + a `collider-audit` rig-shot scenario + `auditPOIColliders`/`auditArchetypeColliders` in `poiAssembler` + `__game.auditPOIColliders` dev hook — asserts every STRUCTURAL mesh (decorations/overhead/hollow-enterable shells exempt — D235) is ≥40%-covered by a declared collider via a 3×3×3 sample grid, ×5 seeds ×5 archetypes; wired into `verify:all`. Turns the ACBA "found-by-eye" author errors (un-collided dome, mis-axis strut) into a headless regression gate. Also added a grounded collider for the flagship **satelliteDish** reflector (§E — was walkthrough; feel owes the walk-test).

**Tier 4 — silhouette/detail.** **Husk hollow read fixed** (D-comment): widened the open-top gap 126°→153° → the front/end view now clearly reads as a gutted ribbed shell (was a flat curved plate from 3q). **Derelict wide-body** rebuilt as a parallel-hull TRIMARAN + cross-strut (was a perpendicular plus-sign — the §G "try parallel twin-hulls"); verified on seed 2.

**Polish bucket (§E, 4/4).** Brighter night stars (`STAR_BRIGHTNESS 1.55`, additive-alpha gain clamped); speeder antenna leans REARWARD (sign flip, forward=-Z); **F8** opens the dev item-spawner under pointer-lock (was click-only); **scrap pickup** reworked into a torn buckled rusted-metal sheet (`scrapMesh.ts` shared helper, 5 item-studio rounds, held+world synced).

**Tier 5 — SCOPE-CUT (D237, pre-authorized).** Yard ~3215-draw-call worst-case is a rare frustum-culled destination biome; field perf is fine (842). A proper cross-POI yard merge is a non-trivial architecture change for low player impact → deferred to a perf session.

**Owed (attended walk-test, headless can't judge):** the original collision/feel/seating walk-test of the declared colliders + the NEW flagship dish-collider feel + the in-world feel of the banking drifts / crash-poses.

**ACBB follow-up (same day, user-directed):** finished the §G polish — satellite wings lifted again (`_solarMat` 0x8d8a84→0x9e9b92) · debris fragment bodies → `_rustMat` so they read as TORN HULL METAL not grey rock · husk shell got proud longitudinal hull-plate SEAMS + rivet rows · derelict barrels (`hullBarrel`) got 4 hoop bands + a dorsal spine ridge + sensor box (scaled to r). Material/decoration only → perf still 842/69, all gates green. **Tier 5 yard merge ATTEMPTED + REVERTED (D239):** a panel-rim-greeble merge cut the yard ground worst-case 3215→2633 (~18%) but regressed the terrain bury-audit (~30 fails/seed) → reverted per the determinism tripwire; Tier 5 stays cut (D237). Kept the rig-shot wreck-yard draw-call probe. Also dropped raiders from the roadmap + kept both ship paths (D238).

## Session ACBA — 2026-06-17 — POI VARIETY overhaul: socket/component/grammar system + 5 archetypes (4 non-ship + a weird-ship) + pervasiveness + bug-fix pass ✓ verify pass (tsc clean)

`verified` — `npm run verify` (tsc) PASS; `npm run verify:placement` PASS (0 occlusion + **0 NEW terrain-audit** fails ×5 seeds, re-run after every change; held green through the composite-share bump + husk-in-roulette); perf-probe field **drawCalls 839 (<1000, ↓ from 960)** / triangles 367k / programs **67** (↓ from 69); wreck-yard ground worst-case ~3193 (dense-graveyard, frustum-culled in play); boot ~2.6s (heavier component assembly at 0.85 share). **No save bump** (archetype-agnostic, additive). User feedback after the ACAZ wrecks: still "all long tubes", buggy collision, panels under terrain, a disliked cockpit window + scout. Driven by an **adversarial 5-lens critique Workflow** (6 agents over the renders + source) that caught 3 real collision/grounding bugs + the sev-1 quality fixes. D226-D231.

**Phase 0 — bug-fix pass (the user's reported defects).** (1) Removed the cockpit "window/canopy" (both variants) + the `_windowMat` singleton — read as fake (user). (2) Removed the `scout` class everywhere (union/recipe/roulette re-weighted 7-way/CLASS tables/salvageKind/anchor-scale) — "lame/unrealistic" (user). (3) `bodies.ts` now SKIPS colliders for `isWreckDecoration` meshes → kills invisible interior walls from greebles + cuts collider count. (4) `addHullGreebles` now seats on each variant's REAL flank (`flankZ` threaded from `anchorSurfZ`), not the buggy `0.93r` that buried 10cm greebles INSIDE the skin (the "floating/disconnected pieces"). (5) Rule-7: SHEARED breach patch 0.06→0.15m, louver slats 0.03→0.05m. (6) **Corner-aware terrain cull** (`panelPlacement.ts`): samples the panel's bottom-edge midpoint vs sand (size-aware `SALVAGE_PANEL_TERRAIN_CORNER_MARGIN −0.25`) replacing the lenient center-only `−0.10` that passed half-buried panels; + a **new TERRAIN-AUDIT gate line** (interior-safe via a `terrainCullEligible` tag the gen-cull sets) so the gate now catches what the eye sees.

**Phase A — the socket/component/grammar foundation (D226, headline).** Replaced the linear `cockpit→hull→engine→tail` `+X`-cursor assembly (the root cause of "every wreck is a tube") with a general system: NEW `world/poiComponents.ts` (Socket = point+outward-frame+radius; `mate()` glues a child socket onto a parent socket in ANY topology — radial/vertical/scatter; declared `ColliderSpec` per component; all `phash`-derived → ZERO rand draws), NEW `world/poiArchetypes.ts` (the grammar + biome-weighted `pickArchetype`), NEW `world/poiAssembler.ts` (`placeProcgenPOI` = a generalized copy of the proven burial/align/re-skin/merge/salvage-panel/prune pipeline). `bodies.ts` gained `attachDeclaredColliders` + `ColliderSpec` (exact Rapier primitives + convex-hull at the assembled world TRS — **no more world-AABB fallback**, the D228 collision-accuracy fix). `pickArchetype` rolls `ship` (delegates to the legacy `placeProcgenComposite` — ZERO regression) or a new archetype, wired into `procgenPoi` + `wreckYard`. Shared hull mats + a reusable `reskinToBucket`/`getBucketMats` exported from `procgenWreck` so POIs collapse into the same ≤3 hull buckets at merge.

**Phase B — 4 non-ship archetypes (the payoff, adversarially iterated).** **SATELLITE** — foil-wrapped bus + mirrored solar wings (crash-banked, gridded) + dish on a mast + a human-scale hatch; the canonical "not a ship". **WRECKED_TANK** (D233 — replaced the original neat "tank farm" after the user walk-test: it read as an intact fuel depot, not wreckage; then a 4-lens critique drove a 2nd pass) — one big tank toppled on its SIDE + RIPPED OPEN with WHOLE-silhouette damage so no random yaw hides all trauma: a torn-open +X end (exposed ribs + jagged irregular flaps + peel-back plates + dark void), a CRUSHED dented −X cap, TWO recessed flank breaches, a mid buckle + a snapped hoop, a human-scale manway. DEEPLY half-swallowed by sand via a scale-invariant `buryFrac` (~57% of the height → the sand line crosses the hull axis at any size; the critique found the old fixed `bury` left it only ~35% sunk). **DEBRIS_FIELD** — torn plates/struts/a central lootable chunk strewn over a disc, post-rotation seated. **HOLLOW_HUSK** — a gutted hull shell (open-top trough + open torn ends + a flank breach + exposed rib formers, half-buried + listed); enterable-READY (open bore + side-wall colliders, no solid fill); reads as a ribbed skeleton at field distance. **DERELICT** (the "wider/weirder ships" direction, D232) — an intact ship built from socket-mated parts (nose + hull barrel + engine bell) in a `phash`-picked FORM: **linear** (a clean pointed ship), **wide-body** (outrigger hull pods on radial sockets → a catamaran-ish hauler), or **stacked** (a dorsal conning-tower) — the same parts, only the socket choice differs → non-tube ships for free. ADDITIVE: the refined legacy linear ship stays (reached via `ship`); the derelict adds the wide/stacked forms it can't make. All declare exact colliders; all biome-weighted into the field.

**Pervasiveness — the system applies to ALL wrecks, not just the graveyard (user).** The new system was wired into the general world field (`procgenPoi`) AND the graveyard from the start, but the general field only routed `PROCGEN_COMPOSITE_SHARE`=0.50 of scattered wrecks through it (the rest stayed legacy hand-props). Bumped to **0.85** so the variety system handles the bulk of scattered wrecks across salt/rocky/dune (biome-weighted: dune→tanks/satellites, rocky→tanks/husks, etc.). Field perf actually IMPROVED (drawCalls 960→**839**, programs 69→67 — the new-system POIs merge better than the legacy hand-props they replaced); boot rose to ~2.6s (heavier component assembly at 0.85 share). Once the ship→socket migration lands (#27), the whole composite path is the new system and the share can go to 1.0.

**Critique-fix round (D230) — the adversarial workflow earned its keep.** Real bugs fixed: debris STRUT collider was a vertical capsule over a horizontal bar (walk-through) → Z-aligned; debris GROUNDING seated by un-rotated bbox (tilted pieces floated/speared) → post-rotation lowest-vertex; tank DOME had no collider (tallest part) → ball cap; satellite panel mounted ON a foil decoration (→ non-lootable) → cleared +Z face. Plus sev-1 visual: wing crash-pose, dish proportion+rim, tank spread+silo+seatSink, warm-shifted accents.

**ALL FOUR user directions + "wider/weirder ships" now shipped** (non-ship industrial = wrecked_tank; satellites; debris; hollow husks; weirder ships = derelict). The wrecked_tank replaced the tank_cluster on the user's walk-test feedback (D233). **Deferred (honest, backlog §G):** RETIRING the legacy linear ship in favour of the socket derelict (optional — the derelict already delivers the weird-ship value without the regression risk of migrating the heavily-refined ACAZ ship; then the composite share → 1.0); a formal COLLIDER-AUDIT harness; yard cluster-merge (cut the ~3193 ground-view); sev-2/3 polish (tank terracotta→steel gradient, fleet palette cohesion, pipe-strut wiring, sand-drift banking, husk exterior detail, derelict wide-body refinement). **OWED:** the attended in-world walk-test (felt collision + variety + seating under sky/fog — verified headless only).

## Session ACAZ — 2026-06-17 — Procedural-wreck REALISM overhaul (materials + scale anchor + new variants + crash realism + fleet convergence) ✓ verify pass (tsc clean)

`verified` — `npm run verify` (tsc) PASS; `npm run verify:placement` PASS (0 bury-audit fails across 5 seeds, re-run after EVERY geometry/burial change); perf-probe field drawCalls 960 (<1000) / triangles 375k / programs 69 (unchanged all session); wreck-yard aerial drawCalls 1477 (worst-case all-in-frustum, frustum-culled in play). **No save bump.** A long reference-grounded, adversarially-verified overhaul: the ACAX panels went photoreal and left the WRECKS out-classed. Driven by **2 adversarial-critique Workflows** (anchor: 2 rounds × 6 agents; whole-overhaul capstone: 6 agents) + per-element screenshot iteration. NEW inspection-only `scripts/wf-anchor-critique.mjs` + `wf-overhaul-critique.mjs`; rig-shot gained `--forceanchor` (aims at a hatch) + `--variant=N` (force one hull variant).

**T1 — hull MATERIAL (`hullMaterial.ts`).** Reworked the procedural weathering shader to 12 channels (root-caused over the adversarial panels): warm dust-crust on decks + paint-chalk haze + rust streaks gravity-flowing down seams + saturated underside/belly oxide + cool scoured lower hull, all as uniforms → still ONE shader program. Fixed: fBm-ceiling dead masks, diagonal-streak shear, triple-stacked muddy undersides, cylinder-defeats-orientation (belly `lowBand`).

**T1B/T3 — per-class palette → 3 BUCKETS.** Per-class hexes were a ~19° brown wedge (no identity) AND fragmented the yard merge 3→24×. Replaced with 3 distinct LIGHTNESS-tiered material buckets (light cool-grey / mid warm-tan / dark industrial), memoised by bucket key — class identity reads via lightness (hue still partly tint-washed; full hue needs a per-bucket weathering rebalance, deferred).

**T2A — scale-anchor HATCH + LADDER (`addScaleAnchor`).** A human-scale access door (recessed flange + rust slab + spoked handwheel on the latch edge + barrel/strap hinges + straps) + climbing ladder on a solid hull's clean lee flank; ONE per wreck (2-3 on mega/bulk by length). **Band-seated** to each variant's flat-flank range so nothing floats off a chining hull; door sized to the band (clamped ≤1.7m human-constant → reads tiny on the fat mega = sells scale). **Rand-NEUTRAL** via a new deterministic `hash2()` → panel stream byte-identical, verify:placement stays 0.

**T2B — 2 new hull VARIANTS (8 total).** SHEARED_HULL (a blasted-open section: charred breach patch + exposed bent ribs + torn flaps, built PROUD — recessed voids don't read at ~1m procgen scale, D197) + CARGO_POD_ROW (freight spine of container pods, ~25% torn off). Weighted into ALL 4 `HULL_SEGMENT_BIOME_WEIGHTS` rows.

**T2C — size CLASSES (8 total).** `scout` (~4-5m sleek tapered dart) + `mega_freighter` (~33m) + a per-class **GIRTH** (T3 fix): the ladder was length-only (mega = scout girth → a "pipeline"); seeding `prevRadius = 0.9·CLASS_GIRTH[cls]` fattens heavy classes (mega hull 4.1→6.7m tall) at ZERO tri/draw cost (faceted hulls scale at fixed vertex count).

**T2D — CRASH realism.** Class-scaled half-burial (scout upright on the surface ↔ mega bedded DEEP), clamped to keep ≥50% + the anchors proud; an always-fire crash-LIST roll; engine SHEAR (drooped torn nozzles). All rand-neutral (class tables + position hash). Engine-bell flare re-skin fixed (it was a cloned material the identity test missed → exported `_bellOuterMat`).

**Deferred (honest scope-cut, sev-3):** SHEARED breach-patch depth/recessed floor (rule-7); yard size-tentpole + class-scaled spacing; one non-axial mass per heavy class; weathering chroma up close; sign-randomized engine droop; scout guaranteed trauma; the 2A exclusion-pocket / fin-antenna-to-impact-flank / seam-grid. **OWED:** an attended in-world walk-test (sky/fog + the amber salvage pry-glow) — verified headless only. D221-D225.
