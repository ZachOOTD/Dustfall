...
**Current state**: Session **ACAN** shipped (2026-06-08 — **flagship static-merge (finishes T6) + mega-wreck panel fix**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material only). 2 commits (`2d6127d`, `d907893`); D195-D196. A tight follow-up to the ACAM wreck-fleet overnight, finishing the 2 deferred high-value items. **Headlines**: (1) **Flagship static-merge (D195)**: the ACAM merge covered the procgen fleet + the mega-wreck, but D193 showed the 3 hand-modeled FLAGSHIPS were the real mesh hogs (deferred for lack of a verification rig-shot). Wired `mergeStaticByMaterial` into `placeMegaShip`/`placeSatelliteDish`/`placeCrashedHull` (merge the root before return; panels stay live; their collision is hand-built static boxes, independent of the visual meshes). **megaShip 160→67 · satelliteDish 148→47 · crashedHull 51→44 (~201 saved)** — the static-merge now covers EVERY big wreck/flagship. NEW generic `flagship` rig-shot (`--name=<group.name>`) finds-by-name + frames + reports mesh count (each flagship NAMED); all 3 render structurally identical. (2) **Mega-wreck panels (D196)**: the 2 persistent bury-audit fails were a faceYaw SIGN bug — `+π/2` faced the panel's +Z (its outward normal + recess axis + the axis the audit raycasts) INTO the hull. Fixed to `-π/2` (face out). **Bury-audit 68/68 ALL CLEAR** (was 61/63). **Attempted+reverted**: T5 real-breach holes on the procgen hulls — procgen wrecks are unnamed/random-positioned so no rig-shot frames them → couldn't verify; reverted per rule 8. **Still owed**: a procgen-wreck rig-shot framer (BLOCKS T5); then T5 greeble/breaches; T7 LOD; the ACAL mega-wreck interior WALK-TEST. **Next session (ACAO)** = build the procgen-wreck framer → T5 → T7 → walk-test.
