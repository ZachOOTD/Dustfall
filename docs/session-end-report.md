...
**Current state**: Session **ACAM** shipped (2026-06-08 — **wreck-fleet level-up: WebGL static-merge perf + faceted procgen hulls + half-burial**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material/collider only). 2 commits (`6eb925f`, `84cda18`); D192-D194. Finished the deferred ACAJ T3/T4/T6 from a planned overnight; shipped 3 core tiers fully-verified (T5/T7 + the flagship-merge extension are noted stretch/follow-ups). **Headlines**: (1) **T6 WebGL static-mesh MERGE (the perf win, never-cut — D192)**: NEW `mergeStaticByMaterial(root)` (wreckForms) collapses a wreck's many static, non-interactive meshes into 1-few per (material, attribute-sig, shadow-flags), baked root-local. Salvage panels stay live (skips `accessPanel`); transparent meshes unmerged. Colliders built per-part FIRST (`attachCompoundCollider`) so collision survives. Wired into `placeProcgenComposite` + `placeWreck` + the mega-wreck (merged into its `shell` sub-group → tilt once + interior stays framable). **Mega-wreck 491→79 meshes (84%); fleet ~254→52 across ~42 wrecks (~80%).** Exterior + interior render identical. (2) **Measurement finding (D193)**: the "~4900 wreck meshes" claim was WRONG — wrecks are ~250; the hand-modeled FLAGSHIPS are the hogs (mega-wreck was 491 alone). NEW `perf-probe` biggest-objects breakdown surfaced it; the 3 other flagships (megaShip/satelliteDish/crashedHull) deferred (need rig-shots). (3) **T3 faceted procgen hulls (D194)**: RIBBED_CYLINDER + PANELED_TAPERED → `makeLoftedHull` faceted SHIP_SECTION (~0.1m plate, lofted-Z-then-rotated-to-+X) instead of smooth cylinders → procgen wrecks read like the hero; panel bury-audit 61/63 (all procgen pass). (4) **T4 half-burial**: windward `makeSandMound` beds each procgen wreck into the dune. **Still owed (honest)**: the 3 flagship merges (the biggest remaining perf win, need rig-shots); the mega-wreck's 2 hand-placed lee-flank panels fail the audit (need `findPanelMount`); the ACAL mega-wreck interior WALK-TEST. **Next session (ACAN)** = flagship merges → mega-wreck panel findPanelMount → T5 greeble/T7 LOD → the walk-test.
