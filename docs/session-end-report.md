...
**Current state**: ⏸ **CAMPAIGN PAUSED — M12 milestone review** (`campaign/2026-06-18`, autonomous; `status: paused`, `awaiting_approval: true`, `stop_reasons: ["milestone-review"]`). **Phase A + Phase B both SHIPPED + reviewed; in the Phase-B review-fix pass (M11→M13).** **M11 COMPLETE (C61-C64); M12 COMPLETE (C65-C67) → PAUSED for the user's batch validation.** Latest = **Campaign C67** (2026-06-22, cycle **67/75**) — **M12 ⓗ: the sand-worm ALERT is now a quiet, mysterious dread buildup (a low rumble + a growing screen-shake), not a loud roar** (the last M12 unit): `verify:all` PASS (tsc + placement 0/0 ×5 + colliders **0/40**), audio/camera driving logic, no rand, no save bump. Removed `playWormRoar()` from `enterAlert`; the C16 rumble graph (reused unchanged) now starts quietly on alert + ramps `0→0.28` over the windup then louder by proximity on the charge; `applyTremorEffects` gained a buildup factor ramping the shake from ~0 over the alert (D267). The roar stays for the lunge STRIKE. **M12 (ⓕ ridges C65 · ⓖ breach-dive C66 · ⓗ alert C67) COMPLETE.** **Prior C64:** M11 COMPLETE (the 3 straggler panels = mega-wreck companion wrecks, `hideAccessPanel`, D265). **Framework upgrade in force:** the visual gate renders the PLAYER'S REAL view (creatures: the rig poses the real worm mesh via the drift-free `applyLungePose`); anti-punt; feel/audio → the per-tier user pause. **Per-cycle detail for the C1–C67 campaign lives in `docs/campaign/campaign-log.md` + `docs/changelog.md`** (this report's session-named history below predates the campaign — ⚠ overdue a prune to ~3 recent tiers + one-paragraph older summaries). **Human action (the M12 review):** walk-test the worm-attack FEEL (the C66 breach-dive) + LISTEN to the alert rumble/shake buildup (C67) → `/campaign-approve` → **M13** (the FINAL review-fix tier: ⓘ gunshot+reload SFX all guns · ⓙ lower smoother speeder hum). **Owed**: the M9/M10 flag-flip + diegetic-HUD + survival walk-tests (backlog §A); the Skyfall hero wreck + cave rework (dedicated solo sessions, NOT the loop).
