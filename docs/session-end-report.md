...
**Current state**: Session ACAJ (partial) shipped (2026-06-06 — wreck procedural-hull toolkit + mega-wreck rebuild; T1+T2 of a 7-tier "wrecks + perf" overnight). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material only). 3 commits; ~5 screenshot-iterated rounds on the mega-wreck (rule 8). **Headlines**: (1) **NEW shared wreck toolkit** (`world/wreckForms.ts`, D185) — the building blocks for believable crashed-ship wrecks: `makeLatheHull`+`fuselageProfile` (tapered hull via LatheGeometry, `axis:'x'|'y'|'z'`, partial-arc torn ends), `makeFormerRings` (exposed rib skeleton at breaks), `makeBreach` (torn hole = recessed void + bent torn-metal flaps, NO boolean/CSG), `makeSandMound` (windward half-burial drift). Each screenshot-verified via `wreck-form`/`__game.wreckFormStudio`. (2) **Mega-wreck exterior rebuilt** (T2, D185) — read as a cluster of dark boxes; replaced the ABL cylinder shell with a curved toolkit hull (tapered aft fuselage + nose + exposed formers at the mid-hull FRACTURE + 3 asymmetric breaches + tapered/raked bridge cap), all FrontSide + `noCollider` so the **entire 120m walkable interior + ~25 colliders + 2 panels + shelter + journal is UNTOUCHED** (the hull ENVELOPS the box so the box reads as inner structure). Reads as a downed ship. (3) **Deliberately deferred T3-T7** (procgen-fleet level-up, sand half-burial, greebles, perf merge, instancing) rather than rush 5 shallow tiers — rule 8 / the ABP lesson; notably T6 (perf merge) touches the salvage-panel interaction (a core loop) and warrants careful verification. **Next session (ACAJ continuation)** = T3-T7 with the toolkit ready.
