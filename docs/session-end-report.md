...
**Current state**: 🟢 **CAMPAIGN ACTIVE** (`campaign/2026-06-18`, autonomous). **Phase A (M1–M5b) + Phase B (M6–M10) both SHIPPED + reviewed; now in the Phase-B review-fix pass (M11→M13)** from the user's 2026-06-20 triage, autonomous with a per-tier batch pause for the user's walk-test/listen. Latest = **Campaign C66** (2026-06-22, cycle **66/75**) — **M12 ⓖ: the sand-worm attack is now a charge → breach-and-DIVE, not a high airborne jump** (2nd of 3 M12 units): `verify:all` PASS (tsc + placement 0/0 ×5 + colliders **0/40**), creature behavior, no rand, no save bump. Removed the old `+20 m` parabolic body arc (`tickLunge`); the body center HOLDS in the sand through the strike (the head rears up to bite via pitch+bend), then DIVES head-first down (D266). Pose math extracted to `applyLungePose` (exported) so the rig renders the REAL pose — no rig-vs-real drift (the C63 false-pass class); added `__game.poseLunge` + `worm-model --angle=strike|dive`. Verified numerically (centerY never above ground: 1.77 < 5.97) + real renders (low breach-strike → head-first dive). Known limit: the rigid pose see-saws the tail up on a steep dive → DIVE_PITCH kept moderate (a path-following dive is the deeper fix). **Prior C65:** M12 ⓕ removed the dorsal scutes (mesh 66→53). **Prior C64:** M11 COMPLETE — the 3 straggler panels were the mega-wreck companion wrecks (`hideAccessPanel`, D265). **Framework upgrade in force:** the visual gate renders the PLAYER'S REAL view (creatures: the rig poses the real worm mesh via the drift-free `applyLungePose`); anti-punt. **Per-cycle detail for the C1–C66 campaign lives in `docs/campaign/campaign-log.md` + `docs/changelog.md`** (this report's session-named history below predates the campaign — ⚠ overdue a prune to ~3 recent tiers + one-paragraph older summaries). **Next** = cycle 67 = **M12 ⓗ alert audio** (quiet low rumble + screen-shake buildup) — the LAST M12 unit → then the **M12 milestone PAUSE** for the user's worm-attack-FEEL walk-test + alert-rumble LISTEN (can't self-verify). **Owed**: the M9/M10 flag-flip + diegetic-HUD + survival walk-tests (backlog §A); the Skyfall hero wreck + cave rework (dedicated solo sessions). **Owed**: the M9/M10 flag-flip + diegetic-HUD + survival walk-tests (backlog §A); the Skyfall hero wreck + cave rework (dedicated solo sessions, NOT the loop).

**(prior) Session ACBB** shipped (2026-06-18 — **FINISH THE WRECK ARC: weathering cohesion + sand integration + a COLLIDER-AUDIT gate + husk/derelict silhouette + §E polish**). `npm run verify` (tsc) PASS; `npm run verify:placement` PASS (0 occlusion + 0 terrain ×5 seeds); **NEW `npm run verify:colliders` PASS** (25 archetype×seed audits, 0 uncovered — wired into `verify:all`); perf-probe field **drawCalls 842** (+3 from new decals, <1000) / programs **69** (≤72, no new programs); no save bump. A long autonomous overnight finishing the ACBA 5-archetype system, driven by a 3-critic adversarial cohesion pass + screenshot iteration. D234-D237. **Tier 1 (weathering cohesion, D234):** the 3 hull buckets collapsed to one warm-brown (cool→rust, dark→pale-ceramic) + the satellite wings rendered near-black → fixed at the `getBucketMats` lever with per-bucket `BUCKET_WEATHERING` channel-strength overrides (cool→cool-steel, dark→a DARK-steel anchor on base `0x3a3d43`, warm unchanged) + wings to in-family grey + LED bezel + `wearAmplitude` break-up; material-only, ZERO new shader programs. **Tier 2 (sand integration):** `makeSandMound` was sinking drifts BELOW the sand → NEW `proud` param (D236, ship callers byte-identical) so drifts BANK against the base; satellite reads crash-landed (list+seat, was the critique's #1 floating/showroom issue), derelict bedded in, debris scorch-disc, tank sand-tongue. **Tier 3 (COLLIDER-AUDIT, D235):** NEW `verify:colliders` + `collider-audit` scenario + `auditPOIColliders` (structural meshes ≥40%-covered via a 3×3×3 grid; decorations/overhead/hollow-shells exempt), wired into `verify:all`; + a grounded flagship `satelliteDish` reflector collider (§E walkthrough). **Tier 4:** husk hollow read FIXED (gap 126°→153°); derelict wide-body → a parallel-hull TRIMARAN (was a perpendicular plus-sign). **§E polish (4/4):** brighter stars (`STAR_BRIGHTNESS 1.55`), antenna rearward, F8 dev keybind, scrap pickup → rusted-metal sheet. **Tier 5 (yard merge) SCOPE-CUT (D237).** **Owed (attended walk-test):** the declared-collider feel + the NEW flagship dish collider + the in-world feel of the banking drifts / crash-poses. **Next session** = the owed walk-test, then optional legacy-ship retirement (→ composite share 1.0, D232) or the remaining §G polish + the deferred Tier 5 yard merge.

**(prior) Session ACBA** shipped (2026-06-17 — **POI VARIETY overhaul: a socket/component/grammar system + 3 non-ship archetypes**). `npm run verify` (tsc) PASS; `npm run verify:placement` PASS (**0 occlusion + 0 terrain fails** ×5 seeds — the gate gained a TERRAIN-AUDIT line); perf-probe field **drawCalls 869** (↓ from 960, under 1000) / programs 69; wreck-yard ground worst-case 3215 (dense-graveyard, frustum-culled in play). SAVE_VERSION 14 (unchanged — POI placement is archetype-agnostic + additive). D226-D231. User feedback after the ACAZ wrecks: still "all long tubes", buggy collision, panels under terrain, a disliked cockpit window + scout. **Phase 0 — bug-fix pass:** removed the cockpit window + the `scout` class (both user-disliked; roulette re-weighted 7-way); `bodies.ts` now SKIPS colliders for `isWreckDecoration` meshes (kills invisible interior walls + cuts collider count); `addHullGreebles` seats greebles on each variant's REAL flank (`flankZ` threaded from `anchorSurfZ`, not the buggy `0.93r` that buried 10cm greebles inside the skin — the "floating/disconnected pieces"); rule-7 depth bumps (SHEARED breach 0.06→0.15, louvers 0.03→0.05); a **corner-aware terrain panel cull** (samples the panel bottom-edge vs sand, size-aware `−0.25` replacing the lenient center-only `−0.10`) + a new **TERRAIN-AUDIT** gate line (interior-safe via a `terrainCullEligible` tag the gen-cull sets). **Phase A (D226, headline):** replaced the linear `cockpit→hull→engine→tail` `+X`-cursor `assembleWreck` (the root cause of "every wreck is a tube") with a general **component/socket/`mate()` grammar** — NEW `world/poiComponents.ts` (a Socket = pos+outward-quat+radius+tag; `mate(parent, parentSocket, childSocket)` glues a child socket OPPOSING a parent socket in ANY topology — radial/vertical/scatter, not just linear; each component DECLARES its Rapier colliders; everything `phash`-derived so components NEVER draw the shared `rand`), NEW `world/poiArchetypes.ts` (the grammar + biome-weighted `pickArchetype`), NEW `world/poiAssembler.ts` (`placeProcgenPOI` = a generalized copy of the proven burial/align/declared-collider/re-skin/merge/salvage-panel/prune pipeline). `bodies.ts` gained `attachDeclaredColliders` + `ColliderSpec` (exact box/cylinder/ball/convex-hull at the assembled world TRS — **no world-AABB fallback**, the D228 collision-accuracy fix). `pickArchetype` rolls `ship` (delegates to the legacy `placeProcgenComposite` → ZERO ship regression; D227 — the ship→socket migration is deferred, now additive) or a new archetype, wired into `procgenPoi` + `wreckYard`. **Phase B — 3 non-ship archetypes:** SATELLITE (foil-wrapped bus + mirrored crash-banked solar wings + dish on a mast + a human-scale hatch), TANK_CLUSTER (3-4 upright tanks + a guaranteed ~10m SILO tentpole on a count-scaled bedded pad, panels face OUTWARD, STANDS vertical via `burySink:false`), DEBRIS_FIELD (torn plates/struts/a central lootable chunk strewn over a disc, post-rotation seated). An **adversarial 5-lens critique Workflow** (D230 — 6 agents reading the render PNGs + the source) caught 3 real collision/grounding bugs (debris strut collider was a vertical capsule over a horizontal bar → walk-through; debris grounding seated by un-rotated bbox → tilted pieces floated/speared; tank dome had no collider) + a panel-mounted-on-a-decoration + the sev-1 quality gaps (floating wings, toylike dish, one-tank-on-a-plinth) → all fixed + re-verified. **Owed (attended, headless can't judge):** the in-world walk-test — felt collision (walk INTO a tank/satellite, the declared-collider fix), felt variety standing among ships + tank farms + satellites + debris under sky/fog, panel seating. **Deferred (additive on the proven foundation):** the HOLLOW-HUSK archetype (the 4th user direction) + the ship→socket "wider/weirder ships" migration (#27); a formal COLLIDER-AUDIT harness; sev-2/3 polish (backlog §G). **Next session** = the owed walk-test OR the husk/ship-migration.

**(prior) Session ACAX** shipped (2026-06-16 — **Salvage-panel interior + exterior REALISM overhaul**). `npm run verify` (tsc) PASS; `npm run verify:placement` PASS (0 bury fails / 5 seeds); NEW `salvage-audit` 0 mismatches / 99 panels; `door-pop` physics smoke test PASS; perf-probe programs 69 / draw calls 728 / sceneMeshes 8602. SAVE_VERSION 14 (unchanged — the salvageables save gained an OPTIONAL additive `extractedIndices?`). D217-D220. A long user-feedback-driven session that reworked the salvage-panel interior + exterior end-to-end. **Stencil portal (D217):** the recessed interior renders as a "window into the hull" — a per-panel MASK writes `stencil=REF`, the interior materials draw `stencilFunc EQUAL` + `depthTest:false` + transparent so they show THROUGH a clipping hull (renderer now built `{stencil:true}`); the MASK itself RESPECTS depth (+ a polygonOffset to win the coplanar flush hull) so terrain / dunes / side-walls / a far-side panel behind the near hull all occlude the window — visible through the WRECK HULL only, never the world (NEW `world/panelPortal.ts`). **Breaker-board interior (D218, headline):** the prior scattered greeble read as "junk in a box"; redesigned via a 3-concept adversarially-judged design Workflow into a real DIN-rail board — a FIXED skeleton (mounting board + brass bus bar + 3 DIN rails + wiring trough + terminal block + 12 grid-aligned empty bay-SOCKETS + labels, merged, NOT salvageable) + 5 salvageable BREAKER modules clipped onto the first bays at the SAME slot+depth, so salvaging a module (extraction only hides) reveals its socket — ZERO jitter (alignment is the realism), ZERO new logic; per-archetype variety on one skeleton (toggles / screen cards / a gauge-valve accent). `mergeStaticByMaterial` gained an `includeTransparent` opt (the portal materials are transparent) → the board collapses to ~1 mesh/material (sceneMeshes 21k→8.6k). **WYSIWYG salvage (D219):** `registerSalvageable` hides the surplus modules past the condition count so visible == salvageable (corroded ~1-2 → "picked over", pristine 5 → "full board") + thins as stripped; `save.ts` persists `extractedIndices` for reload consistency. **Door pop-off (D220):** 50% of pried doors SHEAR LOOSE + fall with real physics (NEW `world/panelDebris.ts`, dropped-item pattern + a `playMetalClang`). Plus: darker/warmer rusted hull + panel-exterior palette, a DOUBLE-SIDED rusted body (fixed "floating sides"), a relief'd rusted iron door, the gold brass frame → rusted weathered-iron, `archetypeForKind` fallback (all 16 callsites get the V2 interior), removed the pre-panel wreck recolor-on-salvage dim + the bandage model. **Owed (attended, headless can't judge):** the breaker-board read under the amber pry-glow + actually salvaging (bays empty); the door pop-off FEEL; confirm the portal no longer bleeds through terrain/sides AND a normal panel doesn't flicker against its coplanar hull. **Next session** = the owed walk-tests OR a new feature (raider proc-character / deep cave / drop-pod).

**(prior) Session ACAW** (2026-06-15 — salvage-panel overhaul VISUAL half, Tiers 3-5, D214-216): `AccessPanelOpts {shape,aspect,archetype}` → square + a CIRCULAR bolted port with a disc LIFT-OFF cover (`coverPivot` named `panelDoor`); NEW `panel-studio` harness; NEW `world/panelGreeble.ts` decorative component library + `buildGreeble` (5 archetypes) — the greeble builders are SUPERSEDED by the ACAX breaker board but still present (exported); NEW `npm run verify:placement` multi-seed scalability gate + the "new wreck class must pass" contract in architecture.md.

**(prior) Session ACAV** (2026-06-15 — salvage-panel overhaul PLACEMENT half, Tiers 0-2, D211-213): unified the 3 drifting bury-raycast copies into one `validatePanels` (NEW `world/panelPlacement.ts`); a SURFACE-scoped center-clearance terrain cull (the phase-through fix) on the procgen/legacy/wreck-yard gen paths, register-all-then-prune; replaced `findPanelMount` (±Z-flank + cardinal snap) with shape-agnostic `findSurfaceMounts` (bounding-sphere rays + full-quaternion flush mount, fixed 1-rand budget). Also reverted ACAS B2's dropped-item capsule/sphere colliders → cuboid (D211). **Tier 0:** collapsed the 3 drifting bury-raycast copies (`pruneBuriedPanels`/`panelBuryAudit`/wreck-yard gate; D210 drift) into ONE `validatePanels` in NEW `world/panelPlacement.ts` (imports only THREE+Tuning, cull via injected callback; provably inert — audit byte-identical). **Tier 1 (the headline phase-through fix):** there was NO terrain check anywhere; added a CENTER-clearance cull (a surface panel whose centre sits >0.10m below the sand is culled) on the three SURFACE-wreck gen paths (procgen composite, the legacy `placeWreck` branch that was never covered, the wreck-yard cluster), all register-all-then-prune (RNG-safe, D208); also dropped the wreck-yard center gate that conditionally skipped `registerSalvageable`. Terrain-culling is SURFACE-scoped not global — interior panels (mega-wreck interior, rockyEntrance chamber −2.05m, flagship recessed bells) are legitimately below the surface (D212). **Tier 2 (the weird-angle fix):** replaced `findPanelMount` (±Z bounding-box flanks + cardinal-yaw 90° snap — broke on cockpits/bells/boxes, never flush) with `findSurfaceMounts` — bounding-sphere inward rays read the REAL hull surface (any shape), footprint flatness+clearance scoring, a FULL quaternion so the panel sits flush; ONE rand = fixed budget (D208); 48 dirs + early-exit keep boot cheap (D213). **Also (D211):** reverted ACAS B2's dropped-item capsule/sphere `colliderHint` colliders → the original cuboid (spheres rolled, thin capsules tunnelled). **Verified:** tsc clean · occlusion bury-audit 94/94 0 fails (seed 1337) · perf-probe boot 964ms + programs 67 (D207 invariant) · panel close-ups show flush mounts + readable cavities. **Deferred — the VISUAL half (next session):** Tier 3 shape/size variants (square + bolted lift-off circular port), Tier 4 the 5-archetype scrappy interior overhaul (decorative greeble + 5 lootable, deeply iterated via a `panel-studio` harness), Tier 5 verification hardening (`placement-torture` + `flagship-audit` + `verify:placement` + the scalability contract). **Next session** = continue the overhaul (Tiers 3-5).

**(prior) Session ACAU** (2026-06-15 — bury-audit register-all-then-prune + material shared-noise helper, D209-D210): `placeProcgenComposite` register-all-then-prunes via the exported `pruneBuriedPanels` (the 4th seed-1337 fail was a wreck-yard CROSS-wreck bury → a 2nd cluster pass against `yardGroup`; door subtree excluded for open-door parity); `panels` audit 0 fails across 5 seeds. NEW `world/shaderNoise.ts` `iqNoise2D(...)` replaced the IQ noise GLSL hand-copied across 11 material factories (byte-identical, ~260 dup lines gone, programs 67). **(1) Bury-audit done right (D210, closes D208's deferred fix):** `placeProcgenComposite` now register-all-then-prunes — every panel-bearing part still `registerSalvageable`s unconditionally (skipping desyncs the seeded `rand`, D208), then a NEW exported `pruneBuriedPanels` 2nd pass raycasts each panel and drops the occluded ones (removes the record + clears `interactType`/`interactId` → inert). A temporary occluder-trail diagnostic on `panelBuryAudit` revealed the stubborn 4th seed-1337 fail wasn't self-occlusion but a wreck-yard **CROSS-wreck** bury (`rootName:"wreckYard"` — the panel sits behind a NEIGHBOUR's hull merged into the cluster mesh), so the helper is also called from `wreckYard.ts` AFTER `mergeStaticByMaterial(yardGroup)`, raycasting the whole yard (mirrors the audit's walk-up `root=wreckYard`). The prune raycast EXCLUDES the panel's `panelDoor` subtree to reproduce the audit's open-door state (build-time doors are closed → a proud closed door masks the occluder). **Verified — `panels` bury-audit 0 fails across 5 seeds (1/2/42/1337/2024)**; seed 1337 went 133 panels/4-fails → 129/**0** (drops exactly the 4 D208 predicted). RNG-safe (the prune does no `rand`). **(2) Material shared-noise helper (D209, the D207 stretch):** NEW `world/shaderNoise.ts` `iqNoise2D({hash,valueNoise,fbm,octaves})` replaces the IQ hash/value-noise/fBm GLSL block that **11** material factories (metal/hull/stone/paint/wood/glass/bone/skin/fabric/concrete/terrain) each hand-copied; each factory calls it with its EXACT existing function names + octave count so the emitted GLSL is byte-identical (zero call sites changed) → provably pixel-identical, ~260 dup lines gone. **Verified:** tsc clean · `procgen-wreck` render pixel-identical to baseline · `perf-probe` programs **67** (the D207 invariant — every material still compiles, sharing intact) · no shader errors. **Next session** = the still-owed human walk-tests (Sarlacc pull-feel + climb-out, dropped-item settle, graveyard read, mega-wreck interior — NONE done yet) OR a buildable feature now the perf/material/bury debt is cleared (raider proc-character / deep cave design pass / drop-pod intro).

**(prior) Session ACAT** (2026-06-15 — perf + wreck-detail debt: material-uniforms + brighter hulls, D207-D208): T1/W5 lifted the procgen hull base color (`WRECK_HULL_HEX` 0x5f5b54→0x6a6657) + lowered `_hullMat` streak/AO so the greeble detail surfaces; T3 (D207) converted the 7 remaining material factories from `.toFixed()`-baked GLSL → uniforms + runtime branches, dropping the per-instance `customProgramCacheKey` (default `onBeforeCompile.toString()` key + the fabric/skin `pbr` base-class fork) → **perf-probe programs 105→67, boot shader-compile 270→197ms**. Deferred T2 bury-audit (D208 — register-time skip desyncs the RNG; the register-all-then-prune fix landed ACAU) + T4 flagship greebles (architecturally awkward; T1 covers the intent).

**(prior) Session ACAS** (2026-06-15 — wreck-yard perf+polish + item/collision breadth, D205-D206): yard-merge for the loose props (graveyard 4082→3994 draw calls); speeder static-merge (~40+→14 meshes, D205); graveyard ground mottle; big hand-wrecks lootable; denser maw; cloth→draped folds + skewer→real-branch; per-item dropped-collider shapes (`colliderHint`, D206); crafting-chooser discovery-bug fix. The 4-agent audit found the roadmap's "~19 item models" stale (all 62 already hero) + the chooser already built. NEW dev hooks: `dropTestItem`/`craftChooserTest`/`injectTestRecipe`. Owed (human): pull-feel + dropped-item settle + graveyard/interior walk-tests.

**(prior) Session ACAR2** (2026-06-14 — Sarlacc pit RECESSED into the terrain, D204): `terrain.ts` carves a funnel crater into the shared `heights` array (radial smoothstep on distance to `sarlaccPitAnchor`, deepest at center) so the visual mesh + Rapier heightfield collider + `heightAt()` all dip together and the player descends into the bowl (depth 13 / clearing 24 → peak wall ~39° < KCC 50° climb limit; sand dusked toward center). `sarlaccPit.ts` maw rebuilt at the crater floor (fleshy collar + leathery lip + glowing gullet + central beak + 3 teeth tiers + 9 tapered tentacles); FSM animates the maw only. (Folded in ACAR: the pit got its own `biomes.sarlaccPitAnchor`.) Pull feel + climb-out flagged for the walk-test. User feedback: the ACAQ Sarlacc pit read as a raised sand-MOUND on top of the dunes; it should sink INTO the sand like the Star Wars Sarlacc. Researched the Great Pit of Carkoon via a 4-agent parallel sweep (overall crater form / beak+teeth / tentacles+color / code-archaeology of the terrain-carve) and rebuilt it recessed (~3 crater + ~2 maw screenshot-iteration rounds). **Headlines**: (1) **Terrain carve (D204)**: `terrain.ts` carves a funnel crater into the shared `heights` array (radial smoothstep on distance to `sarlaccPitAnchor`, deepest at center, eased to 0 at the clearing rim) — so the visual mesh + the Rapier heightfield collider + `heightAt()` all dip together and the player physically descends into the bowl. `SARLACC_PIT_CRATER_DEPTH=13` / clearing=24 → peak wall ~39°, under the KCC 50° climb limit (escapable, no softlock). Sand dusked toward center (`BIOME_COLOR_SARLACC_PIT`) so the recess reads in flat light. (2) **Recessed maw**: `sarlaccPit.ts` `makeSarlaccPitMesh` rebuilt to sit at the carved floor (nothing above the sand) — fleshy collar + leathery rim lip + warm-glowing gullet + central **beak** (1997 SE: lathe core + splaying mandibles) + **3 ivory teeth tiers** + **9 tip-tapered sinuous tentacles**. (3) **FSM**: `updateSarlaccPit` no longer sinks the whole mesh (crater is permanent terrain) — only the maw animates (beak gnash, mandible splay, teeth gape/clench, tentacle writhe, gullet pulse). (4) **Framer**: `wreck-yard` rig-shot pinned to a fixed seed (deterministic shots) + new `maw`/`pit-eye` angles. Smoke test re-passes (open → pull 5.6→0.8m → bite; mesh stays at crater floor). Also folds in the prior **ACAR** follow-up (separated the pit onto its own `biomes.sarlaccPitAnchor`). **Still flagged for the attended WALK-TEST**: the pull feel + whether the player can climb back out of the bowl while the pull is active. **Next session** = the wreck-yard + mega-wreck WALK-TESTS (human) + the flagged perf follow-ups.

**Recent sessions (condensed — full detail in changelog.md):**
- **ACAQ** (2026-06-14): marathon overnight — Wreck-yard biome (Cycle 8): NEW `wreck_yard` distance-override destination biome (seed-derived anchor 620-1000m out; D201) + `wreck-yard` framer; NEW `wreckYard.ts` dense graveyard (~30 corroded wrecks + bone-fields + debris); NEW `relic_core` exclusive glowing loot; NEW `sarlaccPit.ts` Sarlacc hazard (recessed in ACAR2; gapes + pulls via the KCC desired-vector + bites; D202); vulture ecology; graveyard perf merge 2055→1664 via `group.attach` (D203).
- **ACAP** (2026-06-14): overnight wreck-arc finish — hand-POI static-merge (opening wreck 80-130→13, entrances; D198, merge hardened to skip interactType) + shared hull-material depth (form-AO + oxidation + flecks lift every wreck; D199) + crash-debris fans (merged, rest-on-sand via local-y=buryY; D200).
- **ACAO** (2026-06-13): procgen-wreck rig-shot framer (the BLOCKER — `spawnProcgenWreckRig` deterministic spawn + orientation-pin + the `procgen-wreck` scenario; D197 found `makeBreach` doesn't read on small intact hulls → reverted) + T5 greebles 3→7 vocab + impact asymmetry; Windows rig-shot teardown fix.
- **ACAN** (2026-06-08): flagship static-merge (megaShip 160→67, satelliteDish 148→47, crashedHull 51→44; D195) + the `flagship` find-by-name rig-shot; mega-wreck panel faceYaw sign fix → bury-audit 68/68 (D196).
- **ACAM** (2026-06-08): wreck-fleet level-up — WebGL static-merge perf (`mergeStaticByMaterial`; mega-wreck 491→79, fleet ~254→52; D192), the measure-first finding that flagships are the hogs (D193), faceted procgen hulls (D194), half-burial.
- **ACAL** (2026-06-07): mega-wreck walkthrough fixes — exact full-model trimesh collision (D189), natural-only lighting (D190), open-fracture entrance + ramp, thicker hull + de-floated interior (D191, `thickness`/`arc` on the toolkit + `ceilingY`/`hullHalfWAt` samplers).
- **ACAK** (2026-06-07): mega-wreck FROM-SCRATCH rebuild — sleek crashed DAGGER (`makeLoftedHull` faceted hull, tilted shell, `hullAt` sampler) + aligned wreck interior built in the same tilted frame; adversarial critique/defect-hunt workflow harness (D186-188).
- **ACAJ (partial)** (2026-06-06): wreck procedural-hull toolkit `wreckForms.ts` (`makeLatheHull`/`makeFormerRings`/`makeBreach`/`makeSandMound`) + the FIRST mega-wreck rebuild (wrap-the-box curved shell — D185, later reworked from scratch in ACAK).
- **ACAI** (2026-06-06): vulture rig + per-state anim + relocate-land FSM + dynamic-body death (D181-182) + carcass ecology (circle/swoop/carry-feed-return, shrew burrow-escape, meat scavenge — D183-184) + gaunt model + flight head-fix.
- **ACAH** (2026-06-05): big overnight — bug sweep + loot bootstrap (D178) + game-wide shader-cache fix (D177) + NEW vulture creature (D179, static mesh) + cloud shadows + mounted-lighting fix (D180).
...
1. **ACAJ continuation — finish the wrecks + perf overnight (T3-T7)** with the `wreckForms.ts` toolkit now built. **T3**: apply the toolkit to the shared procgen part vocabulary (`wrecks.ts` hull/cockpit variants + `procgenWreck.ts` `HULL_SEGMENT_VARIANTS` → Lathe hulls + exposed formers + real `makeBreach` holes) → levels up all ~80 procgen wrecks. **T4** half-burial + `makeSandMound`. **T5** greeble + asymmetry. **T6 (never-cut)** WebGL wreck perf merge (`mergeGeometries` static-by-material; panels/colliders separate; `perf-probe` delta). **T7** instancing/LOD. Full brief in `next-session-prompt.md`. **Keep the salvage loop intact — `panelBuryAudit` PASS + panel interaction unchanged.**
2. **Foreground feel-tune playtest** (needs a human): the owed ACW/ACX in-motion pile (D150) + the vulture in-motion feel (flap/flight-arc/swoop/carry-feed-return/tumble — `VULTURE_*` block) + the cloud-shadow strength.
3. **Remaining smaller breadth**: material-factory→uniforms, desktop packaging (Electron) + WebGPU exploration, procedural-repairable speeder bikes, ODST drop-pod opening cutscene, painted-metal rust gap — all in backlog.

---

## Time spent

100+ sessions shipped (A through ACAJ). ACAJ (partial) was the opening 2 tiers of a planned 7-tier wrecks+perf overnight, run in the same long session as the ACAI vulture work: T1 the shared `wreckForms.ts` toolkit + T2 the mega-wreck exterior rebuild — 3 commits (`0f3d718`→`5262f0e`). Touched NEW `wreckForms.ts`, `megaWreck.ts` (exterior shell swap), `debugPanel.ts` (`wreckFormStudio`), `rig-shot.mjs` (`wreck-form`/`megawreck` scenarios). No save change. **T3-T7 deferred** to keep the iterated tiers deep (rule 8 / ABP).

---

## State at session end

- **Git status**: ACAJ feature work committed in 3 commits (`0f3d718` T1, `5262f0e` T2 + the megawreck-scenario fixes) on `master`; the session-end doc set committed + tagged `session-ACAJ`.
- **Branch**: `master`. **Save state**: localStorage **v14** (unchanged — wreck rebuild is geometry/material only; colliders/panels rebuild from seed — no bump).
- **Ports bound**: a `npm run dev` server may still be running on **5173/5174**; the rig-shot harness used **5191** (transient — Windows `dev.kill()` can orphan the vite child).
- **Verification status**: tsc clean throughout. Toolkit primitives + the mega-wreck silhouette screenshot-iterated (`wreck-form`, `megawreck` — bow nose → exposed-rib fracture → aft dome reads as a downed ship); interior/colliders/panels/shelter/journal untouched (FrontSide + `noCollider` shell). **Deferred (next session)**: T3-T7 (procgen level-up, sand, greebles, perf merge, instancing). **Owed (foreground)**: the standing vulture/ACW-ACX in-motion feel pile (D150).

---

## Iteration-discipline self-check (rule 8)

ACAJ shipped pure VISUAL surface, fully iterated — NOT shipped on `tsc` alone. T1: each toolkit primitive (lathe hull, formers, breach, mound) was rendered in isolation via `wreck-form` and confirmed (lathe = clean tapered fuselage, formers = torn-open hull showing its rib skeleton, breach = torn hole). T2: the mega-wreck silhouette went through ~5 screenshot-critique-iterate rounds (`megawreck` 3q+side) — box-cluster baseline → curved dome (box corners poked out) → enlarged hull to envelop the box → exposed-rib fracture + breaches + tapered bridge cap → reads as a downed ship. HONEST scope note: only **2 of 7 tiers** shipped — T3-T7 deferred ON PURPOSE (rule 8: ship fully-iterated tiers, not 5 shallow ones; T6 perf-merge touches the salvage-panel interaction and needs careful verification, not a rushed pass). This is the "1-2 iterated tiers" outcome, not the "5 shallow tiers" failure mode.

---

## Token spend this session (estimated)

ACAJ T1+T2 ran inside an already-very-long session (after the full ACAI vulture arc + ecology). Tool-heavy: research fan-out (2 parallel agents — Explore code survey + game-researcher references), then many build→rig-shot→read-image→iterate loops on the toolkit primitives + the mega-wreck (each rig-shot boots its own Vite+Playwright ~40s).

- Input: high — the research-agent digests + reads across `megaWreck.ts`/`wrecks.ts`/`procgenWreck.ts`/`openingWreck.ts` + repeated image reads.
- Output: moderate — NEW `wreckForms.ts` (~210 lines), `megaWreck.ts` shell swap, `wreckFormStudio` hook, 2 rig-shot scenarios, D185, the session-end doc set.
- Cost (Opus 4.8 rates): elevated (this whole session bundled the ACAI vulture arc + ACAJ T1+T2). Justified — shipped the wreck toolkit foundation + the hero mega-wreck fully iterated.

---

## Commit handoff

Per CLAUDE.md (session-end auto-runs commit + tag + push). The ACAJ feature work is committed + pushed (3 commits on `master`). The session-end doc edits (changelog/CLAUDE/roadmap/decisions/backlog/report/next-prompt) are committed at session-end + tagged `session-ACAJ`.
