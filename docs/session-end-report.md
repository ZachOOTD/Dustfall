...
**Current state**: Session ACZ shipped (2026-06-03 — item-model detail pass part 2: the remaining ~22 models). 100+ sessions post-MVP. `npm run verify` (tsc) PASS. **SAVE_VERSION 14** (unchanged — ACZ is mesh-only, 1 file). ACZ finished the ACY item-model thread: **every item in the game now has a deep-detailed or verified-at-parity mesh.** Deep-polished 7 primitives (tent_kit, sled_kit, grill_kit, companion_pod, cactus_pulp, alien_fruit, raw_worm_meat), light-touched branch, verified the rest at parity — all via the ACY `item-studio` harness. **The item-model arc is now COMPLETE.** **Next session (ACAA)** = pick a lane: (a) DEEP CAVE SYSTEM design+build (default — the big standing vision; needs a collision-below-heightfield spike) or (b) foreground feel-tune playtest of the owed ACW/ACX in-motion pile (needs a human).

## ACZ scope (this session) — item-model detail pass part 2: the remaining ~22 models
