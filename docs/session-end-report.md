...
**Current state**: Session **ACAP** shipped (2026-06-14 — **overnight: finish the wreck arc — hand-POI static-merge + shared-material depth + crash debris**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material/tooling only). D198-D200; 3 commits. A measure-first long overnight, 3 fully-iterated tiers. **Headlines**: (1) **W1 hand-POI merge (D198)**: a NEW `perf-probe` deep-dump (`topGroupsDeep` child-kind histogram) showed the remaining draw-call hogs were the hand-modeled POIs never given the ACAM/ACAN static-merge → merged render-identical (**opening wreck ~80-130→13, rockyEntrance →37, saltOutpost →28**); hardened `mergeStaticByMaterial` to also skip `userData.interactType` subtrees (so merging arbitrary hand POIs can't fold a live interaction into a static mesh); **heavy distance-LOD CUT** (the merge already won the budget — speeder + pickups flagged as attended follow-ups, not forced). (2) **W3 shared-material depth (D199)**: `createRustedHullMaterial` read flat-grey → added form-AO (volumetric undersides) + bare-metal scuff-flecks + warm oxidation-zones (color depth) + wearAmplitude 0.15→0.20; ONE shared-material change lifted EVERY wreck (procgen corvette/bulk_hauler + the mega-wreck hero + the megaShip flagship), blast-radius-verified none over-rusted. (3) **W4 crash-debris fans (D200)**: 2-4 fragments on ~60% of procgen wrecks, added to the group BEFORE the merge (~0 draw cost) at local-y=buryY so they rest on the sand after the half-burial sink. **Deferred**: W2 flagship greebles (W3 already lifted them), W5 brighter lighting (global/risky headlessly), the ACAL mega-wreck interior WALK-TEST (needs a human). **Next session (ACAQ)** = the interior walk-test (human) + the attended perf follow-ups (speeder static-merge, pickup InstancedMesh).
