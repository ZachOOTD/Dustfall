...
**Current state**: Session **ACAQ** shipped (2026-06-14 — **marathon overnight: Wreck-yard biome (Cycle 8) — graveyard + Sarlacc pit**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material/additive-item only). D201-D203; 5 commits. A long marathon building a whole new rare DESTINATION biome end-to-end (6 tiers Y1-Y6), planned via 4 parallel read-only investigations (biome/POI/loot/sandworm). **Headlines**: (1) **Y1 biome (D201)**: `wreck_yard` is a DISTANCE-OVERRIDE disc around a seed-derived anchor 620-1000m from spawn (one per seed) — not a noise band; the sampler exposes `wreckYardAnchor` + a `wreckYardAt(x,z)` strength field driving terrain's ashen-ground tint + basin flatten; NEW `wreck-yard` rig-shot framer. (2) **Y2 graveyard**: NEW `wreckYard.ts` packs ~26-38 corroded procgen wrecks + big tilted silhouettes + ribcage bone-fields + debris, with a 24m center clearing for the pit. (3) **Y3 loot**: NEW additive `relic_core` (glowing alien-tech artifact, near-full-restore) scattered as graveyard-exclusive glowing pickups. (4) **Y4 hero (D202)**: NEW `sarlaccPit.ts` — a raised sand-maw (4 screenshot-iterated rounds) that gapes when the player nears, PULLS them toward the throat (`ctx.player.externalPull` → the KCC `desired` vector) + BITES; smoke-tested (pull 5.6m→0.3m, health 1→0.86). **Pull FEEL flagged for attended tune.** (5) **Y5 ecology**: the graveyard ribcages join the vulture ecology (approach telegraph). (6) **Y6 perf (D203)**: the dense field (2055 draw calls) re-parents wrecks via `group.attach` + a yard-level `mergeStaticByMaterial` → 1664. **Deferred/flagged**: route mounds/debris/bones into the yard merge; the pull-feel + a wreck-yard WALK-TEST (human); the muddy-bare ground color; the big-wreck panels aren't registered. **Next session (ACAR)** = the wreck-yard + mega-wreck WALK-TESTS (human) + the flagged perf follow-ups.
