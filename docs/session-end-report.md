...
**Current state**: 🟢 **CAMPAIGN ACTIVE** (`campaign/2026-06-18`, autonomous, PHASE-level review). **Phase A (M1–M5b) shipped (cycles 1–36); Phase B (M6–M10) APPROVED + RELEASED 2026-06-20** and now building unattended. Latest = **Campaign C38** (2026-06-20, cycle **38/75**) — **M6 ② survival-rebalance-newgame (the KEYSTONE)**: `verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25), **survival-probe gate PASS**, **code-auditor PASS**, **no save bump**. Survival is now REAL (`GOD_MODE` flipped off) + FORGIVING (Long Dark): a prepared player survives indefinitely + slowly heals (new health-regen-when-provisioned); a neglected player dies in ~8–10 min on the urgent paths (heat/cold/thirst), ~15 min on hunger; the godmode floor is gated on `devMode` so dev boots stay safe (D246). NEW `survivalProbe`/`triggerDeath` hooks + a `survival-probe` rig scenario pin the time-to-death bands + the death→overlay wiring. **Per-cycle detail for the C1–C38 campaign lives in `docs/campaign/campaign-log.md` + `docs/changelog.md`** (this report's session-named history below predates the campaign). The loop pauses next at the **Phase B milestone** (after M10). **Next** = cycle 39 = **M6 ③ flat-color-texture-audit** (scope-first — upgrade the ~6–8 weakest flat-shaded surfaces via the existing procedural shader factories; no new programs/assets). **Owed**: the survival-curve FELT pacing + stamina-cadence walk-test (backlog §A).
