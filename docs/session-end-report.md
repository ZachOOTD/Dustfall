...
**Current state**: Session **ACAS** shipped (2026-06-15 — **overnight: wreck-yard perf+polish + item-fidelity / collision breadth**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material/collider/data only). D205-D206; 5 commits. Unattended overnight on two user-picked themes (options 3+4), max-depth budget, opened with a 4-agent parallel AUDIT that reshaped the work — the roadmap's "~19 item models need fidelity" was **stale/false** (all 62 are DECENT/HERO) and the crafting chooser UI **already existed**, so B1/B3 became focused/honest rather than sprawling. **Headlines:** **A1** — `placeProcgenComposite`/`placeDebrisField`/`placeRibcage` gained an optional `parent`; `placeWreckYard` routes the sand mounds + debris + ribcages into the yard merge (graveyard 4082→3994 draw calls). **A2 (D205)** — static-merged the speeder body (~40+→14 meshes): collider is hand-defined cuboids (merge-safe), live parts (headlamp-disc/tow-bar/antenna) tagged `noMerge`, lights are direct group children (untouched); `speeder-fx` verifies refs survive + it drives. **A3** — graveyard ground mottle (oil pools + ash drifts). **A4** — big hand-wrecks now LOOTABLE (register their existing panels from `wreckYard` after `placeWreck`, above-ground only; salvageables=75). **A5** — denser Sarlacc maw (46→63 teeth). **B1** — `cloth` → soft draped folds + lizard-skewer spit → real branch (rest already hero). **B2 (D206)** — `ItemDef.colliderHint` (box|sphere|capsule): shape from the hint, SIZE from the bbox; dropped pipe/rifle/branch lie as rods, flask/orb as balls; `drop-test` confirms settle (FEEL flagged). **B3** — fixed the crafting chooser's discovery-spoiler (undiscovered → "?") + verified it end-to-end (`craft-chooser`); dormant only because no recipes collide. NEW dev hooks: `dropTestItem`/`craftChooserTest`/`injectTestRecipe`. **Owed (human, npm run dev):** the recessed-pit pull feel + climb-out (D204), the wreck-yard graveyard read, the mega-wreck interior, and the new dropped-item settle feel. **Next session** = those walk-tests + the deferred pickup-InstancedMesh + (open design) activating the chooser via a colliding recipe.
