...
**Current state**: 🟢 **CAMPAIGN ACTIVE** (`campaign/2026-06-18`, autonomous, PHASE-level review). **Phase A (M1–M5b) shipped (cycles 1–36); Phase B (M6–M10) APPROVED + RELEASED 2026-06-20** and now building unattended. **M6 (Survival & UX) COMPLETE (C37-C40); M7 (Wreck depth & new POIs) underway (⑤ done, ⑥ COMPLETE).** Latest = **Campaign C43** (2026-06-20, cycle **43/75**) — **M7 ⑥ more-wreck-types-new-pois COMPLETE — the WELL + the DEBRIS-TRAIL**: `verify:all` PASS (tsc + placement 0/0 ×5 + colliders **0/40**), **visual gate PASS** (well 2 rounds, trail 3 rounds), **no save bump**. Finished ⑥ (watchtower was C42): a **WELL** (`wellHead()` — a metal curb around a recessed dark shaft + a 2-post windlass-drum winch with a hanging bucket) + a **DEBRIS-TRAIL** (`assembleDebrisTrail` — a directional crash-ejecta streak: a big tilted impact chunk + a dark skid/crater with debris trailing back getting SMALLER; `debrisPiece` gained a `scale` param + a dark `_skidMat`). Both registered in `ARCH_WEIGHTS` + the collider-audit (30→40). D251. Also did the overdue **decisions.md archival** (D189–D206 → archive; active back to ~45). 3 new non-ship POIs now vary the scatter beyond ships. **Per-cycle detail for the C1–C43 campaign lives in `docs/campaign/campaign-log.md` + `docs/changelog.md`** (this report's session-named history below predates the campaign). The loop pauses next at the **Phase B milestone** (after M10). **Next** = cycle 44 = **M7 ⑦ walkable-wreck-interiors** (the LAST M7 unit; XL, spike→build). **Owed**: the diegetic-HUD + survival + wreck + watchtower + well/trail FELT walk-tests (backlog §A); a full ship→socket migration (D227/D249).
