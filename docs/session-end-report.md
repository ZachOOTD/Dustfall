...
**Current state**: Session **ACAO** shipped (2026-06-13 — **procgen-wreck rig-shot framer (the BLOCKER) + T5 greebles & impact asymmetry**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material/tooling only). D197. Built the verification unblock the ACAN T5 attempt was missing, then shipped the procgen visual level-up it enables. **Headlines**: (1) **Framer (the BLOCKER)**: procgen wrecks are unnamed + random-positioned, so no rig-shot could frame one (rule 8 killed the ACAN breach attempt). NEW `__game.spawnProcgenWreckRig(cls, seed)` spawns a chosen wreck CLASS at a fixed clear anchor with a DETERMINISTIC seed + **pins its orientation** (kills `placeProcgenComposite`'s random yaw so the +Z detail flank — breaches + panels — faces the camera) + names it `procgenWreckRig`; NEW `procgen-wreck` rig-shot reuses the `flagship` find/frame/mesh-count logic, is ground-aware (frames the EXPOSED hull above the sand line, not the buried bbox), and adds a `--seeds=` sweep (N shots per dev-server boot) + `--zoom`. Also fixed a Windows vite-orphan teardown hang (`dev.kill()` left vite holding the port + stdio pipes → harness wedged after the screenshot; now `taskkill /T /F` + `process.exit(0)`). (2) **T5**: `addHullGreebles` vocabulary 3→7 types (louvered vents, circular ports/portholes, fins/strakes, antenna stubs) + impact-flank asymmetry (~78% of greebles + the damage patch cluster on the side that "took the hit") on RIBBED_CYLINDER + PLATED_RECTANGULAR; verified corvette (small) + bulk_hauler (large); **bury-audit 56/56 ALL CLEAR** (greebles tagged `isWreckDecoration`). (3) **Reverted (D197)**: real `makeBreach` holes don't read on small intact procgen hulls (no boolean cut → recessed void occluded by the skin / proud void reads as a bump; only the 136m hero has the scale) — kept the flat dark damage patch. **Still owed**: T7 InstancedMesh/LOD (stretch); the ACAL mega-wreck interior WALK-TEST (needs a human in `npm run dev`). **Next session (ACAP)** = T7 LOD → the interior walk-test → richer flagship greebles + a brighter-lit procgen pass.
