...
**Current state**: 🟢 **CAMPAIGN ACTIVE** (`campaign/2026-06-18`, autonomous, PHASE-level review). **Phase A (M1–M5b) shipped (cycles 1–36); Phase B (M6–M10) APPROVED + RELEASED 2026-06-20** and now building unattended. **M6 (Survival & UX) COMPLETE (C37-C40); M7 (Wreck depth & new POIs) underway.** Latest = **Campaign C41** (2026-06-20, cycle **41/75**) — **M7 ⑤ procedural-wreck-overhaul**: `verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25), **visual gate PASS in 3 rounds** (recon agent + 5 critics), **no save bump**. Fixed "every wreck is a long tube": the socket-grammar `derelict` archetype gained 2 NEW structure-axis components (`splayedEngineCluster`, `dorsalMast`), grew 3→5 forms (no bare-tube form left), got an elongated hull, and was re-weighted vs the legacy linear `ship` so the wider/weirder hulls appear ~as often as tubes (D249). A NEW rig `--pinyaw` length-frame fixed an end-on framing artifact (a Z-long ship read as a "blob"). **Per-cycle detail for the C1–C41 campaign lives in `docs/campaign/campaign-log.md` + `docs/changelog.md`** (this report's session-named history below predates the campaign). The loop pauses next at the **Phase B milestone** (after M10). **Next** = cycle 42 = **M7 ⑥ more-wreck-types-new-pois** (watchtower/debris-trail/well via the socket grammar). **Owed**: the diegetic-HUD + survival + wreck-overhaul FELT walk-tests (backlog §A); a full ship→socket migration (D227/D249); **decisions.md archival overdue** (61 entries).
