...
**Current state**: Session **ACAW** shipped (2026-06-15 — **Salvage-panel overhaul COMPLETE: the VISUAL half (Tiers 3-5 of 6) — shapes + scrappy interiors + scalability gate**). `npm run verify` (tsc) PASS; `npm run verify:placement` PASS (0 bury-audit fails across 5 seeds). SAVE_VERSION 14 (unchanged). D214-D216. A long autonomous session finishing the ACAV overhaul — all 6 tiers now shipped. **Tier 3 (D214):** `addAccessPanel` gained `AccessPanelOpts {shape,aspect,archetype}` (absent ⇒ today's exact rect); `'square'` = rect aspect 1, `'circle'` = a CylinderGeometry bore + a bolted torus ring rim + a disc LIFT-OFF cover on a `coverPivot` NAMED `panelDoor` (so the audit/prune door-exclusion + `completePry` apply unchanged; `updatePanelDoors` slides+tumbles it normalised by the open-angle). Procgen derives shape from already-rolled values (engine→circle, small→square). NEW `panel-studio` harness (`__game.spawnPanelStudio` + a `panel-studio` rig-shot scenario with `--sweep`/`--allarch` + a cavity fill light + tight framing) = the screenshot loop. **Tier 4 (D215):** NEW `world/panelGreeble.ts` low-poly component library (coiled wire, fuse bank, PCB+chips, gauge, valve wheel, conduit elbow, terminal block, frayed wires, cracked screen, emissive indicator) + `buildGreeble(archetype)` depth-layered; INTERIOR_V2 = MERGED decorative greeble (the wreck merge skips accessPanel subtrees, so merge BEFORE parenting under the body) + the 5 UNCHANGED lootable components (reuse the 7 existing kinds → loot economy + save untouched); 5 archetypes (electrical/plumbing/avionics/mechanical/junction) screenshot-iterated 3-4 rounds each; shared singleton materials → programs stayed 67. **Tier 5 (D216):** NEW `npm run verify:placement` multi-seed bury-audit = the scalability gate a future wreck-model change re-runs; contract documented in `architecture.md`. **Verified:** tsc clean · `verify:placement` 0 fails (seeds 1/2/42/1337/2024) · perf-probe programs 67 + draw calls 891 + boot 1145ms · panel-studio shows all 3 shapes × 5 archetypes reading as distinct scrappy salvage panels. **Owed (attended, headless can't judge):** the in-world pry-FEEL — prying a panel + the amber-glow-lit cavity read (the audit force-opens doors without igniting the glow). **Next session** = the owed walk-tests OR a new feature (raider proc-character / deep cave / drop-pod).
