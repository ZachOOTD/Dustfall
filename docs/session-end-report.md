...
**Current state**: Session **ACAK** shipped (2026-06-07 — **mega-wreck FROM-SCRATCH rebuild: sleek crashed DAGGER + aligned wreck interior**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material/collider only, rebuilds from seed). ~18 commits (`6c3fe99`→`bcff628`); D186-D188. The user flagged the ACAJ wreck as boxy/shipped-too-early; the WHOLE session reworked the hero mega-wreck ground-up via web research + an adversarial **workflow harness** (independent critic/defect-hunter agents that READ the rig-shot PNGs + the geometry code with line numbers → ranked fix lists; the iteration engine — D188). **Headlines**: (1) **Exterior = a sleek crashed DAGGER** (replaces the wide hauler, D186): NEW `makeLoftedHull` (a FACETED ship-hull cross-section lofted along Z — replaces the lathe "dome" that read as a lump); the whole exterior in a `shell` group tilted ~17° into a list + sunk, pivot-compensated (`shellQuat()`/`shellPos()`) so it kisses the ground; a `hullAt(z)` sampler so flank detail sits ON the curved hull (no floating/clipping); ~5:1 dagger, sharp bow driven into the dune, mid-hull FRACTURE notch + cut-open cross-section, tapered command island + mast/dishes, 2 hollow engine bells on a transom plate, plating strakes/ribs, rust streaks, tilt-aware half-burial + debris; coarse exterior collider proxy. (2) **Interior v2 = the broken GUTS of the dagger** (scrapped the clean corridors; D187): `interiorDecks()` built in the SAME tilted `shell` frame (one rigid listed object — the root fix: a level box-cavity under a tilted shell never agreed with itself); the floor carries the full ~17° list (user's call); the DoubleSide hull IS the walls/ceiling (cavity = `hullAt` minus inset → no poke-through); wreckage (ribs, peeled bulkheads, varied debris, beams, cables) + cargo/survivor set dressing + a dressed bridge; the open fracture is the HERO daylight flood; collision-correct decks + curbs + prop colliders + a STEPPED fracture crossing fixing a ~2m unclimbable step that had blocked the bridge payoff. (3) **NEW reusable workflows**: `megawreck-research`/`-interior-research`/`-critique`/`-interior-defects`; `megawreck` rig-shot gained front-lit framing + 5 interior camera angles. **Still owed (honest)**: the interior needs an **in-app WALK-TEST** (a ~17° canted floor + the 9-step fracture crossing can't be verified from screenshots) + a deeper lighting/material pass. **Next session (ACAL)** = walk-test → interior lighting/material polish → the deferred ACAJ T3-T7 (procgen fleet on the bigger toolkit).
