...
**Current state**: Session **ACAT** shipped (2026-06-15 — **perf + wreck-detail debt: material-uniforms + brighter hulls**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — material/tuning only). D207-D208; 3 commits. User-chosen "perf + wreck-detail debt" bundle, planned via 3-agent exploration; two tiers shipped, two honestly deferred after investigation reshaped them. **T1 (W5):** lifted the procgen hull base color (`WRECK_HULL_HEX` 0x5f5b54→0x6a6657) + lowered the shared `_hullMat` `streakIntensity` 0.55→0.40 + `aoStrength` 0.34→0.24, so the ACAO greeble/seam detail surfaces (verified corvette+gunship). **T3 (headline, D207):** converted ALL 7 remaining procedural material factories (glass/bone/stone/paint/wood/fabric/skin) from per-instance `.toFixed()`-baked GLSL → shader UNIFORMS + runtime branches (the metal/ACAH template); dropped the per-instance `customProgramCacheKey` (the default `onBeforeCompile.toString()` key shares within-factory + distinguishes factories; the fabric/skin `pbr` fork rides the base material class, so no key is needed) → **perf-probe programs 105→67 (−36%), boot shader-compile 270→197ms**; materials verified pixel-identical across both base-class paths (wood/glass/paint/fabric, pbr skin+fabric on the player, Lambert skin on the vulture). **Deferred:** T2 bury-audit (the 4 fails are procgen findPanelMount occlusion, not the assumed hand-modeled curved parts; a register-time skip desyncs the procgen RNG since `registerSalvageable` consumes `rand` — D208; reverted, real fix = register-all-then-prune, a ~3% cosmetic dev-audit tail); T4 flagship greebles (architecturally awkward, the procgen variants are intentionally distinct, T1 covers the intent). **Next session** = the still-owed human walk-tests (Sarlacc pull-feel, dropped-item settle, graveyard, mega-wreck interior) OR a buildable item (bury-audit register-all-then-prune; a feature: raider proc-character / deep cave / drop-pod intro).
