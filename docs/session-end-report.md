...
**Current state**: 🟢 **CAMPAIGN ACTIVE** (`campaign/2026-06-18`, autonomous, PHASE-level review). **Phase A (M1–M5b) shipped (cycles 1–36); Phase B (M6–M10) APPROVED + RELEASED 2026-06-20** and now building unattended. **M6 (Survival & UX) is COMPLETE** as of C40. Latest = **Campaign C40** (2026-06-20, cycle **40/75**) — **M6 ④ remove-hud-stat-bars → M6 COMPLETE**: `verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25), **diegetic-probe PASS**, **visual gate PASS in 3 rounds**, **no save bump**. The always-on HUD stat bars are replaced by DIEGETIC tells — per-stat screen-edge vignettes (cold=blue, thirst=sepia, heat=amber, hunger=sickly-green, low-health=blood-crimson PULSE) + procedural heartbeat/growl audio — behind `FEATURES.diegeticSurvival` (flipped ON, bars hidden by default) + a default-ON pause-menu opt-in (D248). **M6 shipped: ① crafting chooser (C37) · ② survival rebalance (C38) · ③ flat-color audit (C39) · ④ diegetic HUD (C40).** **Per-cycle detail for the C1–C40 campaign lives in `docs/campaign/campaign-log.md` + `docs/changelog.md`** (this report's session-named history below predates the campaign). The loop pauses next at the **Phase B milestone** (after M10). **Next** = cycle 41 = **M7 ⑤ procedural-wreck-overhaul** (net-new structure axes in the socket grammar; wider/weirder ships). **Owed**: the diegetic-HUD + survival-curve + stamina FELT walk-tests (backlog §A); **decisions.md archival is overdue** (60 entries).
