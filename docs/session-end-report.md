...
**Current state**: 🟢 **CAMPAIGN ACTIVE** (`campaign/2026-06-18`, autonomous, PHASE-level review). **Phase A (M1–M5b) shipped (cycles 1–36); Phase B (M6–M10) APPROVED + RELEASED 2026-06-20** and now building unattended. Latest = **Campaign C37** (2026-06-20, cycle **37/75**) — **M6 ① crafting-chooser-colliding-recipe**: `verify:all` PASS (tsc + placement 0/0 ×5 + colliders 0/25), adversarial visual gate PASS in 2 rounds, **no save bump**. Lit up the dormant multi-match crafting **chooser** (ACAS B3) via the first live recipe collision — a NEW **`signal_kit` (SIGNAL FLARE)** shares `fire_kit`'s `branch×3+scrap×1` recipe → "fire kit OR signal flare" ("warm yourself OR call out"). `signal_kit` is a one-shot transient flare (`world/signalFlare.ts`, additive sprites, never touches the fire list/save — D245). **Per-cycle detail for the C1–C37 campaign lives in `docs/campaign/campaign-log.md` + `docs/changelog.md`** (this report's session-named history below predates the campaign). The loop pauses next at the **Phase B milestone** (after M10). **Next** = cycle 38 = **M6 ② survival-rebalance-newgame** (KEYSTONE — flip `GOD_MODE` off + tune the forgiving Long-Dark curve).
