...
**Current state**: Session **ACAL** shipped (2026-06-07 — **mega-wreck walkthrough fixes: exact collision + natural light + entrance + de-floated interior**). `npm run verify` (tsc) PASS. SAVE_VERSION 14 (unchanged — geometry/material/collider only). 6 commits (`2ed662b`→`0aae9f9`); D189-D191. The user WALK-TESTED the ACAK mega-wreck and reported issues across two passes; ACAL fixed each + ran an adversarial **floater-hunt** (4 agents read the interior PNGs + geometry code → ranked floaters) to convergence over 3 rounds. **Headlines**: (1) **Collision = 100% EXACT trimesh (D189)**: the coarse hand-authored cuboid proxy (player clipped through) → ONE Rapier trimesh built from EVERY solid mesh in the wreck, baked into the body-local frame so it inherits the ~17° shell tilt — matches the rendered model triangle-for-triangle (flat decals skipped). Curbs + entrance ramp kept; panels/journal added AFTER → stay interactive. (2) **Natural light only (D190)**: removed ALL custom wreck lighting (god-ray cones + fracture flood + every fill/glow PointLight) + the unlit `MeshBasic` rust-streak decals that glowed pink in the dark; the lit hull-shader rust stays. (3) **Entrance + open fracture**: `makeLoftedHull` builds open-ended tubes → the fracture gap is open in the trimesh = the walkable way in (+ a lee-flank sand ramp); removed the `aBack`/`bBack` backboard walls so the split opens to the interior. (4) **Panels** reseated on the real exposed lee-flank hull surface (`hullAt` + terrain guard). (5) **Thicker hull + de-float (D191)**: `makeLoftedHull` gains `thickness` (inner skin + rim caps → ~0.4m plating, no paper edges); `makeFormerRings` gains `arc` (ribs → top-arcs springing from the deck, no belly hoops); ~16 floaters fixed via shared `ceilingY`/`hullHalfWAt` samplers (the bug class was a fixed Y at a cluster-center, never sampling the curved surface). **Still owed (honest)**: an in-app WALK-TEST RE-CONFIRM of the fixes (collision holds / entrance walks / panels reachable / interior brightness) + residual sub-0.6m floater nits. **Next session (ACAM)** = re-walk-test → interior material depth → the deferred ACAJ T3-T7 (procgen fleet on the bigger toolkit, now incl. `thickness`/`arc`).
