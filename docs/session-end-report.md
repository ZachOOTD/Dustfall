...
**Current state**: Session ACY shipped (2026-06-03 — item-model detail pass + dynamic salvage-panel placement + POI greebles). 100+ sessions post-MVP. `npm run verify` (tsc) PASS. **SAVE_VERSION 14** (unchanged — all ACY changes are mesh/material/placement, additive at most). ACY was a long overnight on headless-verifiable visual lanes (the owed ACW/ACX feel work is foreground-only): **deep-polished all 12 high-visibility held item models** against a new isolated `itemStudio` view (D169); a **dynamic salvage-panel placer** `findPanelMount` (raycast surface-sampler replacing the hardcoded `panelAnchor`, kept as fallback — D168) + size variants + a numeric bury audit (75/77+ pass across seeds); POI hull **greebles**; and made the **amban a `massive`-wreck loot drop**. **Next session (ACZ)** = pick a lane: deep-cave design+build / finish the remaining ~22 item models / foreground feel-tune playtest (default: finish the item models).

## ACY scope (this session) — item-model detail pass (12 held items) + dynamic salvage-panel placement + POI greebles
