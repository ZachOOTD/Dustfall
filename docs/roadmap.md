...
Continue P7 procgen wreck expansion (more variants, more wreck classes, biome bias on recipes) OR pick from the big-ticket bucket: infinite chunk streaming, trading/NPC economy, POI narrative beats, biome-specific POI kinds, salvage thermal-cut tier (fire-starter as alternative pry tool). Multiplayer + player model + character customization were added to backlog as long-horizon items.
