...
Infinite chunk streaming (Minecraft-style) remains the queued big-ticket lift. With sandworm + music both shipped, the remaining big-ticket items are: trading/NPC economy (needs design pass), POI overhaul (themed clusters), salvage mechanics overhaul. Medium picks: multi-worm population (deferred from AAP scope), tutorial coverage gaps, scrap_gun reload action, satellite dish backing geometry.
