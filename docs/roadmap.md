...
ABT fixed the 3 user-reported issues (camera too high/far, feet under
sand, head needs polish). Rig + camera now reads at modern over-the-
shoulder game quality. **ABU continues the realism push** — deferred
items remain from ABS+ABT plans:
**Polish / quick wins** (~30-90min each, mostly `src/player/playerRig.ts`):
- **P1 ABU: realistic cloth drape (3-5 rounds)** — current poncho
  ...
  HIGH-IMPACT visual lift remaining.
- **P2 ABU: rig sub-pivots (1-2 rounds, code-heavier)** — add wrist
...
- **P3 ABU: body model polish refinements** — shoulder-arm transition
  smoothness (gap visible where upper arm meets torso), neck cap blend
  (Lathe neck-base creates small visible lip), hand-wrist joint, finger
  knuckle inflections.
- **P4 ABU: walk-cycle to footstep cadence sync** (ABR backlog).
- **P5 ABU: per-item viewmodel readability at 3P** (ABR backlog).
