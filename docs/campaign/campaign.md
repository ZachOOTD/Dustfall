...
- **M6 — POI breadth** `[gate-verified]` — DoD MET (cycle 5 A1 `relay_mast` D285; cycle 6 A2 `buried_pipeline` D286 — 2 distinct archetypes; A3 cargo_crawler = optional stretch, cycle 7) — 2-3 new procgen wreck/POI archetypes via the `src/world/poiComponents.ts` socket grammar (+ `poiArchetypes.ts` biome weighting). Verify: `verify:placement` (0 occlusion/terrain fails ×5 seeds) + `verify:colliders`.
