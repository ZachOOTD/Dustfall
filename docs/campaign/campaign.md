...
- **M6 — POI breadth** `[gate-verified]` — IN PROGRESS (cycle 5: A1 `relay_mast` shipped, D285; sub-plan `docs/feature-poi-archetypes.md`; A2 buried_pipeline + A3 cargo_crawler[cuttable] queued) — 2-3 new procgen wreck/POI archetypes via the `src/world/poiComponents.ts` socket grammar (+ `poiArchetypes.ts` biome weighting). Verify: `verify:placement` (0 occlusion/terrain fails ×5 seeds) + `verify:colliders`.
