...
[feat] salvage mechanics overhaul — AAR shipped tactile pry + extract; AAS shipped variant interiors + per-component loot mapping + electrical glow. REMAINING: (1) condition tiers (corroded panel: easier pry, fewer components / pristine: harder pry, premium loot); (2) thermal-cut tier with fire-starter for sealed/heavier panels (alternative to scrap_bar — fire-starter melts the seal); (3) rare key-card panels gated behind a quest item; (4) salvage durability per-wreck (each wreck has finite mass; deplete completely → "corpse" wreck with no panels at all). Core salvage loop fully replaced + iterated; remaining items are depth additions.
