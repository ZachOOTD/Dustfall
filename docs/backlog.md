...
[polish] paper-thin decoration audit — AAM-followup #8 fixed opening wreck (cockpit windows + breach patches + entrance fragments + salvage panels — all bumped to ≥10cm depth, matching the new CLAUDE.md rule 7). AAN fixed wrecks.ts (engine_cluster panel, escape pod hatch + rust, cargo container door, debris hull plate, fuselage end cap CircleGeometry → CylinderGeometry). Remaining flagship POIs: megaShip.ts, megaWreck.ts, crashedHull.ts, engineBlock.ts, satelliteDish.ts — find BoxGeometry decorations on hull surfaces with depth < 0.10m and bump them.
[polish] cook-progress-per-fire HUD — AAM ships parallel cooks on grilled fires (up to 4) but the salvage-progress bar UI only shows one cook's progress. Add per-cook progress indicators above/around the fire so the player can see each cook's timer.
...
[feat] companion storm-peak behavior — companion currently behaves identically through any storm intensity. Add a "huddle" state at weather.intensity > 0.8 (legs tucked, body pressed to ground, no wandering) + a one-shot toast "Rocky huddles down" on first storm peak.
