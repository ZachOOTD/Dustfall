...
[feat] salvage mechanics overhaul — AAR/AAS/AAT shipped tactile pry + extract + variant interiors + per-component loot + electrical glow + condition tiers (D96). REMAINING: (1) thermal-cut tier with fire-starter for sealed/heavier panels (alternative to scrap_bar — fire-starter melts the seal); (2) rare key-card panels gated behind a quest item; (3) salvage durability per-wreck (each wreck has finite mass across all panels; deplete completely → "corpse" wreck dimmed entirely); (4) player ability to "restore" corroded panels to standard via a weld kit (would invert D96 — condition becomes player-mutable, must move to save). Core loop is fully iterated; remaining items add deeper progression beats.
