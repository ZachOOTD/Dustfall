...
**Still PENDING:** [debt] **Tier 5 yard cross-POI merge** (D237/D239 — ATTEMPTED + REVERTED: the rim-greeble merge cut 3215→2633 ~18% but regressed the terrain bury-audit; re-attempt WITHOUT perturbing the panelDoorExtents bottom-edge measure — see D239 leads; the rig-shot wreck-yard scenario now reports drawCalls) · [polish] the reworked **scrap pickup reads thin from a 3q edge-on angle** (re-judge in-hand; the front view reads well) · [bug] the flagship dish-collider FEEL owes the attended walk-test (slab approximation).
