...
[polish] **corvette/gunship forced-trauma READ** (campaign C9, `addForcedTrauma` in procgenWreck.ts): guaranteed + determinism-safe (black-void gash + peeled flap + lip) but on some seeds reads as **scattered debris near the wreck** rather than a clean attached **main-hull** breach — the part-pick takes any ≥0.7 body part (may be a forward/minor one). Polish: target the LARGEST/most-central body part + seat the gash+flap tight to that part's surface so it reads as an in-hull breach; consider an all-angle silhouette break that stays attached (the C9 top-plate attempt read as detached debris). Judge in the walk-test.
