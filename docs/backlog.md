...
[debt] lift `alignMeshToTerrain` to shared util — currently duplicated across `tent.ts`, `largeTent.ts`, and `companion.ts` (AAY). Three copies is acceptable but a fourth caller should trigger extraction (see D98). Candidate path: `src/util/terrainAlign.ts` exporting `alignToTerrain(obj, terrain, pos, yaw, sampleRadius)`.
