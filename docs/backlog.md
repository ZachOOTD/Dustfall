...
- `[debt]` **Finish material-factory → uniforms** (wood/fabric/skin/bone/glass/paint/stone, like metal got in ACAH) to collapse ~100 compiled shader programs → faster real-GPU startup. Each: bake `.toFixed()` consts → uniforms + a `uLocalSpace` branch, drop `customProgramCacheKey`. Highest-instance first (wood, skin).
- `[polish]` **Flagship + procgen greebles (W2) + brighter-lit procgen pass (W5).** Greebles on the hand-modeled flagships (megaShip/satelliteDish/crashedHull) + the boxy/truss/barrel procgen variants; the procgen hulls read fairly dark, so a brighter-lit pass would surface the detail. (Ribbed/plated variants already greebled ACAO.)
- `[debt]` **Curved-surface panels still flag the bury-audit** — hand-modeled `engine_bell`/`escape_pod` panels (`panelAnchor:null`) aren't touched by `findPanelMount`; extend the sampler to curved parts if wanted.
