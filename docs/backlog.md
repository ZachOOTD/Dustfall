...
- `[debt]` **Salvage-panel overhaul — VISUAL half remaining (ACAV Tiers 3-5).** Placement half (Tiers 0-2) shipped ACAV. Remaining: T3 shape/size variants (square + bolted lift-off circular port), T4 the 5-archetype scrappy interior overhaul (pipes/fuses/machinery/wires/rust; decorative greeble + keep 5 lootable; build a `panel-studio` harness + deeply iterate all 5), T5 verification hardening (`placement-torture` + `flagship-audit` + `verify:placement` + a surface-scoped terrain audit + the "new wreck class must pass" contract). Full plan in `.claude/plans/`.
