...
- [x] **A3 — cargo_crawler** (cycle 7, D287) ✅ — a tracked desert hauler wreck: `crawlerBody` (cab + exhaust stack + open cargo bed + two track bogies with road wheels, end drums, tread lugs) + 1-2 spilled cargo containers (reused `debrisPiece`). The bulky-ground-vehicle silhouette (nothing else in the set is a vehicle). Cab/bed/2-bogie box colliders (audit 5-6/5-6 ×4 seeds). Visual-gated: round-2 fix dropped `seatSink` 0.45→0.16 so the TRACKS stay visible (the defining feature) — the tread/wheels/drum now read clearly broadside + natural-yaw. `warm` bucket, favors rocky/wreck_yard.

## Status: DONE — M6 shipped 3 distinct archetypes (relay_mast + buried_pipeline + cargo_crawler), exceeding the 2-3 DoD. Next milestone: M7 Skyfall (`[feel-critical]` — pauses for human plan-review).
