...
- **K** — FPS diagnostics + shadow toggle + sandstorm gate. F1 HUD gains `GPU ms` / `CPU ms` / `frame` ms rows via `EXT_disjoint_timer_query_webgl2`; SW-render warning automatically flags WARP/SwiftShader/Microsoft Basic Render. Shadow on/off setting (`sun.castShadow`) live-applies. Pickups + branches no longer cast shadows. Sandstorm Points hidden when intensity ≤ 0.01.
- **L** — Tutorial & first-time UX. `src/ui/tutorial.ts` owns: first-boot controls panel (14 keybind rows), H-key shortcut to reopen it in-game (unlock→pause→panel at z=200), and per-item pickup hints (canteen/branch/cloth/scrap/bandage/machete/fire_kit/tent_kit/cactus_pulp/raw_lizard_meat — each toast fires once across sessions). State persists in localStorage `dustfall.tutorial.v1`; debug `__game.resetTutorial()` + `__game.showControls()`.
