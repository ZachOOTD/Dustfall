...
**Last shipped — campaign "Sharpen & Deepen" cycles 1-4 (2026-07-09, autonomous, branch `campaign/2026-07-09`)** — **M1** pickup instancing (D281) + housekeeping; **M2** verify-only (pre-built C38); **M3** shade coverage 3→51 + shade-relieves-thirst (D282, C38 bands byte-identical); **M4** procedural ambient life beds on the silent stems (D283, wind stays muted); **M5** diurnal-cycle (D284): lizards diurnal / shrews crepuscular / vultures roost at night via one pure activity function — worms already twilight-bound (verified). Permanent gate suite grew to 7 (verify:all · 2 smokes · pickup-take-sweep · survival-probe · ambient-beds · diurnal-probe) — all green every cycle. **Campaign ACTIVE** — state in `docs/campaign/`; next cycle = M6 (POI breadth: new procgen archetypes).
