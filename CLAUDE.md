...
**🚀 SHIPPED (2026-07-19) — campaign "THE DEEP DESERT" MERGED to `master` and LIVE.** The world has **mega dune-sea (erg) regions**: rare multi-km ergs (nearest ~5-10km by seed; `__game.gotoErg()` / the dev-panel "dune sea" button), 45-58m dunes at 220-320m wavelength with walkable windward faces + 30-34° slip faces, aeolian surface read (aErgMask suppresses the hardpan cracks), crest spindrift (noise-masked streaks, near-camera fade, weather-scaled, dark at night), sparse trough finds (shard/bone/dead-tree on true trough floors), the erg soundscape hush, and a one-time first-crest discovery beat — permanent `dune-slope` gate ×2 seeds. **The scrap sled is RIDABLE** (`FEATURES.rideableSled` default ON; `VITE_RIDEABLE_SLED=0` kill-switch): D257 kinematic seat-pin (no KCC fight), E mount / JUMP dismount, kneeling pose, gravity descent ~2.4× sprint with sub-stepped surface follow, carve steering (A=left — the inversion Zach caught; the carve gate now asserts DIRECTION), uphill stall, sand kick-up + speed-FOV + sand-hiss, **RMB pack-up** back into sled_kit (guards + save/load-proven teardown; `sled-ride`/`sled-dune`/`sled-packup` probes). Review tooling: `dustfall-sled` launch config, "dune sea"/"spawn sled" dev buttons. D-entries + research: `docs/research/dune-field-synthesis.md`.

**🚀 SHIPPED (2026-07-20) — campaign "UNDERWORLD" MERGED to `master` and LIVE.** The cave system replaces the D255 single chamber. Architecture per **D307** (the attended spike: the Rapier heightfield is TWO-SIDED for the KCC — no ejection below the sheet, full control under it, impassable from either side): **trimesh cave interiors under the intact terrain + a chunk-local heightfield→trimesh collider swap at the entrance chunk with a genuinely carved hole** (`src/world/caveTest.ts` owns the entrance weld + hole block; `terrain.ts` does the swap). NO portals. `src/world/caveGen.ts` = a seed-pure room-graph generator: 8-11 chambers on a branching TREE (no maze loops), squeeze corridors → galleries → a great hall, the EGG chamber deepest+largest with a walkable rock dais; multi-octave displaced rock (strata + mineral staining, sediment pooling in floor dips), stalactites/stalagmites/columns, a rubble-collar sinkhole mouth. `caveAtmosphere.ts` = TRUE darkness (ambient ×0.03, sun ×0 past the throat; **no free light** — torch/flashlight/lantern are survival gear), near-black cave fog lerped from live weather fog, and a sun-tracking mouth light shaft. Faint teal **bioluminescent mushrooms** (harvestable → alien_fruit) + cave audio (desert bed ducks out; stone hush + delay-echoed drips). **The companion EGG now spawns on the cave dais**; the legacy funnel `deepCave` is retired behind the same flag. Ships ON: `FEATURES.caveTest` default true, **`VITE_CAVE=0` kill-switch**; one cave per world 130-260m from origin (findability verified ×5 seeds). Permanent gates: **`cave-walk` seeds 1337+7 in `verify:chunks`** (full-tree KCC Euler-tour march: every chamber reached + ascent out). Two generator defects found by widening the seed net and fixed BY CONSTRUCTION: sibling-corridor crossing (min 55° divergence) and the chamber-disk/corridor-ramp step (flat floors across chamber disks + clear-span run sizing) + a fail-loud dev assert so an untraversable cave can never ship silently. 6/6 seeds traversable.

**PROCESS — the SPEED RULES (2026-07-20, Zach: a greybox took 5h, "should be 20 minutes").** Root causes: full-boot probes re-run per tweak, fresh-port permission prompts blocking unattended, agents parking in watcher-wait patterns. Standing rules (also in `docs/campaign/steering.md`): probes ONLY via **`npm run rig -- --scenario=… --port=52xx`** (matches the npm-run allow rule → zero prompts; never raw `node scripts/…`); the full-boot gate runs at most ONCE per iteration BATCH; **no monitor/wait/watcher patterns** — run synchronously, then report; state a wall-clock budget per agent. Result: the hero-geometry pass took **26 minutes**.

**NEXT:** Zach's cave walk-test surfaced problems (feedback pending as of the 07-20 ship). Then: character/MP (digests in `docs/research/`; needs the GDD multiplayer re-anchor). Owed: economy feel pass (drop rates/recipe costs — data edits), Deep-Desert taste tweaks (music duck, trough-find rarity, shard collision, sled feel — all `tuning.ts`), cave taste tweaks (darkness depth, torch range, mushroom glow, drip rate — all `CAVE_*`), audio listen session (C1, attended), `docs/backlog.md` §A.
