...
**⚙ ACTIVE CAMPAIGN — "Sharpen & Deepen" at M7 Skyfall** (branch `campaign/2026-07-12-skyfall`, cycle 10 done, ~3.9M/10M tokens). The enterable hero freighter ships as a rare **S4 far-field landmark kind** (`skyfall_freighter`, 34% of landmark regions, `FEATURES.skyfall` kill-switch), ~46m heavy freighter, interior bar = INTRO-SHIP detail in the wrecked art style, **no sand mounds** (steering). **S1 SHIPPED** (D300, exterior blockout). **S2 SHIPPED** (cycle 10, D302): `src/world/skyfallWreck.ts` opens into a walkable greybox interior (3 compartments + doorways/sills, exact collider set), gate-proven by the NEW permanent `skyfall-walk` leg. **Probe infra (D301): GPU headless is now the rig-shot default — ~10× faster + CPU-cool** (`RIG_GL=swiftshader` to revert). **NEXT: S3-S5 hero detail** (exterior plating/weathering + interior to intro-ship detail in the wrecked style + lighting + the Skyfall landing-fire fix), then **S6 integration+loot**. The charter's ⏸ post-blockout human walk-test is deferred to the morning review (user authorized an overnight "finish it" run 2026-07-13). Campaign detail: `docs/campaign/`; owed feel-tests: `docs/backlog.md` §A.
