...
**Last shipped — cockpit-glass playtest fixes (2026-07-08, attended, round ACN)** — sealed + cleaned the escape-pod-intro cockpit dome glass (all in `shipScene.ts`, one commit `07a818d`, D279-D280): removed a floating hazard placard; the **side-closure glass now connects to the hull** (collar-side edge lands on the collar/shell ring with a small overlap, DoubleSide so it never back-face-culls to a hole); added a **crown ROOF CAP** (the dome was open above the crown ring — panes now loft to the shell forward ring, framed by new roof ribs); and **all "wrap" glass (side closures + roof) uses a flat unlit tint** (`_glassRoof`) so it reads uniform — the sheen-tuned `_glass` is now front-window-only, killing the view-dependent bright-triangle artifacts. tsc clean; render-verified via `ship-shot.mjs` from seated/back-left/look-up (the preview MCP wedges on this scene).

**NEXT:** genuinely-open backlog is thin (most 2026-06 triage items shipped — verify against code, the backlog is stale). Verified-open: pickup-instancing (perf, no `InstancedMesh` yet), day-bird/night-insect ambient life beds (still silent). Bigger net-new: an endgame goal arc (Long-Storm finale), a new enterable hero wreck (Skyfall), the cave multi-chamber expansion, desktop follow-ups (code signing / file saves / CSP tighten). Boot from [docs/next-session-prompt.md](docs/next-session-prompt.md).
