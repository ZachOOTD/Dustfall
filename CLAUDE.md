...
**Last shipped — campaign "Sharpen & Deepen" cycle 1 (2026-07-09, autonomous, branch `campaign/2026-07-09`)** — **M1 perf + housekeeping**: seed-spawned branch/scrap pickups now render via shared **InstancedMesh pools** (D281 — `intersection.instanceId → instanceToPickupId` resolver in interaction.ts, swap-with-last slot frees, legacy-mesh overflow fallback, seeded rand-order preserved; NEW permanent `pickup-take-sweep` rig-shot gate proves E-takes end-to-end incl. the swap-fixup case); decisions.md archival (D221–D235 → archive, count conserved); panel dead-code sweep (backlog was stale — mostly pre-shipped; deleted `clearPanelDebris`); `survivalProbe` crash-heat determinism guard; scrubbed the forbidden endgame-finale candidates from 3 docs. **M2 discovery**: the survival curve was ALREADY built + probe-pinned (C38/D246 — heat 7.54/cold 8.67/thirst 10/hunger 15 min; re-verified green) → M2 is verify-only-done; the FEEL walk-test stays a human end-review item. Gates: `verify:all` + both intro smokes + take-sweep + survival-probe, all PASS. **A campaign is ACTIVE** — state in `docs/campaign/`; next cycle = M3 (sun-shade occluder decouple + water-scarcity).
