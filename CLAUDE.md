...
**Last shipped — campaign "Sharpen & Deepen" cycles 1-6 (2026-07-09, autonomous, branch `campaign/2026-07-09`)** — **M1** pickup instancing (D281); **M2** verify-only (pre-built C38); **M3** shade coverage + shade-thirst (D282); **M4** procedural ambient beds (D283); **M5** diurnal-cycle (D284); **M6 [2 of 2-3, DoD MET]** POI breadth — `relay_mast` (D285: fallen lattice comms tower, the TALL+THIN silhouette) + `buried_pipeline` (D286: a bedded pipe run + manifold hub, the LOW-HORIZONTAL silhouette); both wired into all biomes, envelope-cylinder collision, visual-gated. Permanent gate suite = 7 (verify:all + smoke-intro/pod + pickup-take-sweep + survival-probe + ambient-beds + diurnal-probe), all green every cycle; collider audit now 50 (both new archetypes covered). **Campaign ACTIVE** — state in `docs/campaign/`; next cycle = M6 A3 (cargo_crawler, optional stretch) then M7 Skyfall [feel-critical] PAUSES for human plan-review.
