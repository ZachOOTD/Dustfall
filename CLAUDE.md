...
**⚙ ACTIVE CAMPAIGN — "Sharpen & Deepen" RESUMED at M7 Skyfall** (2026-07-12, branch `campaign/2026-07-12-skyfall`, cycle 9 done, ~3.55M/10M tokens). Plan approved with changes: the enterable hero freighter ships as a rare **S4 far-field landmark kind** (`skyfall_freighter`, 34% of landmark regions, `FEATURES.skyfall` kill-switch), ~30m+ heavy freighter, interior bar = INTRO-SHIP detail in the wrecked art style (steering), **no sand mounds** (steering). **Cycle 9 / M7-S1 SHIPPED** (D300): `src/world/skyfallWreck.ts` exterior blockout iterated 4 rounds in the real streamed view; fast `skyfall-shot` rig scenario; all gates green. **NEXT: cycle 10 / M7-S2** — enterable greybox interior + colliders + the `skyfall-walk` real-motion probe, then the **⏸ post-blockout human walk-test pause** (charter pause #1). Campaign detail: `docs/campaign/`; owed feel-tests: `docs/backlog.md` §A.
