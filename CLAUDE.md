...
**Last shipped — campaign "Infinite Sands" cycles 1-4 / S1-S4 (2026-07-11, autonomous, branch `campaign/2026-07-10-procgen`)** — **the world STREAMS, is INHABITED, and has DESTINATIONS.** S1: anchor-margin terrain tile ring + `src/world/chunkManager.ts` (112m chunks, pure descriptors) + the permanent `verify:chunks` gates. S2: streamed POI wrecks beyond 1250m (save-TRANSIENT per D292 until S5's per-chunk diffs). S3: rock scatter + rare wordless scenes + chunk-keyed prey clusters (vultures deferred, D294). S4: region-grid (1792m) hero landmarks — `colossal_ribcage` (5-8× titan skeleton) + `wreck_knot` (3-wreck salvage triangle; Skyfall's future slot) — and regional wreck-yard biome anchors (≥2200m from origin) feeding the SAME `wreckYardAt` → far graveyards get ground/biome/POI-mix/6×-density for free (D295). All 10 gates green every cycle; the released origin world bakes byte-identically; boot rand streams sacred. D288–D295. (Mid-cycle-4: the machine-slowdown investigation shipped the framework-wide `reap-orphans` fix — see `gamedev-framework/shared-memory/process-leak-hygiene.md`.)

**NEXT:** campaign "Infinite Sands" cycle 5 = **S6 — perf: frame-budgeted generation**: spread the ~100-200ms terrain-tile bake across frames (the known D288 hitch at ring crossings); draw-call/body ceilings for the active set; extend the perf probe to a cross-chunk walk asserting steady counts. Then ⏸ S5 save (the ONE sanctioned pause). Boot from [docs/next-session-prompt.md](docs/next-session-prompt.md); campaign state in `docs/campaign/`.
