...
**Last shipped — campaign "Infinite Sands" cycles 1-2 / S1+S2 (2026-07-11, autonomous, branch `campaign/2026-07-10-procgen`)** — **the world STREAMS, wrecks included.** S1: terrain converted to an anchor-margin tile ring following the player (full disposal, `heightAt` infinite); NEW `src/world/chunkManager.ts` (112m chunks, r3, pure `describeChunk` descriptors — determinism law extended to the infinite grid); 2 NEW permanent gates (`verify:chunks` = chunk-determinism + a streaming/leak walk past ±1500m). S2: descriptor-rolled POI wrecks stream beyond a 1250m origin exclusion (0.07/chunk ≈ origin density, biome-weighted archetypes, real `placeProcgenPOI` render, full teardown incl. salvage splice-out); streamed wrecks are save-TRANSIENT (excluded from serialization, regenerate pristine — per-chunk diffs land at S5) and the streaming gate saves at +1500m to prove the file stays boot-only. All 10 gates green both cycles; the released intro region byte-identical. D288–D293.

**NEXT:** campaign "Infinite Sands" cycle 3 = **S3 — scatter + ambient life streaming**: rocks + wordless scenes per-chunk on the ChunkManager lifecycle; creatures via an active ring around the player (FIRST verify how `spawn*Procgen` works — the feature slice's open ❓). Then S4 landmarks/biomes → S6 perf → ⏸ S5 save (the ONE sanctioned pause). Boot from [docs/next-session-prompt.md](docs/next-session-prompt.md); campaign state in `docs/campaign/`.
