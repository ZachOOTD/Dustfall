...
**Last shipped — campaign "Infinite Sands" cycle 1 / S1 (2026-07-11, autonomous, branch `campaign/2026-07-10-procgen`)** — **the world STREAMS**: `terrain.ts` converted from a fixed 3×3×800m boot grid to an anchor-margin tile ring following the player (tiles fully disposed outside it; `heightAt` answers at ANY coordinate via a closed-form fallback; ONE shared terrain material — per-tile materials leaked `_shaderRefs`); NEW `src/world/chunkManager.ts` (112m content chunks, load radius 3, `chunkSeed(worldSeed,cx,cz)` avalanche, pure `describeChunk` descriptors — the determinism law extended to the infinite grid, full Rapier/mesh teardown, marker-post spike content off by default); 2 NEW permanent gates in `verify:all` (`verify:chunks` = chunk-determinism + a chunk-streaming/leak walk past ±1500m); rig-shot hardened (startDev 120s, walk probes at 320×240). All 10 gates green; placement stayed 0-fails ×5 seeds — the released intro region is byte-identical. D288–D291.

**NEXT:** campaign "Infinite Sands" cycle 2 = **S2 — POI streaming**: `placeProcgenPOIs`/`placeProcgenPOI` become per-chunk on the ChunkManager lifecycle via the descriptor model (D290) — biome weights, collider audit, static merge, salvage/journal registration per chunk, full teardown on unload, origin-chunk spawn exclusion. The S5 save-schema rung is the ONE sanctioned pause. Skyfall stays parked (resumes as an S4 landmark). Boot from [docs/next-session-prompt.md](docs/next-session-prompt.md); campaign state in `docs/campaign/`.
