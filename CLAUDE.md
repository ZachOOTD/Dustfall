...
**NEXT: the queue + the human walk-test.** Queue: #28 Skyfall stern seam (hairline gap at the fracture rim — likely the SHARED `solidInner` lip weld, so fix in `wreckForms.ts`, not a stern patch; no gate catches inter-mesh sliver gaps) · #29 boneyard scatter overhaul (still on the old lighter `_boneMat` + ring-y `placeRibcage`; bring to hero quality + add a day/night render check — 2 lighting-invariant bugs found with no gate covering that class). Then plan the next campaign from `docs/campaign/audit-and-roadmap-2026-07-15.md` (Phase 2 "Scavenger's Economy" → mega dunes + ridable sled → cave system → character/MP, which needs the GDD multiplayer contradiction resolved first). Owed feel-tests: `docs/backlog.md` §A + `docs/campaign/STATE-2026-07-16.md`.
