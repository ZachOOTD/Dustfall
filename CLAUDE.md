...
**NEXT:** the "UNDERWORLD" cave campaign — the D254 KCC-under-heightfield spike ran attended 2026-07-19 (see `docs/campaign/`); build per its outcome. Then character/MP (digests in `docs/research/`; needs the GDD multiplayer re-anchor). Owed: economy feel pass (drop rates/recipe costs — data edits), Deep-Desert taste tweaks (music duck, trough-find rarity, shard collision, sled feel — all `tuning.ts`), audio listen session (C1, attended), `docs/backlog.md` §A. Desktop exe stale — rebuild when wanted.
