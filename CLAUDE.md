...
**Last shipped — campaign "Sharpen & Deepen" cycles 1-5 (2026-07-09, autonomous, branch `campaign/2026-07-09`)** — **M1** pickup instancing (D281); **M2** verify-only (pre-built C38); **M3** shade coverage + shade-thirst (D282); **M4** procedural ambient beds (D283); **M5** diurnal-cycle (D284); **M6 [1 of 2-3]** POI breadth — the `relay_mast` archetype (D285): a fallen guyed lattice comms tower, the missing TALL+THIN silhouette; `latticeMast` component, envelope-cylinder collision, wired into all biomes; visual-gated. Permanent gate suite = 7 (verify:all + smoke-intro/pod + pickup-take-sweep + survival-probe + ambient-beds + diurnal-probe), all green every cycle; collider audit now 45 (relay_mast covered). **Campaign ACTIVE** — state in `docs/campaign/`; M6 is a multi-archetype slice (`docs/feature-poi-archetypes.md`) — next cycle = M6 A2 (buried_pipeline).
