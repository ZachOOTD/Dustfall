...
**🏁 M7 SKYFALL COMPLETE — campaign "Sharpen & Deepen" PAUSED at the human walk-test** (branch `campaign/2026-07-12-skyfall`, cycles 9-13 done, ~4.75M/10M tokens; `awaiting_approval`, `stop_reasons:["feel-review"]`). The enterable hero freighter is a rare **S4 far-field landmark kind** (`skyfall_freighter`, 34% of landmark regions, `FEATURES.skyfall`/`VITE_SKYFALL=0` kill-switch): a ~46m crashed heavy freighter with a hero-detailed exterior (multicoloured cargo containers, plated hull, greebles, warm-rust weathering) AND a walkable hero-detailed interior at intro-ship density in the wrecked style (cargo hold / dead-console machine bay / crew cabin, "sun through the tear" lighting) + 2 persistent salvage panels + the pilot's crash-log journal. `src/world/skyfallWreck.ts` (~1000 lines). Ladder: S1 blockout (D300) → S2 enterable + `skyfall-walk` gate (D302) → S3 exterior hero → S4-S5 interior hero → S6 loot (D303). **Probe infra (D301): GPU headless is now the rig-shot default — ~10× faster + CPU-cool** (`RIG_GL=swiftshader` reverts). **NEXT: the human post-blockout WALK-TEST** (feel/scale/lighting — see `docs/campaign/morning-summary-2026-07-13.md`; likely first tweak = cabin lighting), then `/campaign-approve` → merge review (nothing pushed/merged yet). Owed feel-tests: `docs/backlog.md` §A.
