...
**Last shipped — crafting rework + desktop packaging (2026-07-08, attended)** — (1) **Crafting is now PICKUP-GATED** (D277): a recipe unlocks once you've collected all its ingredient TYPES (not by guessing an input combo); `collectedItemTypes` ledger, SAVE_VERSION **16** (additive + pre-v16 seed migration), a category CARD-GRID UI replacing the input slots + the chooser + the TAB recipe book (deleted `recipeBookPanel.ts`; net −723 LOC). (2) **Tauri desktop packaging** (D278, above). tsc clean; NEW `craft-unlock` gate + `smoke-pod-tutorial` pass; save migration verified.
