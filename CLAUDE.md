...
**Last shipped — campaign "Sharpen & Deepen" cycles 1-7 (2026-07-09, autonomous, branch `campaign/2026-07-09`)** — **M1** pickup instancing (D281); **M2** verify-only (pre-built C38); **M3** shade coverage + shade-thirst (D282); **M4** procedural ambient beds (D283); **M5** diurnal-cycle (D284); **M6 COMPLETE (3 archetypes)** POI breadth — `relay_mast` (D285, vertical lattice tower), `buried_pipeline` (D286, horizontal bedded pipe run + manifold), `cargo_crawler` (D287, tracked hauler); all wired into every biome, collider-audited (55), visual-gated. Permanent gate suite = 7, all green every cycle. **Campaign PAUSED at M7 plan-review** — Skyfall (`[feel-critical]`) feature-sliced into `docs/feature-skyfall.md` (cycle 8); awaiting human review + `/campaign-approve`. Confirmed: a new enterable hero wreck is ADDITIVE (no SAVE_VERSION bump). Plan: research+exterior blockout → enterable interior+colliders (+ a new walk-probe) → walk-test pause → hero-detail. State in `docs/campaign/`.
