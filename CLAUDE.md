...
**Last shipped — campaign "Sharpen & Deepen" cycles 1-3 (2026-07-09, autonomous, branch `campaign/2026-07-09`)** — **C1/M1**: pickup instancing (D281, NEW `pickup-take-sweep` gate) + housekeeping; M2 found already-built → verify-only. **C2/M3 (D282)**: sun-occluder decouple (2.5m) + POI registration (occluders 3→51); shade relieves thirst (`THIRST_SHADE_RELIEF`); C38 bands byte-identical; NEW `heat-shade` probe case. **C3/M4 (D283)**: procedural ambient life beds — the silent day/night stems now have synthesized sources (sparse crickets at night, shimmer + distant cries by day) reusing the whole mixer; wind stays muted; NEW `ambient-beds` gate (crossfade + storm-duck + wind-mute asserted). All gates green every cycle. **A campaign is ACTIVE** — state in `docs/campaign/`; next cycle = M5 (diurnal creature activity).
