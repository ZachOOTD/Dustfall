...
**NEXT:** campaign "Infinite Sands" cycle 6 = **⏸ S5 — save: per-chunk diffs (THE SANCTIONED PAUSE)**: write the SAVE_VERSION schema plan (per-chunk diffs over the deterministic base — which diffs, migration story, id strategy per D292/D296), set `awaiting_approval`, and STOP for the human review (D81 — never ship an unreviewed migration). Boot from [docs/next-session-prompt.md](docs/next-session-prompt.md); campaign state in `docs/campaign/`.
