...
**Last shipped — campaign "Infinite Sands" cycles 1-3 / S1-S3 (2026-07-11, autonomous, branch `campaign/2026-07-10-procgen`)** — **the world STREAMS and the far field is INHABITED.** S1: terrain as an anchor-margin tile ring (full disposal, `heightAt` infinite) + `src/world/chunkManager.ts` (112m chunks, pure `describeChunk` descriptors) + 2 permanent gates (`verify:chunks`). S2: descriptor-rolled POI wrecks beyond a 1250m origin exclusion (real `placeProcgenPOI` render, full teardown, save-TRANSIENT per D292 — regenerate pristine until S5's per-chunk diffs; the streaming gate saves mid-walk to prove the file stays boot-only). S3: rocky-biome rock scatter + rare wordless scenes (0.02/chunk) + chunk-keyed prey clusters at streamed wrecks (real `spawnLizard`/`spawnShrew`, transient, despawned with their chunk; vultures deferred — D294). All 10 gates green every cycle; the released intro region byte-identical; boot rand streams untouched (sacred — boot creature ids are save-coupled). D288–D294.

**NEXT:** campaign "Infinite Sands" cycle 4 = **S4 — distributed rare landmarks + per-region biomes**: a rare per-region roll scatters hero destinations across the infinite field (coarse landmark grid / Poisson-disk; Skyfall plugs in here later); re-anchor the distance-override biomes (wreck_yard 620-1000m origin ring) per-region. Then S6 perf → ⏸ S5 save (the ONE sanctioned pause). Boot from [docs/next-session-prompt.md](docs/next-session-prompt.md); campaign state in `docs/campaign/`.
