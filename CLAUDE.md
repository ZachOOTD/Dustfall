...
**Last shipped — live-feedback round ABQ (2026-07-07, attended)** — a long attended fix batch across the ship cockpit + pod interior + audio + airlock, all probe-/render-verified (tsc clean; commits `f17fce6` console rework + the ABQ batch). **Cockpit console** rebuilt as 3 straight angular panels chorded off the dome sill (mitred deck/folded fascia, proud instruments) + moved closer to the glass (`INSET 0.24`, clutter re-anchored to track it). **Pod interior**: the "brown floor" was a stray model — the charred `baseCap` poked 18cm above the deck (dropped below); white splotches = shader flecks (zeroed interior+exterior); floor z-fight flicker (deck/sub-floor separated 1cm) + streak layers stripped; the footwell "circular thing" removed; eject lever → a simple pull-down handle. **Rounded airlock → plain hallway** (bellows sleeve + mating shroud removed, which also cleared the pre-eject porthole bands); pod door confirmed ONE unified model. **Audio**: desert wind + ship hum muted (reversibly). **Cockpit glass**: sill gap-into-space closed + a slight haze added. **Airlock corner z-fight** fixed via `--probe`-confirmed pair (collar wall vs corridor wall → polygonOffset). New tooling: `scripts/floor-probe.mjs`, `scripts/pod-floor-shot.mjs`, ship-shot `--airlock=open` + all-hits diag. Details: [docs/changelog.md](docs/changelog.md) 2026-07-07 + D272-D276.

**NEXT:** the user keeps live-testing the intro + files feedback (→ the running steering queue — the airlock-corner z-fight needs a LIVE-MOTION confirm since the probe can't verify polygonOffset). Deferred: the hero ship explosion through the eject frame · the audio mix listen-pass · the out-of-loop queue (Skyfall hero wreck · CAVE rework · pickup-instancing · §A walk-tests) in [docs/roadmap.md](docs/roadmap.md). Boot the next session from [docs/next-session-prompt.md](docs/next-session-prompt.md).
