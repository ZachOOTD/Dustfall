...
import { findBiomeCentroid } from '../world/biomes.ts';         // review 2026-07-14 — __game.gotoAsh
...
interface DebugApi {
  ...
  /** Escape-pod W2 — swing the DOCKED bay-pod door (0=closed, 1=~110° open). For the pod-diag rig-shot. */
  setBayPodDoorOpen: (t: number) => void;
  ...
  /** Escape-pod round-2b — the chute DEFLATE/DRAPE life-cycle clock + phase name (for the rig strip). */
  chutePopT: () => { t: number; phase: string };
  /** Escape-pod round-2b — step the chute life-cycle by dt at a wind (0..1) + downwind dir (for a
   *  PAUSED rig strip; the camera stays fixed while the canopy motion ticks deterministically). */
  tickChute: (dt: number, wind: number, elapsed: number, dirX?: number, dirZ?: number) => { t: number; phase: string };
  ...
  /** Review — find the nearest ash-barren biome zone and warp there (the M12 biome +
   *  its industrial POI mix + any vultures en route). Null if none within range. */
  gotoAsh: () => { x: number; z: number } | null;
...
export function installDebugPanel(ctx: GameContext, hooks: DebugHooks = {}): void {
  ...
  window.__game = {
    ...
    setBayPodDoorOpen: (t) => { setBayPodDoorOpenDbg(t); },
    ...
    lightDebug: () => {
      ...
      return {
        ...
        sun: ctx.lights?.sun?.intensity,
        ambient: ctx.lights?.ambient?.intensity,
        cloudiness: ctx.weather?.cloudiness,
        introActive: !!ctx.intro?.active,
        introBeat: ctx.intro?.beat ?? null,
        fogNear: fog?.near ?? null,
        bg: bg instanceof THREE.Color ? bg.getHexString() : String(bg),
      };
    },
    ...
    gotoAsh: () => {
      // Ash-barren regions anchor ≥2600m out — search wide + coarse.
      const c = findBiomeCentroid(ctx.biomes, 'ash_barren', { searchRadius: 6000, gridStep: 150 });
      if (!c) { ctx.ui.showToast?.('no ash-barren zone within 6km — reload for a fresh seed'); return null; }
      const y = ctx.terrain.heightAt(c.x, c.z) + 2;
      ctx.player.body.body.setTranslation({ x: c.x, y, z: c.z }, true);
      ctx.player.cameraSnapNextFrame = true;
      ctx.ui.showToast?.('warped to the ash-barren biome');
