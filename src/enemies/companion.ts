...
/** M8 ⑩ (C52) — remove the deployed companion from the world WITHOUT granting a pod
 *  (distinct from packUpCompanion). Used by the boot reconcile for a NEW game where the
 *  egg hasn't been hatched yet — the boot always spawns the companion (so a Continue can
 *  patch it), and this undoes that spawn when `flags.companionAcquired` is false. */
