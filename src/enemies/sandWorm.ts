...
function makeWormMesh(): { group: THREE.Group } {
  ...
  // ─── M12 ⓕ (C65): the C13 dorsal-armor SCUTES were REMOVED per user direction
  //     ("remove the dorsal ridges"). The 13 keeled spine plates read as an armored
  //     stegosaurus-style sawback/crest; the user wants the smoother Dune-leviathan
  //     silhouette. The worm keeps its segmented body (the tapered segments + the
  //     circumferential ridge rings above, which read as body segmentation, NOT a
  //     dorsal crest). Removal consumes no rand (the scutes were hand-modeled). ───
