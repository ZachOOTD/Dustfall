...
// ── The orbit-view shaders (SELF-CONTAINED static orbit backdrop; NOT entangled with the
//    descent's setDescentProgress vista). Reuses the proven planet/atmosphere/star shader
//    idiom (podScene.ts) with fixed ORBIT uniforms. One dawn sun, baked + tone-mapped.
// Light up-and-LEFT, side-on so the day/night terminator curves visibly across the crown.
const _ORBIT_LIGHT = new THREE.Vector3(-0.78, 0.40, 0.30).normalize();
...
const PLANET_FS = /* glsl */ `
  ...
  void main(){
    ...
    // Desert palette — rust valleys → ochre body → pale tan highlands (wide tonal spread so
    // the banding + continents READ at this scale, not a billiard-ball gradient).
    ...
    float shade = 0.18 + 1.42*pow(ndlSurf, 0.78);
    float ao = mix(1.0, 0.60 + 0.40*smoothstep(0.25, 0.65, h), 0.55);
    vec3 sun = vec3(1.10,0.98,0.82);
    ...
    col += vec3(0.66,0.30,0.11) * term * 0.7;   // warm dawn crescent at the terminator
...
const ATMO_FS = /* glsl */ `
  ...
  void main(){
    ...
    vec3 blue  = vec3(0.26, 0.56, 1.05);
    vec3 white = vec3(0.85, 0.86, 0.92);
    vec3 warm  = vec3(1.05, 0.46, 0.16);
    vec3 tint = mix(warm, white, smoothstep(0.0, 0.30, k));
    tint = mix(tint, blue, smoothstep(0.22, 0.60, k));
    float glow = fres * day * 0.95;
...
const STAR_FS = /* glsl */ `
  ...
  vec3 starLayer(vec2 uv, float freq, float seed, float thresh){
    ...
    float hh = hash(cell+seed);
    if (hh <= thresh) return vec3(0.0);
    vec2 sp = vec2(hash(cell+seed+1.3), hash(cell+seed+7.7));
    float dd = length(f - sp);
    float bright = pow(hash(cell+seed+3.1), 2.4);
    float core = smoothstep(0.085, 0.0, dd) * (0.45 + 2.0*bright);
    float glow = smoothstep(0.30, 0.0, dd) * 0.16 * bright;
  ...
  void main(){
    ...
    vec3 stars = vec3(0.0);
    stars += starLayer(uv, 200.0, 0.0,  0.62);
    stars += starLayer(uv, 120.0, 31.0, 0.74);
    stars += starLayer(uv, 70.0,  61.0, 0.88) * 1.3;
    stars += starLayer(uv, 260.0, 91.0, 1.0 - band*0.45) * 0.7;
...
/** The ORBIT VIEW behind the forward window — a self-contained static orbit backdrop: a
 *  LARGE curved desert planet (the dominant payoff per the gate) + a Fresnel atmosphere limb
 *  + a deep starfield. Sized + placed so the curve, lit-side gradient, terminator, and limb
 *  are instantly legible filling the window. */
function buildOrbitView(group: THREE.Group): void {
  const starGeo = new THREE.PlaneGeometry(80, 80);
  _disposables.push(starGeo);
  const starMat = new THREE.ShaderMaterial({
    vertexShader: STAR_VS, fragmentShader: STAR_FS, depthWrite: true, toneMapped: true,
  });
  _buildMats.push(starMat);
  const stars = new THREE.Mesh(starGeo, starMat);
  stars.position.set(0, 1.4, -26);
  stars.frustumCulled = false;
  group.add(stars);

  // the planet body — LARGE so the curve + terminator + lit gradient fill the window. The
  // seated pilot eye is low + close to the window (≈ z−1.0), so the planet at z−10 fills a
  // wide arc of the windscreen.
  const PR = 7.5;
  const planetGeo = new THREE.SphereGeometry(PR, 80, 60);
  _disposables.push(planetGeo);
  const planetMat = new THREE.ShaderMaterial({
    vertexShader: PLANET_VS, fragmentShader: PLANET_FS,
    uniforms: { uLightDir: { value: _ORBIT_LIGHT.clone() } }, toneMapped: true,
  });
  _buildMats.push(planetMat);
  const planet = new THREE.Mesh(planetGeo, planetMat);
  // crown high in the window, body curving down to/below the sill — the world fills the port
  // from the seated pilot eye (~y1.30 looking slightly up over the dash).
  planet.position.set(0.6, -3.2, -10.5);
  planet.frustumCulled = false;
  group.add(planet);

  const atmoGeo = new THREE.SphereGeometry(PR * 1.15, 72, 54);
  _disposables.push(atmoGeo);
  const atmoMat = new THREE.ShaderMaterial({
    vertexShader: ATMO_VS, fragmentShader: ATMO_FS,
    uniforms: { uLightDir: { value: _ORBIT_LIGHT.clone() } },
    transparent: true, blending: THREE.AdditiveBlending,
    side: THREE.BackSide, depthWrite: false, toneMapped: false,
  });
  _buildMats.push(atmoMat);
  const atmo = new THREE.Mesh(atmoGeo, atmoMat);
  atmo.position.copy(planet.position);
  atmo.frustumCulled = false;
  group.add(atmo);

  const occGeo = new THREE.SphereGeometry(PR * 1.05, 48, 32);
  _disposables.push(occGeo);
  const occMat = new THREE.MeshBasicMaterial({ colorWrite: false, depthWrite: true, side: THREE.BackSide });
  _buildMats.push(occMat);
