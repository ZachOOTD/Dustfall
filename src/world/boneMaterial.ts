  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      /* glsl */ `
        ...
        float boneValueNoise(vec2 p) {
          vec2 i = floor(p);
          vec2 f = fract(p);
          vec2 u = f * f * (3.0 - 2.0 * f);
          float a = boneHash(i + vec2(0.0, 0.0));
          float b = boneHash(i + vec2(1.0, 0.0));
          float c = boneHash(i + vec2(0.0, 1.0));
          float d = boneHash(i + vec2(1.0, 1.0));
          return mix(mix(a, b, u.x), mix(c, d, u.x), u.y);
        }
        float boneFbm(vec2 p) {
          float v = 0.0;
          float a = 0.5;
          for (int i = 0; i < 4; i++) {
            v += a * boneValueNoise(p);
            p *= 2.0;
            a *= 0.5;
          }
          return v;
