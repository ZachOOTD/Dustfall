...
// origin-parity digests (4942306d / 33961250 — RE-BASELINED 2026-07-29 from d8f15005 / 99e0015b
...
export interface CaveKindParams {
   ...
   *    millimetre moves the origin cave's digest (4942306d), which is a hard campaign gate. At 0
...
export function assertCaveKindTable(): void {
  ...
  for (const kind of CAVE_KIND_LIST) {
    ...
    if (kind === 'canonical' && p.deadExplorer)
      at('deadExplorer must be false — the origin/egg cave builds with these params and must never carry the beat (origin parity 4942306d/33961250)');
    ...
    if (kind === 'canonical' && p.speleoSolidity !== 0)
      at('speleoSolidity must be EXACTLY 0 for canonical — `buildSpeleothem` bakes world-space vertices, so any non-zero value moves the origin cave digest (4942306d), which is a hard campaign gate');
  ...
  if (Object.keys(CAVE_KIND_OVERRIDES.canonical).length !== 0)
    bad.push('canonical: the override object must stay EMPTY — the origin/egg cave builds with it and origin parity (4942306d/33961250) is a hard campaign gate');
