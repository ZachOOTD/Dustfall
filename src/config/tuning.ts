...
export const Tuning = {
  ...
  CREVICE_LID_MIN_M: 0.42,             // m — the carved-hole lid's minimum thickness. MUST exceed CREVICE_TOR_VOXEL (0.32): surface nets cannot mesh a slab thinner than its grid, and a sub-voxel lid VANISHES (round 8d — a real 3×5m hole in the solid, four rounds of misattribution)
  CREVICE_LID_SAND_M: 0,               // RETIRED (round 8c): the sand-camouflage premise is falsified twice over — a material boundary cannot be hidden by matching albedo (the tor and the terrain SHADE differently), so the tint was first a pixel-level no-op and then, once the lid thinned, painted the hole-shaped lid as visible ORANGE RECT CORNERS. The lid is rock now: the organic outline (the edge lobes) is what kills the rectangle, and a low grey apron inside a lobed boundary reads as one rock. 0 disables the fade; the ground-derived rock tint (CREVICE_TOR_BIOME_MATCH) still carries the biome hue
  ...
  // Round 8 — THE VISIBLE-CONTOUR LOBES (the fix that targets what a viewer actually sees from
  // above: the ramp band, where sand tint hands over to rock). Two scales; expansion free, shrink
  // clamped so the edge can never reach the hole's cover band and re-trace the rect from inside.
  CREVICE_EDGE_LOBE_M: 2.4,            // m — coarse lobes (~14m wl). Round-8b: 1.6 → 2.4 — on a ~10m half-extent, ±1.6 was a nudge (the overhead still read as a rounded rect); organic needs 20-25% of the half-extent. Fold-safe: simplex grad ≈ amp·freq·2.6 = 0.44
  CREVICE_EDGE_LOBE2_M: 0.7,           // m — mid-scale (~6.5m): breaks the corner arcs and straight runs (grad 0.28; total 0.72 < 1 ⇒ no fold by construction)
  CREVICE_EDGE_LOBE_SHRINK_M: 1.9,     // m — max inward pull. 2.5 let the deepest notch pull the rock band off the thin lid, exposing a sand-tinted sliver that shades differently from real terrain (the tint matches albedo, not the material's response) — 1.9 keeps notch floors inside the grey band. The blob read survives: the bays are mostly expansion-side
  CREVICE_OUTLINE_WARP: 0,             // RETIRED (round 8): the domain warp bent only invisible sand-level bands — its anti-fold fade zeroed it exactly at the visible contour. Superseded by CREVICE_EDGE_LOBE_*; kept as a knob only so old saves of this file read sensibly           // m — domain-warp amplitude on the apron outline (round 4: bends the hole-rect's straight edges + corners into meanders; the cover contract stays unwarped)
