...
// DEEPER cycle 2 (2026-07-24) — `caveSdf` was RETIRED as a flag. The watertight SDF surface is the
// cave's only meshing path (the shell kit is deleted), so there is nothing left to select between.
// `VITE_CAVE=0` still kill-switches the whole cave. `VITE_CAVE_SDF_BENCH=1` re-polygonizes at the
// measurement resolutions and prints a CAVE-SDF-BENCH line (cost instrumentation, read directly
// from import.meta.env in caveGen.ts — not a feature flag).
